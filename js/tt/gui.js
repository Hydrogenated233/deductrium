...
import { isTTAssistTacticUnlocked, TT_ASSIST_COMMANDS, TTAssistEngine } from "./assist-engine.js";
...
export class TTGui {
    ...
    floatTypeDiv = null;
    initializeSemanticResourceScale() {
        let stored = null;
        try {
    ...
    setSemanticResourceScale(value) {
        ...
        if (this.semanticResourceScale === previous)
            return;
    ...
    getTacticCompletionContext() {
        ...
        const constants = active ? [
            ...
            ...this.userDefinedConsts.flatMap((definition, index) => definition && index !== targetIndex
    ...
    async replayTacticText(explicitRun, toCursor = false) {
        if (!this.tacticTextMode || !(this.mode instanceof Array))
            return;
        if (this.tacticBusy) {
            this.scheduleTacticTextReplay();
                ...
                if (ast && !error) {
                    try {
                        ...
                        if (ast.type === ":=") {
                            ...
                            if (workerCommitted)
                                this.coreWorker?.rememberDefinition(currentIdx, storedSlot);
                            else
            ...
            this.prepareCoreWorker(currentIdx, workerScopeId).then(() => {
                ...
                if (!shouldFallbackToSynchronousTheoremValidation(workerError)) {
                    ...
                    finish(ast, timedOut
                        ...
                        : String(workerError), undefined, undefined, false, false);
                    return;
                }
                validateSynchronously();
            });
