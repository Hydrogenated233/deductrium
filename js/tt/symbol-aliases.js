...
const aliasesByName = new Map(TYPE_THEORY_SYMBOL_ALIASES.map(({ alias, symbol }) => [alias.toLowerCase(), symbol]));
/** Resolve either `\\lambda` or `lambda` to its surface symbol. */
export function typeTheorySymbolForAlias(source) {
...
function isAliasNameChar(char) {
    return !!char && /[A-Za-z0-9_]/u.test(char);
}
function aliasQuotedEnd(source, start, quote) {
    let cursor = start + 1;
 ...
 */
export function expandTypeTheoryAliasesInSurface(source) {
 ...
 * browser DOM.
 */
export function expandTypeTheoryAliasAtCaret(value, selectionStart, selectionEnd = selectionStart) {
    ...
    if (match.index > 0 && beforeCaret[match.index - 1] === "\\")
        return null;
 ...
 */
export function installTypeTheorySymbolAliases(input) {
    ...
    input.addEventListener("keydown", event => {
        ...
        if (keyEvent.key !== " " || keyEvent.ctrlKey || keyEvent.altKey
            ...
            return;
        const expansion = expandTypeTheoryAliasAtCaret(input.value, input.selectionStart ?? input.value.length, input.selectionEnd ?? input.selectionStart ?? input.value.length);
        if (!expansion)
            return;
