...
function cloneHistory(history) {
    return Array.isArray(history)
        ? history.filter((command) => typeof command === "string")
        : [];
}
function normalizeTheoremIndex(index) {
...
export class TTProofSessionStore {
    ...
    nextId = 1;
    constructor(initial = [], activeId) {
        for (const session of initial)
            this.addInitialSession(session);
    ...
    openTheorem(init, reuseActiveBlank = false) {
        if (!init.theoremItemId)
            throw new Error("定理证明会话缺少定理编号");
        const targetTheoremIndex = normalizeTheoremIndex(init.targetTheoremIndex);
        if (targetTheoremIndex === null)
            throw new Error("定理证明会话缺少定理位置");
    ...
    openManual(init, reuseActiveBlank = false) {
        const active = reuseActiveBlank ? this.activeMutableSession() : null;
        if (active && this.isBlankSession(active)) {
    ...
    openGate(init, reuseActiveBlank = false) {
        const active = reuseActiveBlank ? this.activeMutableSession() : null;
        if (active && this.isBlankSession(active)) {
    ...
    update(id, patch) {
        ...
        if (typeof patch.target === "string")
            session.target = patch.target;
        if (Array.isArray(patch.history))
            session.history = cloneHistory(patch.history);
        if (typeof patch.script === "string")
            session.script = patch.script;
        if (patch.scopeFolderId === null || typeof patch.scopeFolderId === "string") {
            session.scopeFolderId = patch.scopeFolderId;
        }
        if (typeof patch.scopeExplicit === "boolean")
            session.scopeExplicit = patch.scopeExplicit;
        if (session.kind === "theorem") {
            if (patch.detached === true)
        ...
        if (typeof patch.stale === "boolean")
            session.stale = patch.stale;
        return cloneSession(session);
    }
    /** Update a moved theorem without activating its proof tab. */
    updateTheoremLocation(theoremItemId, targetTheoremIndex, scopeFolderId) {
        ...
        if (!session.scopeExplicit)
            session.scopeFolderId = scopeFolderId;
        session.stale ||= scopeChanged;
        return cloneSession(session);
    }
    /** Mark a theorem edit while retaining the user's accepted commands/text. */
    markTheoremTargetChanged(theoremItemId, target, targetTheoremIndex) {
        const session = this.findBoundTheoremSession(theoremItemId);
        if (!session)
            return null;
    ...
    static deserialize(serialized) {
        if (!serialized || !Array.isArray(serialized.sessions))
            return new TTProofSessionStore();
        const sessions = [];
        for (const value of serialized.sessions) {
            if (!value || typeof value !== "object")
                continue;
            if (value.kind !== "theorem" && value.kind !== "manual" && value.kind !== "gate")
                continue;
            if (typeof value.id !== "string" || !value.id || typeof value.target !== "string")
                continue;
        ...
        return new TTProofSessionStore(sessions, typeof serialized.activeId === "string" ? serialized.activeId : null);
    }
    create(init) {
    ...
    addInitialSession(init) {
        const id = init.id ?? this.allocateId();
        if (!id || this.sessionsById.has(id))
            throw new Error(`证明会话编号 ${id} 已存在`);
        const session = this.normalizeSession({ ...init, id });
        if (session.kind === "theorem" && !session.detached) {
            ...
            if (duplicateId || duplicateIndex)
                throw new Error("同一定理只能打开一个证明会话");
        }
        this.sessionsById.set(id, session);
        this.orderedIds.push(id);
        this.bumpCounter(id);
    }
    normalizeSession(init) {
        if (typeof init.target !== "string")
            throw new Error("证明会话缺少目标命题");
    ...
    findBoundTheoremSession(theoremItemId) {
    ...
    findTheoremSessionAt(targetTheoremIndex) {
        return this.orderedIds
            .map(id => this.sessionsById.get(id))
            .find(session => session.kind === "theorem"
            && !session.detached
    ...
    updateTheoremBinding(session, init, targetTheoremIndex) {
    ...
    makeRoomForTheoremSession(targetTheoremIndex) {
        if (!this.findTheoremSessionAt(targetTheoremIndex))
            return;
    ...
    moveTheoremSession(session, targetTheoremIndex) {
        ...
        for (const other of this.boundTheoremSessions()) {
            if (other.id === session.id || other.targetTheoremIndex === null)
                continue;
            if (previous < targetTheoremIndex
    ...
    requireSession(id) {
        const session = this.sessionsById.get(id);
        if (!session)
            throw new Error(`证明会话 ${id} 不存在`);
        return session;
    }
    allocateId() {
        let id;
        do
            id = `proof-session-${this.nextId++}`;
...
//# sourceMappingURL=proof-sessions.js.map