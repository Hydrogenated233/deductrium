...
import { assertCanonicalHitPathLevels, createHitPathLevels, flattenHitPathLevels, highestHitPathLevel, hitPathConstructorsAt, hitPathLevelsFromCanonicalOrLegacy } from "./hit-path-levels.js";
...
function sandboxHitOnePathExpressionIncludesRefl(expression, depth = 0) {
    if (depth > SANDBOX_HIT_ONE_PATH_EXPRESSION_MAX_DEPTH) {
        throw new Error("二维 HIT 一阶路径表达式嵌套过深");
    }
    ...
    if (expression.kind === "inverse") {
        return sandboxHitOnePathExpressionIncludesRefl(expression.value, depth + 1);
    }
    return sandboxHitOnePathExpressionIncludesRefl(expression.left, depth + 1)
        || sandboxHitOnePathExpressionIncludesRefl(expression.right, depth + 1);
}
...
function decomposeConstructorType(type, signatureName, parameters, indices, constructorName, constructorIndex) {
    ...
    rejectFreeHeaderIndexReferences(type, indices, `索引归纳构造子 ${constructorName}`);
    const arguments_ = [];
...
function rejectFreeHeaderIndexReferences(type, indices, owner) {
    const freeNames = new Set(collectFreeNames(type));
    for (const index of indices) {
        if (freeNames.has(index.name)) {
            throw new Error(owner + " 不能直接引用声明头部索引 " + index.name
...
function elaborateHitEndpoint(endpoint, signatureName, parameters, pointConstructors, pathName, boundNames, state = { nodes: 0 }, depth = 0) {
    if (depth > 128)
        throw new Error(`路径构造子 ${pathName} 的点端点嵌套过深`);
    if (++state.nodes > 4_096)
        throw new Error(`路径构造子 ${pathName} 的点端点节点过多`);
    ...
    for (let index = 0; index < point.argumentAsts.length; index++) {
        ...
        if (!recursive || recursive.length)
            continue;
        arguments_[parameters.length + index] = elaborateHitEndpoint(arguments_[parameters.length + index], signatureName, parameters, pointConstructors, pathName, boundNames, state, depth + 1).term;
    }
    ...
    return {
        ...
        resultIndices: point.resultIndices.map(index => substituteSandboxFreeVars(index, argumentReplacements))
    };
}
...
function elaborateHitTwoPathEndpoint(endpoint, signatureName, parameters, pointConstructors, pathConstructors, pathName, boundNames, budget = { nodes: 0 }, depth = 0) {
    if (depth > SANDBOX_HIT_ONE_PATH_EXPRESSION_MAX_DEPTH) {
    ...
    if (endpoint.type === "*" && endpoint.nodes?.[0] && endpoint.nodes?.[1]) {
        const left = elaborateHitTwoPathEndpoint(endpoint.nodes[0], signatureName, parameters, pointConstructors, pathConstructors, pathName, boundNames, budget, depth + 1);
        const right = elaborateHitTwoPathEndpoint(endpoint.nodes[1], signatureName, parameters, pointConstructors, pathConstructors, pathName, boundNames, budget, depth + 1);
    ...
    if (inverseTerms.length === 2
        ...
        const value = elaborateHitTwoPathEndpoint(inverseTerms[1], signatureName, parameters, pointConstructors, pathConstructors, pathName, boundNames, budget, depth + 1);
        return {
    ...
    const reflTerms = flattenApplication(endpoint);
    if (reflTerms.length === 2
        && reflTerms[0]?.type === "var"
        && reflTerms[0].name === "refl") {
        if (boundNames.has("refl")) {
            throw new Error(`二阶路径构造子 ${pathName} 的 refl 端点 refl 被局部参数遮蔽`);
        }
        const pointTerms = flattenApplication(reflTerms[1]);
        const pointName = pointTerms[0]?.type === "var" ? pointTerms[0].name : "";
        if (boundNames.has(pointName)) {
            throw new Error(`二阶路径构造子 ${pathName} 的 refl 端点 ${pointName} 被局部参数遮蔽`);
        }
        const point = pointConstructors.find(candidate => candidate.name === pointName);
        if (!point) {
            throw new Error(`二阶路径构造子 ${pathName} 的 refl 端点必须由 ${signatureName} 的点构造子形成`);
        }
        const localArguments = pointTerms.slice(1);
        if (localArguments.length !== point.argumentAsts.length) {
            throw new Error(`二阶路径构造子 ${pathName} 的 refl 端点 ${pointName} 参数数量错误：`
                + `需要 ${point.argumentAsts.length} 个点参数`);
        }
        const pointData = elaborateHitEndpoint(reflTerms[1], signatureName, parameters, pointConstructors, pathName, boundNames, budget, depth + 1);
        const pointArguments = flattenApplication(pointData.term)
            .slice(1 + parameters.length)
            .map(argument => Core.clone(argument));
        return {
            expression: { kind: "refl", pointName, arguments: pointArguments },
            term: sandboxApply(sandboxVar("refl"), pointData.term),
            sourcePoint: Core.clone(pointData.term),
            targetPoint: Core.clone(pointData.term),
            resultIndices: pointData.resultIndices.map(index => Core.clone(index))
        };
    }
    const terms = flattenApplication(endpoint);
    const headName = terms[0]?.type === "var" ? terms[0].name : "";
...
function elaborateHitThreePathEndpoint(endpoint, signatureName, parameters, pathConstructors, twoPathConstructors, pathName, boundNames, budget = { nodes: 0 }, depth = 0) {
    ...
    if (endpoint.type === "*" && endpoint.nodes?.[0] && endpoint.nodes?.[1]) {
        const left = elaborateHitThreePathEndpoint(endpoint.nodes[0], signatureName, parameters, pathConstructors, twoPathConstructors, pathName, boundNames, budget, depth + 1);
        const right = elaborateHitThreePathEndpoint(endpoint.nodes[1], signatureName, parameters, pathConstructors, twoPathConstructors, pathName, boundNames, budget, depth + 1);
        if (left.resultIndices.length !== right.resultIndices.length) {
    ...
    if (inverseTerms.length === 2
        && inverseTerms[0]?.type === "var"
        && inverseTerms[0].name === "inveq") {
        const value = elaborateHitThreePathEndpoint(inverseTerms[1], signatureName, parameters, pathConstructors, twoPathConstructors, pathName, boundNames, budget, depth + 1);
        return {
            expression: { kind: "inverse", value: value.expression },
            term: sandboxApply(sandboxVar("inveq"), value.term),
    ...
    if (terms[0]?.type === "var" && terms[0].name === "refl") {
        if (terms.length !== 2) {
            throw new Error(`三阶路径构造子 ${pathName} 的 refl 端点必须恰好引用一个一阶路径构造子`);
        }
        ...
        if (boundNames.has(firstPathName)) {
            throw new Error(`三阶路径构造子 ${pathName} 的 refl 端点 ${firstPathName} 被局部参数遮蔽`);
        }
        const firstPath = pathConstructors.find(candidate => candidate.name === firstPathName);
        if (!firstPath) {
            throw new Error(`三阶路径构造子 ${pathName} 的 refl 端点必须引用 ${signatureName} 的一阶路径构造子`);
        }
        ...
        if (supplied.length === firstPath.arguments.length) {
            ...
            for (let index = 0; index < parameters.length; index++) {
                const argument = supplied[index];
                if (argument?.type !== "var" || argument.name !== parameters[index].name) {
                    throw new Error(`三阶路径构造子 ${pathName} 的 refl 端点必须保持统一参数 ${parameters[index].name}`);
                }
            }
            arguments_ = supplied.map(argument => Core.clone(argument));
        }
        else {
            throw new Error(`三阶路径构造子 ${pathName} 的 refl 端点 ${firstPathName} 参数数量错误：`
        ...
        return {
            ...
            resultIndices: firstPath.resultIndices.map(index => substituteSandboxFreeVars(index, argumentReplacements))
        };
    }
    const headName = terms[0]?.type === "var" ? terms[0].name : "";
    if (boundNames.has(headName)) {
    ...
    return {
        ...
        resultIndices: twoPath.resultIndices.map(index => substituteSandboxFreeVars(index, argumentReplacements))
    };
}
...
export function parseSandboxHit(source) {
    ...
    if (inspection.firstUnsupportedPath) {
        ...
        throw new Error(`当前沙盒最高只解析三维 HIT：第 ${unsupported.sectionIndex} 个路径构造段`
    ...
    for (const path of pathSections) {
        ...
        names.add(path.name);
        rejectFreeHeaderIndexReferences(path.type, ordinary.indices, `一阶路径构造子 ${path.name}`);
    ...
    for (const path2 of twoPathSections) {
        ...
        rejectFreeHeaderIndexReferences(path2.type, ordinary.indices, `二阶路径构造子 ${path2.name}`);
        ...
        const elaboratedType = sandboxWrapPis(arguments_, sandboxEquality(Core.clone(left.term), Core.clone(right.term)));
    ...
    for (const path3 of threePathSections) {
        ...
        rejectFreeHeaderIndexReferences(path3.type, ordinary.indices, `三阶路径构造子 ${path3.name}`);
        ...
        const left = elaborateHitThreePathEndpoint(body.nodes[0], ordinary.name, ordinary.parameters, pathConstructors, twoPathConstructors, path3.name, endpointBoundNames);
        const right = elaborateHitThreePathEndpoint(body.nodes[1], ordinary.name, ordinary.parameters, pathConstructors, twoPathConstructors, path3.name, endpointBoundNames);
        if (ordinary.indices.length) {
            if (left.resultIndices.length !== ordinary.indices.length
                || right.resultIndices.length !== ordinary.indices.length) {
...
export function lowerSandboxInductive(signature, additionalReservedNames = []) {
...
function sandboxHitFiberType(signature, resultIndices) {
    return sandboxApply(sandboxVar(signature.name), ...signature.parameters.map(parameter => sandboxVar(parameter.name)), ...resultIndices.map(index => Core.clone(index)));
}
function sandboxHitFiberMotive(signature, motiveName, resultIndices) {
    if (!resultIndices.length)
        return sandboxVar(motiveName);
    ...
    return sandboxLambda(valueName, sandboxHitFiberType(signature, resultIndices), sandboxApply(sandboxVar(motiveName), ...resultIndices.map(index => Core.clone(index)), sandboxVar(valueName)));
}
function sandboxHitHeadAtIndices(head, resultIndices) {
    return resultIndices.length
        ? sandboxApply(Core.clone(head), ...resultIndices.map(index => Core.clone(index)))
        : Core.clone(head);
}
...
function sandboxHitReflPointExpressionData(expression, parameters, pointConstructors, owner) {
    if (typeof expression.pointName !== "string" || !expression.pointName
        || !Array.isArray(expression.arguments)) {
        throw new Error(`二维 HIT refl 点端点 ${owner} metadata 结构无效`);
    }
    const pointIndex = pointConstructors.findIndex(point => point.name === expression.pointName);
    if (pointIndex < 0) {
        throw new Error(`未知的二维 HIT refl 点端点：${expression.pointName || owner}`);
    }
    const point = pointConstructors[pointIndex];
    if (expression.arguments.length !== point.argumentAsts.length) {
        throw new Error(`二维 HIT refl 点端点 ${expression.pointName} 的参数数量与 metadata 不一致`);
    }
    ...
    return {
        ...
        pointIndex,
        arguments_,
        pointTerm: sandboxConstructorTerm(point.name, [
            ...parameters.map(parameter => sandboxVar(parameter.name)),
            ...arguments_
        ])
    };
}
function sandboxHitOnePathExpressionData(expression, parameters, pointConstructors, pathConstructors, owner) {
    if (expression.kind === "compose") {
        const left = sandboxHitOnePathExpressionData(expression.left, parameters, pointConstructors, pathConstructors, owner);
        const right = sandboxHitOnePathExpressionData(expression.right, parameters, pointConstructors, pathConstructors, owner);
        return {
            term: sandboxCompose(left.term, right.term),
            sourcePoint: left.sourcePoint,
            targetPoint: right.targetPoint
    ...
    if (expression.kind === "inverse") {
        const value = sandboxHitOnePathExpressionData(expression.value, parameters, pointConstructors, pathConstructors, owner);
        return {
            term: sandboxApply(sandboxVar("inveq"), value.term),
            sourcePoint: value.targetPoint,
            targetPoint: value.sourcePoint
    ...
    if (expression.kind === "refl") {
        const data = sandboxHitReflPointExpressionData(expression, parameters, pointConstructors, owner);
        return {
...
function sandboxHitOnePathExpressionMethodValue(expression, parameters, pointConstructors, pathConstructors, pointMethodNames, methodNames, owner) {
    if (expression.kind === "compose") {
        return sandboxCompose(sandboxHitOnePathExpressionMethodValue(expression.left, parameters, pointConstructors, pathConstructors, pointMethodNames, methodNames, owner), sandboxHitOnePathExpressionMethodValue(expression.right, parameters, pointConstructors, pathConstructors, pointMethodNames, methodNames, owner));
    }
    if (expression.kind === "inverse") {
        return sandboxApply(sandboxVar("inveq"), sandboxHitOnePathExpressionMethodValue(expression.value, parameters, pointConstructors, pathConstructors, pointMethodNames, methodNames, owner));
    }
    if (expression.kind === "refl") {
        const data = sandboxHitReflPointExpressionData(expression, parameters, pointConstructors, owner);
        const pointValue = sandboxHitBranchValue(data.pointTerm, parameters, pointConstructors, pointMethodNames);
        return sandboxApply(sandboxVar("refl"), pointValue);
    }
    const data = sandboxHitOnePathExpressionData(expression, parameters, pointConstructors, pathConstructors, owner);
    if (data.pathIndex === undefined || !data.arguments_) {
        throw new Error(`二维 HIT 一阶路径端点 ${owner} 缺少原子路径数据`);
    }
    return sandboxApply(sandboxVar(methodNames[data.pathIndex]), ...data.arguments_.map(argument => Core.clone(argument)));
}
...
function sandboxHitReflExpressionPathData(expression, parameters, pathConstructors, owner) {
...
function sandboxHitTwoPathExpressionTerm(expression, parameters, pathConstructors, twoPathConstructors, owner) {
    if (expression.kind === "compose") {
        return sandboxCompose(sandboxHitTwoPathExpressionTerm(expression.left, parameters, pathConstructors, twoPathConstructors, owner), sandboxHitTwoPathExpressionTerm(expression.right, parameters, pathConstructors, twoPathConstructors, owner));
    }
    if (expression.kind === "inverse") {
        return sandboxApply(sandboxVar("inveq"), sandboxHitTwoPathExpressionTerm(expression.value, parameters, pathConstructors, twoPathConstructors, owner));
    }
    if (expression.kind === "refl") {
        const data = sandboxHitReflExpressionPathData(expression, parameters, pathConstructors, owner);
    ...
    if (!path)
        throw new Error(`未知的三维 HIT 二阶路径端点：${expression.name || owner}`);
    if (expression.arguments.length !== path.arguments.length) {
...
function sandboxHitTwoPathExpressionBoundary(expression, parameters, pathConstructors, twoPathConstructors, owner) {
    if (expression.kind === "compose") {
        const left = sandboxHitTwoPathExpressionBoundary(expression.left, parameters, pathConstructors, twoPathConstructors, owner);
        const right = sandboxHitTwoPathExpressionBoundary(expression.right, parameters, pathConstructors, twoPathConstructors, owner);
        return { sourcePath: left.sourcePath, targetPath: right.targetPath };
    }
    if (expression.kind === "inverse") {
        const value = sandboxHitTwoPathExpressionBoundary(expression.value, parameters, pathConstructors, twoPathConstructors, owner);
...
function sandboxHitTwoPathExpressionOnePathBoundary(expression, parameters, pathConstructors, twoPathConstructors, owner) {
    if (expression.kind === "compose") {
        const left = sandboxHitTwoPathExpressionOnePathBoundary(expression.left, parameters, pathConstructors, twoPathConstructors, owner);
        const right = sandboxHitTwoPathExpressionOnePathBoundary(expression.right, parameters, pathConstructors, twoPathConstructors, owner);
        return { source: left.source, target: right.target };
    }
    if (expression.kind === "inverse") {
        const value = sandboxHitTwoPathExpressionOnePathBoundary(expression.value, parameters, pathConstructors, twoPathConstructors, owner);
    ...
    const mapArgument = (ast) => substituteSandboxFreeVars(ast, replacements);
    return {
...
function sandboxHitTwoPathExpressionMethodValue(expression, parameters, pathConstructors, pathMethodNames, twoPathConstructors, twoPathMethodNames, owner) {
    if (expression.kind === "compose") {
        return sandboxCompose(sandboxHitTwoPathExpressionMethodValue(expression.left, parameters, pathConstructors, pathMethodNames, twoPathConstructors, twoPathMethodNames, owner), sandboxHitTwoPathExpressionMethodValue(expression.right, parameters, pathConstructors, pathMethodNames, twoPathConstructors, twoPathMethodNames, owner));
    }
    if (expression.kind === "inverse") {
        return sandboxApply(sandboxVar("inveq"), sandboxHitTwoPathExpressionMethodValue(expression.value, parameters, pathConstructors, pathMethodNames, twoPathConstructors, twoPathMethodNames, owner));
    }
    if (expression.kind === "refl") {
        const data = sandboxHitReflExpressionPathData(expression, parameters, pathConstructors, owner);
        return sandboxApply(sandboxVar("refl"), sandboxApply(sandboxVar(pathMethodNames[data.pathIndex]), ...data.arguments_));
    }
    const pathIndex = twoPathConstructors.findIndex(path => path.name === expression.name);
    if (pathIndex < 0)
        throw new Error(`未知的三维 HIT 二阶路径端点：${expression.name || owner}`);
    return sandboxApply(sandboxVar(twoPathMethodNames[pathIndex]), ...expression.arguments.map(argument => Core.clone(argument)));
}
function sandboxHitTwoPathData(endpoint, parameters, twoPathConstructors, owner) {
    const terms = flattenApplication(endpoint);
    const pathName = terms[0]?.type === "var" ? terms[0].name : "";
    const pathIndex = twoPathConstructors.findIndex(path => path.name === pathName);
    if (pathIndex < 0)
 ...
 */
function sandboxHitPathData(endpoint, parameters, pointConstructors, pathConstructors, motiveName, branchNames, owner, motive = sandboxVar(motiveName)) {
    const terms = flattenApplication(endpoint);
    const pathName = terms[0]?.type === "var" ? terms[0].name : "";
    const pathIndex = pathConstructors.findIndex(path => path.name === pathName);
    if (pathIndex < 0) {
    ...
    const type = sandboxEquality(sandboxApply(sandboxVar("trans"), Core.clone(motive), pathTerm, leftBranch), rightBranch);
...
function sandboxRenameHitPathArguments(path, reserved) {
    ...
    for (const index of path.resultIndices)
        collectSandboxAstNames(index, occupied);
    const replacements = new Map();
    const arguments_ = [];
    for (const argument of path.arguments) {
    ...
    const resultIndices = path.resultIndices.map(index => renameFreeInductiveNames(index, replacements));
...
function sandboxRenameHitTwoPathArguments(path, reserved) {
    ...
    const left = renameFreeInductiveNames(path.left, replacements);
    const right = renameFreeInductiveNames(path.right, replacements);
    const leftExpression = sandboxMapHitOnePathExpression(path.leftExpression, ast => renameFreeInductiveNames(ast, replacements));
    const rightExpression = sandboxMapHitOnePathExpression(path.rightExpression, ast => renameFreeInductiveNames(ast, replacements));
...
function sandboxMapHitOnePathExpression(expression, mapAst) {
    if (expression.kind === "atom") {
        return {
            kind: "atom",
...
function sandboxMapHitTwoPathExpression(expression, mapAst) {
    if (expression.kind === "atom") {
        return {
            kind: "atom",
            name: expression.name,
 ...
 */
function assertSandboxHitOnePathMetadata(signature) {
    ...
    const assertAst = (value, label) => {
        ...
        while (stack.length) {
            ...
            seen.add(current);
            if (++nodes > 4_096)
                throw new Error(`${label} AST 节点过多`);
            const children = current.nodes;
    ...
    const assertAstArray = (value, expectedLength, label) => {
    ...
    for (const constructor of signature.pointConstructors) {
        ...
        assertAstArray(constructor.resultIndices, indexCount, `HIT 点构造子 ${constructor.name} resultIndices`);
        for (const argument of constructor.argumentAsts) {
            ...
            if (recursive) {
                if (!Array.isArray(telescope)) {
                    throw new Error(`HIT 点构造子 ${constructor.name} 递归参数 ${argument.name}`
                        + " 的 telescope 结构无效");
                }
                for (const [index, binder] of telescope.entries()) {
                    ...
                    assertAst(binder.type, `HIT 点构造子 ${constructor.name} 递归参数 ${argument.name}`
                        + ` telescope[${index}]`);
                }
                assertAstArray(argument.recursiveResultIndices, indexCount, `HIT 点构造子 ${constructor.name} 递归参数 ${argument.name}`
                    + " 的 resultIndices");
            }
            else if (argument.recursiveResultIndices !== null
                && argument.recursiveResultIndices !== undefined) {
                throw new Error(`HIT 点构造子 ${constructor.name} 非递归参数 ${argument.name}`
...
export function lowerSandboxHit(signature) {
    ...
    pathConstructors = pathConstructors.map(path => sandboxRenameHitPathArguments(path, reserved));
    twoPathConstructors = twoPathConstructors.map(path => sandboxRenameHitTwoPathArguments(path, reserved));
    threePathConstructors = threePathConstructors.map(path => sandboxRenameHitThreePathArguments(path, reserved));
    signature = {
        ...signature,
        pathLevels: createHitPathLevels(pathConstructors, twoPathConstructors, threePathConstructors)
    };
    const coherenceScope = new Set([
        ...reserved,
        ...pathConstructors.flatMap(path => path.arguments.map(argument => argument.name)),
        ...twoPathConstructors.flatMap(path => path.arguments.map(argument => argument.name)),
        ...threePathConstructors.flatMap(path => path.arguments.map(argument => argument.name))
    ]);
    const pathMethodNames = pathConstructors.map((_, index) => sandboxFreshName(`p${index}`, coherenceScope));
    const recursorPathMethodNames = pathConstructors.map((_, index) => sandboxFreshName(`q${index}`, coherenceScope));
    const dependentTwoPathMethodNames = twoPathConstructors.map((_, index) => sandboxFreshName(`p2_${index}`, coherenceScope));
    const recursorTwoPathMethodNames = twoPathConstructors.map((_, index) => sandboxFreshName(`q2_${index}`, coherenceScope));
    const dependentThreePathMethodNames = threePathConstructors.map((_, index) => sandboxFreshName(`p3_${index}`, coherenceScope));
    const recursorThreePathMethodNames = threePathConstructors.map((_, index) => sandboxFreshName(`q3_${index}`, coherenceScope));
    const parameterVars = signature.parameters.map(parameter => sandboxVar(parameter.name));
    ...
    const explicitDependentOnePathCompose = (domainType, motiveUniverseLevel, motive, left, right) => sandboxApply(sandboxVar("@hit_dep1_comp"), Core.clone(hitUniverseLevel), Core.clone(motiveUniverseLevel), Core.clone(domainType), Core.clone(left.sourcePoint), Core.clone(left.targetPoint), Core.clone(right.targetPoint), Core.clone(left.term), Core.clone(right.term), Core.clone(motive), Core.clone(left.sourceValue), Core.clone(left.targetValue), Core.clone(right.targetValue), Core.clone(left.method), Core.clone(right.method));
    const explicitDependentOnePathInverse = (domainType, motiveUniverseLevel, motive, value) => sandboxApply(sandboxVar("@hit_dep1_inv"), Core.clone(hitUniverseLevel), Core.clone(motiveUniverseLevel), Core.clone(domainType), Core.clone(value.sourcePoint), Core.clone(value.targetPoint), Core.clone(value.term), Core.clone(motive), Core.clone(value.sourceValue), Core.clone(value.targetValue), Core.clone(value.method));
    ...
    for (let index = 0; index < pathConstructors.length; index++) {
        ...
        const dependentType = sandboxWrapPis(path.arguments, sandboxEquality(sandboxApply(sandboxVar("trans"), fiberMotive, pathTerm, leftBranch), rightBranch));
    ...
    const dependentOnePathExpressionMethod = (expression, owner, motive = sandboxVar(motiveName), domainType = hitType, motiveUniverseLevel = sandboxVar("@0")) => {
        if (expression.kind === "compose") {
            const left = dependentOnePathExpressionMethod(expression.left, owner, motive, domainType, motiveUniverseLevel);
            const right = dependentOnePathExpressionMethod(expression.right, owner, motive, domainType, motiveUniverseLevel);
            const result = {
                term: sandboxCompose(left.term, right.term),
                sourcePoint: left.sourcePoint,
                targetPoint: right.targetPoint,
                ...
                method: sandboxHitOnePathExpressionIncludesRefl(expression)
                    ? explicitDependentOnePathCompose(domainType, motiveUniverseLevel, motive, left, right)
                    : sandboxApply(sandboxVar("hit_dep1_comp"), Core.clone(motive), left.sourceValue, left.targetValue, right.targetValue, left.method, right.method)
            };
            return result;
        }
        if (expression.kind === "inverse") {
            const value = dependentOnePathExpressionMethod(expression.value, owner, motive, domainType, motiveUniverseLevel);
            const result = {
                term: sandboxApply(sandboxVar("inveq"), value.term),
                sourcePoint: value.targetPoint,
                targetPoint: value.sourcePoint,
                ...
                method: sandboxHitOnePathExpressionIncludesRefl(expression)
                    ? explicitDependentOnePathInverse(domainType, motiveUniverseLevel, motive, value)
                    : sandboxApply(sandboxVar("hit_dep1_inv"), Core.clone(motive), value.sourceValue, value.targetValue, value.method)
            };
            return result;
        }
        if (expression.kind === "refl") {
            const data = sandboxHitReflPointExpressionData(expression, signature.parameters, signature.pointConstructors, owner);
            const pointValue = sandboxHitBranchValue(data.pointTerm, signature.parameters, signature.pointConstructors, branchNames);
            const result = {
                term: sandboxApply(sandboxVar("refl"), data.pointTerm),
                sourcePoint: Core.clone(data.pointTerm),
                targetPoint: Core.clone(data.pointTerm),
                sourceValue: Core.clone(pointValue),
                targetValue: Core.clone(pointValue),
                method: sandboxApply(sandboxVar("refl"), pointValue)
            };
            return result;
        }
        const data = sandboxHitOnePathExpressionData(expression, signature.parameters, signature.pointConstructors, pathConstructors, owner);
        if (data.pathIndex === undefined || !data.arguments_) {
            throw new Error(`二维 HIT 一阶路径端点 ${owner} 缺少原子路径数据`);
        }
        const sourceValue = sandboxHitBranchValue(data.sourcePoint, signature.parameters, signature.pointConstructors, branchNames);
        const targetValue = sandboxHitBranchValue(data.targetPoint, signature.parameters, signature.pointConstructors, branchNames);
        const result = {
            term: data.term,
            ...
            sourceValue,
            targetValue,
            method: sandboxApply(sandboxVar(pathMethodNames[data.pathIndex]), ...data.arguments_.map(argument => Core.clone(argument)))
        };
        return result;
    };
    const dependentTwoPathBinders = [];
    const fullDependentTwoPathBinders = [];
    ...
    for (let index = 0; index < twoPathConstructors.length; index++) {
        ...
        const leftDependent = dependentOnePathExpressionMethod(path.leftExpression, path.name, fiberMotive, fiberHitType, sandboxVar("@0"));
        const rightDependent = dependentOnePathExpressionMethod(path.rightExpression, path.name, fiberMotive, fiberHitType, sandboxVar("@0"));
        const fullLeftDependent = dependentOnePathExpressionMethod(path.leftExpression, path.name, fiberMotive, fiberHitType, sandboxVar(motiveUniverseName));
        const fullRightDependent = dependentOnePathExpressionMethod(path.rightExpression, path.name, fiberMotive, fiberHitType, sandboxVar(motiveUniverseName));
        ...
        const transportedRightMethod = {
            ...
            name: "",
            nodes: [
                sandboxApply(sandboxVar("trans2"), fiberMotive, sandboxConstructorTerm(path.name, [
                    ...parameterVars,
                    ...pathArguments
                ]), endpointValue),
                rightMethod
        ...
        const fullTransportedRightMethod = {
            type: "*",
            name: "",
            nodes: [
                sandboxApply(sandboxVar("trans2"), fiberMotive, sandboxConstructorTerm(path.name, [
                    ...parameterVars,
                    ...pathArguments
                ]), fullLeftDependent.sourceValue),
                fullRightDependent.method
            ]
        };
        const fullDependentType = sandboxWrapPis(path.arguments, sandboxEquality(fullLeftDependent.method, fullTransportedRightMethod));
        fullDependentTwoPathBinders.push({
            name: dependentTwoPathMethodNames[index],
            type: fullDependentType,
            typeSource: parser.stringify(fullDependentType)
        });
        const leftRecursorMethod = sandboxHitOnePathExpressionMethodValue(path.leftExpression, signature.parameters, signature.pointConstructors, pathConstructors, recursorBranchNames, recursorPathMethodNames, path.name);
        const rightRecursorMethod = sandboxHitOnePathExpressionMethodValue(path.rightExpression, signature.parameters, signature.pointConstructors, pathConstructors, recursorBranchNames, recursorPathMethodNames, path.name);
    ...
    const fullDependentThreePathBinders = [];
    const recursorThreePathBinders = [];
    const dependentTwoPathExpressionIncludesRefl = (expression, depth = 0) => {
        if (depth > SANDBOX_HIT_ONE_PATH_EXPRESSION_MAX_DEPTH) {
    ...
    const explicitDependentTwoPathCompose = (motiveUniverseLevel, domainType, source, right, motive) => sandboxApply(sandboxVar("@hit_dep2_comp"), Core.clone(hitUniverseLevel), Core.clone(motiveUniverseLevel), Core.clone(domainType), Core.clone(source.sourcePoint), Core.clone(source.targetPoint), Core.clone(source.sourcePath), Core.clone(source.targetPath), Core.clone(right.targetPath), Core.clone(motive), Core.clone(source.sourceValue), Core.clone(source.targetValue), Core.clone(source.sourceMethod), Core.clone(source.targetMethod), Core.clone(right.targetMethod), Core.clone(source.term), Core.clone(right.term), Core.clone(source.proof), Core.clone(right.proof));
    const explicitDependentTwoPathInverse = (motiveUniverseLevel, domainType, value, motive) => sandboxApply(sandboxVar("@hit_dep2_inv"), Core.clone(hitUniverseLevel), Core.clone(motiveUniverseLevel), Core.clone(domainType), Core.clone(value.sourcePoint), Core.clone(value.targetPoint), Core.clone(value.sourcePath), Core.clone(value.targetPath), Core.clone(motive), Core.clone(value.sourceValue), Core.clone(value.targetValue), Core.clone(value.sourceMethod), Core.clone(value.targetMethod), Core.clone(value.term), Core.clone(value.proof));
    const dependentTwoPathExpressionMethod = (expression, owner, sourceValue, targetValue, fiberMotive, fiberHitType, motiveUniverseLevel = sandboxVar("@0")) => {
        if (expression.kind === "compose") {
            const left = dependentTwoPathExpressionMethod(expression.left, owner, sourceValue, targetValue, fiberMotive, fiberHitType, motiveUniverseLevel);
            const right = dependentTwoPathExpressionMethod(expression.right, owner, sourceValue, targetValue, fiberMotive, fiberHitType, motiveUniverseLevel);
            const result = {
                term: sandboxCompose(left.term, right.term),
                sourcePath: left.sourcePath,
                targetPath: right.targetPath,
                ...
                proof: dependentTwoPathExpressionIncludesRefl(expression)
                    ? explicitDependentTwoPathCompose(motiveUniverseLevel, fiberHitType, left, right, fiberMotive)
                    : sandboxApply(sandboxVar("hit_dep2_comp"), Core.clone(fiberMotive), Core.clone(sourceValue), Core.clone(targetValue), left.sourceMethod, left.targetMethod, right.targetMethod, left.term, right.term, left.proof, right.proof)
            };
            return result;
        }
        if (expression.kind === "inverse") {
            const value = dependentTwoPathExpressionMethod(expression.value, owner, sourceValue, targetValue, fiberMotive, fiberHitType, motiveUniverseLevel);
            const result = {
                term: sandboxApply(sandboxVar("inveq"), value.term),
                sourcePath: value.targetPath,
                targetPath: value.sourcePath,
                ...
                proof: dependentTwoPathExpressionIncludesRefl(expression)
                    ? explicitDependentTwoPathInverse(motiveUniverseLevel, fiberHitType, value, fiberMotive)
                    : sandboxApply(sandboxVar("hit_dep2_inv"), Core.clone(fiberMotive), Core.clone(sourceValue), Core.clone(targetValue), value.sourceMethod, value.targetMethod, value.term, value.proof)
            };
            return result;
        }
        if (expression.kind === "refl") {
            const data = sandboxHitReflExpressionPathData(expression, signature.parameters, pathConstructors, owner);
            const method = sandboxApply(sandboxVar(pathMethodNames[data.pathIndex]), ...data.arguments_);
            const result = {
        ...
        const source = dependentOnePathExpressionMethod(sourceExpression, owner, fiberMotive, fiberHitType, motiveUniverseLevel);
        const target = dependentOnePathExpressionMethod(targetExpression, owner, fiberMotive, fiberHitType, motiveUniverseLevel);
        const result = {
            ...
            sourceValue,
            targetValue,
            proof: sandboxApply(sandboxVar(dependentTwoPathMethodNames[pathIndex]), ...expression.arguments.map(argument => Core.clone(argument)))
        };
        return result;
    };
    for (let index = 0; index < threePathConstructors.length; index++) {
        ...
        const fiberMotive = sandboxHitFiberMotive(signature, motiveName, path.resultIndices);
        const fiberHitType = sandboxHitFiberType(signature, path.resultIndices);
        const sourceValue = sandboxHitBranchValue(path.sourcePoint, signature.parameters, signature.pointConstructors, branchNames);
        const targetValue = sandboxHitBranchValue(path.targetPoint, signature.parameters, signature.pointConstructors, branchNames);
        const leftDependentExpression = dependentTwoPathExpressionMethod(path.leftExpression, path.name, sourceValue, targetValue, fiberMotive, fiberHitType, sandboxVar("@0"));
        const rightDependentExpression = dependentTwoPathExpressionMethod(path.rightExpression, path.name, sourceValue, targetValue, fiberMotive, fiberHitType, sandboxVar("@0"));
        const fullLeftDependentExpression = dependentTwoPathExpressionMethod(path.leftExpression, path.name, sourceValue, targetValue, fiberMotive, fiberHitType, sandboxVar(motiveUniverseName));
        const fullRightDependentExpression = dependentTwoPathExpressionMethod(path.rightExpression, path.name, sourceValue, targetValue, fiberMotive, fiberHitType, sandboxVar(motiveUniverseName));
        ...
        const coherenceFamily = sandboxLambda(pathValueName, sandboxEquality(Core.clone(path.sourcePath), Core.clone(path.targetPath)), sandboxEquality(sourceMethod, sandboxCompose(sandboxApply(sandboxVar("trans2"), Core.clone(fiberMotive), pathValue, endpointValue), targetMethod)));
        ...
        const fullCoherenceFamily = sandboxLambda(pathValueName, sandboxEquality(Core.clone(path.sourcePath), Core.clone(path.targetPath)), sandboxEquality(fullLeftDependentExpression.sourceMethod, sandboxCompose(sandboxApply(sandboxVar("trans2"), Core.clone(fiberMotive), fullPathValue, Core.clone(sourceValue)), fullLeftDependentExpression.targetMethod)));
        const fullDependentType = sandboxWrapPis(path.arguments, sandboxEquality(sandboxApply(sandboxVar("trans"), fullCoherenceFamily, pathTerm, fullLeftDependentExpression.proof), fullRightDependentExpression.proof));
        fullDependentThreePathBinders.push({
            name: dependentThreePathMethodNames[index],
            type: fullDependentType,
            typeSource: parser.stringify(fullDependentType)
        });
        const leftRecursorMethod = sandboxHitTwoPathExpressionMethodValue(path.leftExpression, signature.parameters, pathConstructors, recursorPathMethodNames, twoPathConstructors, recursorTwoPathMethodNames, path.name);
        const rightRecursorMethod = sandboxHitTwoPathExpressionMethodValue(path.rightExpression, signature.parameters, pathConstructors, recursorPathMethodNames, twoPathConstructors, recursorTwoPathMethodNames, path.name);
        const recursorType = sandboxWrapPis(path.arguments, sandboxEquality(leftRecursorMethod, rightRecursorMethod));
    ...
    const fullEliminatorType = sandboxInsertPis(fullEliminatorEntry[1], 1 + signature.parameters.length + 1 + signature.pointConstructors.length, allFullDependentPathBinders);
    ...
    for (let index = 0; index < pathConstructors.length; index++) {
        ...
        const fullRecursorHead = sandboxHitHeadAtIndices(sandboxApply(sandboxVar(`@rec_${signature.name}`), sandboxVar(motiveUniverseName), ...parameterVars, sandboxVar(motiveName), ...recursorBranchNames.map(name => sandboxVar(name)), ...recursorPathMethodNames.map(name => sandboxVar(name)), ...recursorTwoPathMethodNames.map(name => sandboxVar(name)), ...recursorThreePathMethodNames.map(name => sandboxVar(name))), path.resultIndices);
        ...
        fullApdType = sandboxWrapPis(allFullDependentPathBinders, fullApdType);
    ...
    const makeSharedDependentOnePathComputation = (expression, head, full, owner, motive = sandboxVar(motiveName), domainType = hitType) => {
        if (expression.kind === "compose") {
            const left = makeSharedDependentOnePathComputation(expression.left, head, full, owner, motive, domainType);
            const right = makeSharedDependentOnePathComputation(expression.right, head, full, owner, motive, domainType);
            const method = sandboxHitOnePathExpressionIncludesRefl(expression)
                ? explicitDependentOnePathCompose(domainType, full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), motive, left, right)
                : sandboxApply(sandboxVar("hit_dep1_comp"), Core.clone(motive), Core.clone(left.sourceValue), Core.clone(left.targetValue), Core.clone(right.targetValue), Core.clone(left.method), Core.clone(right.method));
            ...
            return {
                ...
                sourceValue: left.sourceValue,
                targetValue: right.targetValue,
                type: sandboxEquality(sandboxApply(sandboxVar("trans"), Core.clone(motive), Core.clone(term), Core.clone(left.sourceValue)), Core.clone(right.targetValue)),
                method,
                computation: sandboxApply(sandboxVar("@hit_apd1_corrected_comp"), Core.clone(hitUniverseLevel), full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), Core.clone(domainType), Core.clone(left.sourcePoint), Core.clone(left.targetPoint), Core.clone(right.targetPoint), Core.clone(left.term), Core.clone(right.term), Core.clone(motive), Core.clone(head), Core.clone(left.method), Core.clone(right.method), Core.clone(left.computation), Core.clone(right.computation))
            };
        }
        if (expression.kind === "inverse") {
            const value = makeSharedDependentOnePathComputation(expression.value, head, full, owner, motive, domainType);
            const term = sandboxApply(sandboxVar("inveq"), value.term);
            const method = sandboxHitOnePathExpressionIncludesRefl(expression)
                ? explicitDependentOnePathInverse(domainType, full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), motive, value)
                : sandboxApply(sandboxVar("hit_dep1_inv"), Core.clone(motive), Core.clone(value.sourceValue), Core.clone(value.targetValue), Core.clone(value.method));
            return {
                term,
                sourcePoint: value.targetPoint,
                targetPoint: value.sourcePoint,
                sourceValue: value.targetValue,
                targetValue: value.sourceValue,
                type: sandboxEquality(sandboxApply(sandboxVar("trans"), Core.clone(motive), Core.clone(term), Core.clone(value.targetValue)), Core.clone(value.sourceValue)),
                method,
                computation: sandboxApply(sandboxVar("@hit_apd1_corrected_inv"), Core.clone(hitUniverseLevel), full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), Core.clone(domainType), Core.clone(value.sourcePoint), Core.clone(value.targetPoint), Core.clone(value.term), Core.clone(motive), Core.clone(head), Core.clone(value.method), Core.clone(value.computation))
            };
        }
        if (expression.kind === "refl") {
            const data = sandboxHitReflPointExpressionData(expression, signature.parameters, signature.pointConstructors, owner);
            const pointValue = sandboxHitBranchValue(data.pointTerm, signature.parameters, signature.pointConstructors, branchNames);
            const term = sandboxApply(sandboxVar("refl"), data.pointTerm);
            const method = sandboxApply(sandboxVar("refl"), pointValue);
            return {
                term,
                sourcePoint: Core.clone(data.pointTerm),
                targetPoint: Core.clone(data.pointTerm),
                sourceValue: Core.clone(pointValue),
                targetValue: Core.clone(pointValue),
                type: sandboxEquality(sandboxApply(sandboxVar("trans"), Core.clone(motive), term, pointValue), Core.clone(pointValue)),
                method,
                computation: sandboxApply(sandboxVar("refl"), method)
            };
        }
        const data = sandboxHitOnePathExpressionData(expression, signature.parameters, signature.pointConstructors, pathConstructors, owner);
        if (data.pathIndex === undefined || !data.arguments_) {
        ...
        const sourceValue = sandboxHitBranchValue(data.sourcePoint, signature.parameters, signature.pointConstructors, branchNames);
        const targetValue = sandboxHitBranchValue(data.targetPoint, signature.parameters, signature.pointConstructors, branchNames);
        const method = sandboxApply(sandboxVar(pathMethodNames[data.pathIndex]), ...data.arguments_.map(argument => Core.clone(argument)));
        return {
            term: data.term,
            sourcePoint: data.sourcePoint,
            targetPoint: data.targetPoint,
            sourceValue,
            targetValue,
            type: sandboxEquality(sandboxApply(sandboxVar("trans"), Core.clone(motive), Core.clone(data.term), Core.clone(sourceValue)), Core.clone(targetValue)),
            method,
            computation: sandboxApply(sandboxVar(`${full ? "@" : ""}apd_${atom.name}`), ...(full ? [sandboxVar(motiveUniverseName)] : []), ...parameterVars, sandboxVar(motiveName), ...branchNames.map(name => sandboxVar(name)), ...pathMethodNames.map(name => sandboxVar(name)), ...dependentTwoPathMethodNames.map(name => sandboxVar(name)), ...dependentThreePathMethodNames.map(name => sandboxVar(name)), ...data.arguments_.map(argument => Core.clone(argument)))
        };
    };
    const makeSharedRecursorOnePathComputation = (expression, head, full, owner, domainType = hitType) => {
        if (expression.kind === "compose") {
            const left = makeSharedRecursorOnePathComputation(expression.left, head, full, owner, domainType);
            const right = makeSharedRecursorOnePathComputation(expression.right, head, full, owner, domainType);
            ...
            return {
                ...
                targetPoint: right.targetPoint,
                method,
                computation: sandboxApply(sandboxVar("@hit_ap1_corrected_comp"), Core.clone(hitUniverseLevel), full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), Core.clone(domainType), sandboxVar(motiveName), Core.clone(left.sourcePoint), Core.clone(left.targetPoint), Core.clone(right.targetPoint), Core.clone(left.term), Core.clone(right.term), Core.clone(head), Core.clone(left.method), Core.clone(right.method), Core.clone(left.computation), Core.clone(right.computation))
            };
        }
        if (expression.kind === "inverse") {
            const value = makeSharedRecursorOnePathComputation(expression.value, head, full, owner, domainType);
            ...
            return {
                ...
                targetPoint: value.sourcePoint,
                method,
                computation: sandboxApply(sandboxVar("@hit_ap1_corrected_inv"), Core.clone(hitUniverseLevel), full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), Core.clone(domainType), sandboxVar(motiveName), Core.clone(value.sourcePoint), Core.clone(value.targetPoint), Core.clone(value.term), Core.clone(head), Core.clone(value.method), Core.clone(value.computation))
            };
        }
        if (expression.kind === "refl") {
            const data = sandboxHitReflPointExpressionData(expression, signature.parameters, signature.pointConstructors, owner);
            const pointValue = sandboxHitBranchValue(data.pointTerm, signature.parameters, signature.pointConstructors, recursorBranchNames);
            const term = sandboxApply(sandboxVar("refl"), data.pointTerm);
            const method = sandboxApply(sandboxVar("refl"), pointValue);
            return {
                term,
        ...
        const atom = pathConstructors[data.pathIndex];
        const method = sandboxApply(sandboxVar(recursorPathMethodNames[data.pathIndex]), ...data.arguments_.map(argument => Core.clone(argument)));
        return {
            term: data.term,
            sourcePoint: data.sourcePoint,
            targetPoint: data.targetPoint,
            method,
            computation: sandboxApply(sandboxVar(`${full ? "@" : ""}ap_${atom.name}`), ...(full ? [sandboxVar(motiveUniverseName)] : []), ...parameterVars, sandboxVar(motiveName), ...recursorBranchNames.map(name => sandboxVar(name)), ...recursorPathMethodNames.map(name => sandboxVar(name)), ...recursorTwoPathMethodNames.map(name => sandboxVar(name)), ...recursorThreePathMethodNames.map(name => sandboxVar(name)), ...data.arguments_.map(argument => Core.clone(argument)))
        };
    };
    ...
    for (let index = 0; index < twoPathConstructors.length; index++) {
        ...
        const makeDependentOnePathComputation = (expression, head, full) => {
            if (expression.kind === "compose") {
                ...
                const method = sandboxHitOnePathExpressionIncludesRefl(expression)
                    ? explicitDependentOnePathCompose(fiberHitType, full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), fiberMotive, left, right)
                    : sandboxApply(sandboxVar("hit_dep1_comp"), Core.clone(fiberMotive), Core.clone(left.sourceValue), Core.clone(left.targetValue), Core.clone(right.targetValue), Core.clone(left.method), Core.clone(right.method));
                const term = sandboxCompose(left.term, right.term);
                return {
                    term,
                    sourcePoint: left.sourcePoint,
            ...
            if (expression.kind === "inverse") {
                ...
                const term = sandboxApply(sandboxVar("inveq"), value.term);
                const method = sandboxHitOnePathExpressionIncludesRefl(expression)
                    ? explicitDependentOnePathInverse(fiberHitType, full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), fiberMotive, value)
                    : sandboxApply(sandboxVar("hit_dep1_inv"), Core.clone(fiberMotive), Core.clone(value.sourceValue), Core.clone(value.targetValue), Core.clone(value.method));
                return {
                    term,
                    sourcePoint: value.targetPoint,
                    targetPoint: value.sourcePoint,
            ...
            if (expression.kind === "refl") {
                const data = sandboxHitReflPointExpressionData(expression, signature.parameters, signature.pointConstructors, path.name);
                const pointValue = sandboxHitBranchValue(data.pointTerm, signature.parameters, signature.pointConstructors, branchNames);
                const term = sandboxApply(sandboxVar("refl"), data.pointTerm);
                const method = sandboxApply(sandboxVar("refl"), pointValue);
                return {
                    ...
                    sourceValue: Core.clone(pointValue),
                    targetValue: Core.clone(pointValue),
                    type: sandboxEquality(sandboxApply(sandboxVar("trans"), Core.clone(fiberMotive), term, pointValue), Core.clone(pointValue)),
                    method,
                    computation: sandboxApply(sandboxVar("refl"), method)
                };
            }
            const data = sandboxHitOnePathExpressionData(expression, signature.parameters, signature.pointConstructors, pathConstructors, path.name);
            if (data.pathIndex === undefined || !data.arguments_) {
            ...
            return {
                ...
                sourceValue,
                targetValue,
                type: sandboxEquality(sandboxApply(sandboxVar("trans"), Core.clone(fiberMotive), Core.clone(data.term), Core.clone(sourceValue)), Core.clone(targetValue)),
                method,
                computation: sandboxApply(sandboxVar(`${full ? "@" : ""}apd_${atom.name}`), ...(full ? [sandboxVar(motiveUniverseName)] : []), ...parameterVars, sandboxVar(motiveName), ...branchNames.map(name => sandboxVar(name)), ...pathMethodNames.map(name => sandboxVar(name)), ...dependentTwoPathMethodNames.map(name => sandboxVar(name)), ...dependentThreePathMethodNames.map(name => sandboxVar(name)), ...data.arguments_.map(argument => Core.clone(argument)))
            };
        };
        const makeRecursorOnePathComputation = (expression, head, full) => {
            if (expression.kind === "compose") {
        ...
        const leftPathData = makeDependentOnePathComputation(path.leftExpression, dependentHead, false);
        const rightPathData = makeDependentOnePathComputation(path.rightExpression, dependentHead, false);
        const leftPathDataFull = makeDependentOnePathComputation(path.leftExpression, fullDependentHead, true);
        const rightPathDataFull = makeDependentOnePathComputation(path.rightExpression, fullDependentHead, true);
        const leftRecursorData = makeRecursorOnePathComputation(path.leftExpression, recursorHead, false);
        const rightRecursorData = makeRecursorOnePathComputation(path.rightExpression, recursorHead, false);
        const leftRecursorDataFull = makeRecursorOnePathComputation(path.leftExpression, fullRecursorHead, true);
        const rightRecursorDataFull = makeRecursorOnePathComputation(path.rightExpression, fullRecursorHead, true);
        const endpointValue = leftPathData.sourceValue;
        const transport2Value = sandboxApply(sandboxVar("trans2"), Core.clone(fiberMotive), pathTerm, endpointValue);
        ...
        fullApd2Type = sandboxWrapPis(allFullDependentPathBinders, fullApd2Type);
        ...
        const leftRecursorAction = sandboxApply(sandboxVar("ap"), recursorHead, leftRecursorData.term);
        const rightRecursorAction = sandboxApply(sandboxVar("ap"), recursorHead, rightRecursorData.term);
        ...
        const leftFullRecursorAction = sandboxApply(sandboxVar("ap"), fullRecursorHead, leftRecursorDataFull.term);
        const rightFullRecursorAction = sandboxApply(sandboxVar("ap"), fullRecursorHead, rightRecursorDataFull.term);
    ...
    for (let index = 0; index < threePathConstructors.length; index++) {
        ...
        const leftTwoPathTerm = sandboxHitTwoPathExpressionTerm(path.leftExpression, signature.parameters, pathConstructors, twoPathConstructors, path.name);
        const rightTwoPathTerm = sandboxHitTwoPathExpressionTerm(path.rightExpression, signature.parameters, pathConstructors, twoPathConstructors, path.name);
        const onePathBoundary = sandboxHitTwoPathExpressionOnePathBoundary(path.leftExpression, signature.parameters, pathConstructors, twoPathConstructors, path.name);
        const sourcePath = makeSharedDependentOnePathComputation(onePathBoundary.source, dependentHead, false, path.name, fiberMotive, fiberHitType);
        const targetPath = makeSharedDependentOnePathComputation(onePathBoundary.target, dependentHead, false, path.name, fiberMotive, fiberHitType);
        const sourcePathFull = makeSharedDependentOnePathComputation(onePathBoundary.source, fullDependentHead, true, path.name, fiberMotive, fiberHitType);
        const targetPathFull = makeSharedDependentOnePathComputation(onePathBoundary.target, fullDependentHead, true, path.name, fiberMotive, fiberHitType);
        const sourceRecursorPath = makeSharedRecursorOnePathComputation(onePathBoundary.source, recursorHead, false, path.name, fiberHitType);
        const targetRecursorPath = makeSharedRecursorOnePathComputation(onePathBoundary.target, recursorHead, false, path.name, fiberHitType);
        const sourceRecursorPathFull = makeSharedRecursorOnePathComputation(onePathBoundary.source, fullRecursorHead, true, path.name, fiberHitType);
        const targetRecursorPathFull = makeSharedRecursorOnePathComputation(onePathBoundary.target, fullRecursorHead, true, path.name, fiberHitType);
        ...
        const leftDependentExpression = dependentTwoPathExpressionMethod(path.leftExpression, path.name, endpointValue, targetEndpointValue, fiberMotive, fiberHitType);
        const rightDependentExpression = dependentTwoPathExpressionMethod(path.rightExpression, path.name, endpointValue, targetEndpointValue, fiberMotive, fiberHitType);
        const fullLeftDependentExpression = dependentTwoPathExpressionMethod(path.leftExpression, path.name, endpointValue, targetEndpointValue, fiberMotive, fiberHitType, sandboxVar(motiveUniverseName));
        const fullRightDependentExpression = dependentTwoPathExpressionMethod(path.rightExpression, path.name, endpointValue, targetEndpointValue, fiberMotive, fiberHitType, sandboxVar(motiveUniverseName));
        ...
        const makeDependentTwoPathComputation = (expression, head, full) => {
            if (expression.kind === "compose") {
                ...
                const method = dependentTwoPathExpressionIncludesRefl(expression)
                    ? explicitDependentTwoPathCompose(full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), fiberHitType, {
                        term: left.term,
                        sourcePath: left.sourcePath,
                        targetPath: left.targetPath,
                    ...
                    : sandboxApply(sandboxVar("hit_dep2_comp"), Core.clone(fiberMotive), Core.clone(endpointValue), Core.clone(targetEndpointValue), Core.clone(left.sourceMethod), Core.clone(left.targetMethod), Core.clone(right.targetMethod), Core.clone(left.term), Core.clone(right.term), Core.clone(left.method), Core.clone(right.method));
                return {
                    term: sandboxCompose(left.term, right.term),
                    sourcePath: left.sourcePath,
                    targetPath: right.targetPath,
                    ...
                    proof: sandboxApply(sandboxVar("@hit_apd2_corrected_comp"), Core.clone(hitUniverseLevel), full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), Core.clone(fiberHitType), Core.clone(path.sourcePoint), Core.clone(path.targetPoint), Core.clone(left.sourcePath), Core.clone(left.targetPath), Core.clone(right.targetPath), Core.clone(fiberMotive), Core.clone(head), Core.clone(left.sourceMethod), Core.clone(left.targetMethod), Core.clone(right.targetMethod), Core.clone(left.term), Core.clone(right.term), Core.clone(left.sourceComputation), Core.clone(left.targetComputation), Core.clone(right.targetComputation), Core.clone(left.method), Core.clone(right.method), Core.clone(left.proof), Core.clone(right.proof))
                };
            }
            if (expression.kind === "inverse") {
                const value = makeDependentTwoPathComputation(expression.value, head, full);
                const method = dependentTwoPathExpressionIncludesRefl(expression)
                    ? explicitDependentTwoPathInverse(full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), fiberHitType, {
                        term: value.term,
                    ...
                    : sandboxApply(sandboxVar("hit_dep2_inv"), Core.clone(fiberMotive), Core.clone(endpointValue), Core.clone(targetEndpointValue), Core.clone(value.sourceMethod), Core.clone(value.targetMethod), Core.clone(value.term), Core.clone(value.method));
                return {
                    term: sandboxApply(sandboxVar("inveq"), value.term),
                    sourcePath: value.targetPath,
                    targetPath: value.sourcePath,
                    ...
                    proof: sandboxApply(sandboxVar("@hit_apd2_corrected_inv"), Core.clone(hitUniverseLevel), full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), Core.clone(fiberHitType), Core.clone(path.sourcePoint), Core.clone(path.targetPoint), Core.clone(value.sourcePath), Core.clone(value.targetPath), Core.clone(fiberMotive), Core.clone(head), Core.clone(value.sourceMethod), Core.clone(value.targetMethod), Core.clone(value.term), Core.clone(value.sourceComputation), Core.clone(value.targetComputation), Core.clone(value.method), Core.clone(value.proof))
                };
            }
            if (expression.kind === "refl") {
                const data = sandboxHitReflExpressionPathData(expression, signature.parameters, pathConstructors, path.name);
                const sourceData = makeSharedDependentOnePathComputation({
                    kind: "atom",
                    name: data.path.name,
                    arguments: data.arguments_.map(argument => Core.clone(argument))
                }, head, full, path.name, fiberMotive, fiberHitType);
                ...
                return {
                    ...
                    proof: sandboxApply(sandboxVar("@hit_apd2_corrected_refl"), Core.clone(hitUniverseLevel), full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), Core.clone(fiberHitType), Core.clone(path.sourcePoint), Core.clone(path.targetPoint), Core.clone(data.pathTerm), Core.clone(fiberMotive), Core.clone(head), Core.clone(sourceMethod), Core.clone(sourceComputation))
                };
            }
            const pathIndex = twoPathConstructors.findIndex(item => item.name === expression.name);
            if (pathIndex < 0)
                throw new Error(`未知的三维 HIT 二阶路径端点：${expression.name}`);
            ...
            const mapArgument = (ast) => substituteSandboxFreeVars(ast, replacements);
            const atomSourceExpression = sandboxMapHitOnePathExpression(atom.leftExpression, mapArgument);
            const atomTargetExpression = sandboxMapHitOnePathExpression(atom.rightExpression, mapArgument);
            const sourceData = makeSharedDependentOnePathComputation(atomSourceExpression, head, full, path.name, fiberMotive, fiberHitType);
            const targetData = makeSharedDependentOnePathComputation(atomTargetExpression, head, full, path.name, fiberMotive, fiberHitType);
            const method = sandboxApply(sandboxVar(dependentTwoPathMethodNames[pathIndex]), ...expression.arguments.map(argument => Core.clone(argument)));
            return {
                term: sandboxHitTwoPathExpressionTerm(expression, signature.parameters, pathConstructors, twoPathConstructors, path.name),
                ...
                proof: sandboxApply(sandboxVar(`${full ? "@" : ""}apd_${atom.name}`), ...(full ? [sandboxVar(motiveUniverseName)] : []), ...parameterVars, sandboxVar(motiveName), ...branchNames.map(name => sandboxVar(name)), ...pathMethodNames.map(name => sandboxVar(name)), ...dependentTwoPathMethodNames.map(name => sandboxVar(name)), ...dependentThreePathMethodNames.map(name => sandboxVar(name)), ...expression.arguments.map(argument => Core.clone(argument)))
            };
        };
        ...
        const leftDependentTwoPathComputation = makeDependentTwoPathComputation(path.leftExpression, dependentHead, false).proof;
        const rightDependentTwoPathComputation = makeDependentTwoPathComputation(path.rightExpression, dependentHead, false).proof;
        const leftDependentTwoPathComputationFull = makeDependentTwoPathComputation(path.leftExpression, fullDependentHead, true).proof;
        const rightDependentTwoPathComputationFull = makeDependentTwoPathComputation(path.rightExpression, fullDependentHead, true).proof;
        ...
        const dependentCorrectedValueName = sandboxFreshName("dependentCorrectedValue", dependentCorrectionScope);
        const dependentTwoPathDomain = sandboxEquality(Core.clone(sourcePath.term), Core.clone(targetPath.term));
        const rawDependentFamily = (twoPathValue) => sandboxEquality(Core.clone(sourceDependentMethod), sandboxCompose(sandboxApply(sandboxVar("trans2"), Core.clone(fiberMotive), twoPathValue, Core.clone(endpointValue)), Core.clone(targetDependentMethod)));
        const actualDependentFamily = (head, twoPathValue) => sandboxEquality(sandboxApply(sandboxVar("apd"), Core.clone(head), Core.clone(sourcePath.term)), sandboxCompose(sandboxApply(sandboxVar("trans2"), Core.clone(fiberMotive), twoPathValue, Core.clone(endpointValue)), sandboxApply(sandboxVar("apd"), Core.clone(head), Core.clone(targetPath.term))));
        const dependentCorrection = (sourceComputation, targetComputation) => {
            ...
            const targetWhisker = sandboxLambda(dependentTargetPathName, Core.clone(targetPath.type), sandboxCompose(sandboxApply(sandboxVar("trans2"), Core.clone(fiberMotive), Core.clone(twoPathValue), Core.clone(endpointValue)), sandboxVar(dependentTargetPathName)));
        ...
        const mappedDependentMethod = sandboxApply(sandboxVar("hit_map_transport"), publicDependentCorrection, Core.clone(leftTwoPathTerm), Core.clone(rightTwoPathTerm), Core.clone(pathTerm), Core.clone(leftDependentTwoPathMethod), Core.clone(rightDependentTwoPathMethod), Core.clone(dependentMethodValue));
        const mappedDependentMethodFull = sandboxApply(sandboxVar("hit_map_transport"), fullDependentCorrection, Core.clone(leftTwoPathTerm), Core.clone(rightTwoPathTerm), Core.clone(pathTerm), Core.clone(fullLeftDependentTwoPathMethod), Core.clone(fullRightDependentTwoPathMethod), Core.clone(dependentMethodValue));
        ...
        const transportedComputation = (actualFamily, leftComputation) => {
            const leftActualType = actualDependentFamily(actualFamily === publicActualFamily ? dependentHead : fullDependentHead, Core.clone(leftTwoPathTerm));
            return sandboxApply(sandboxVar("ap"), sandboxLambda(dependentCorrectedValueName, leftActualType, sandboxApply(sandboxVar("trans"), Core.clone(actualFamily), Core.clone(pathTerm), sandboxVar(dependentCorrectedValueName))), leftComputation);
        };
        ...
        const correctionScope = new Set([
            ...coherenceScope,
            ...path.arguments.map(argument => argument.name)
        ]);
        const makeStrongTwoPathComputation = (expression, head, full) => {
            if (expression.kind === "compose") {
                ...
                return {
                    ...
                    sourceComputation: left.sourceComputation,
                    targetComputation: right.targetComputation,
                    proof: sandboxApply(sandboxVar("@hit_ap2_corrected_comp"), Core.clone(hitUniverseLevel), full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), Core.clone(fiberHitType), sandboxVar(motiveName), Core.clone(path.sourcePoint), Core.clone(path.targetPoint), Core.clone(left.sourcePath), Core.clone(left.targetPath), Core.clone(right.targetPath), Core.clone(head), Core.clone(left.sourceMethod), Core.clone(left.targetMethod), Core.clone(right.targetMethod), Core.clone(left.term), Core.clone(right.term), Core.clone(left.sourceComputation), Core.clone(left.targetComputation), Core.clone(right.targetComputation), Core.clone(left.corrected), Core.clone(right.corrected), Core.clone(left.proof), Core.clone(right.proof))
                };
            }
            if (expression.kind === "inverse") {
                ...
                return {
                    ...
                    sourceComputation: value.targetComputation,
                    targetComputation: value.sourceComputation,
                    proof: sandboxApply(sandboxVar("@hit_ap2_corrected_inv"), Core.clone(hitUniverseLevel), full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), Core.clone(fiberHitType), sandboxVar(motiveName), Core.clone(path.sourcePoint), Core.clone(path.targetPoint), Core.clone(value.sourcePath), Core.clone(value.targetPath), Core.clone(head), Core.clone(value.sourceMethod), Core.clone(value.targetMethod), Core.clone(value.term), Core.clone(value.sourceComputation), Core.clone(value.targetComputation), Core.clone(value.corrected), Core.clone(value.proof))
                };
            }
            if (expression.kind === "refl") {
                const data = sandboxHitReflExpressionPathData(expression, signature.parameters, pathConstructors, path.name);
                const sourceMethod = sandboxApply(sandboxVar(recursorPathMethodNames[data.pathIndex]), ...data.arguments_);
                const sourcePathData = sandboxHitPathData(data.pathTerm, signature.parameters, signature.pointConstructors, pathConstructors, motiveName, recursorBranchNames, path.name, fiberMotive);
                ...
                return {
                    ...
                    sourceComputation: Core.clone(sourceComputation),
                    targetComputation: Core.clone(sourceComputation),
                    proof: sandboxApply(sandboxVar("@hit_ap2_corrected_refl"), Core.clone(hitUniverseLevel), full ? sandboxVar(motiveUniverseName) : sandboxVar("@0"), Core.clone(fiberHitType), sandboxVar(motiveName), Core.clone(path.sourcePoint), Core.clone(path.targetPoint), Core.clone(data.pathTerm), Core.clone(head), Core.clone(sourceMethod), Core.clone(sourceComputation))
                };
            }
            const pathIndex = twoPathConstructors.findIndex(item => item.name === expression.name);
            if (pathIndex < 0)
                throw new Error(`未知的三维 HIT 二阶路径端点：${expression.name}`);
            const atom = twoPathConstructors[pathIndex];
            const proof = sandboxApply(sandboxVar(`${full ? "@" : ""}ap2_${atom.name}`), ...(full ? [sandboxVar(motiveUniverseName)] : []), ...parameterVars, sandboxVar(motiveName), ...recursorBranchNames.map(name => sandboxVar(name)), ...recursorPathMethodNames.map(name => sandboxVar(name)), ...recursorTwoPathMethodNames.map(name => sandboxVar(name)), ...recursorThreePathMethodNames.map(name => sandboxVar(name)), ...expression.arguments.map(argument => Core.clone(argument)));
            const boundary = sandboxHitTwoPathExpressionOnePathBoundary(expression, signature.parameters, pathConstructors, twoPathConstructors, path.name);
            const sourceData = makeSharedRecursorOnePathComputation(boundary.source, head, full, path.name, fiberHitType);
            const targetData = makeSharedRecursorOnePathComputation(boundary.target, head, full, path.name, fiberHitType);
            return {
                term: sandboxHitTwoPathExpressionTerm(expression, signature.parameters, pathConstructors, twoPathConstructors, path.name),
                sourcePath: sourceData.term,
                targetPath: targetData.term,
                sourceMethod: sourceData.method,
                targetMethod: targetData.method,
                corrected: sandboxHitTwoPathExpressionMethodValue(expression, signature.parameters, pathConstructors, recursorPathMethodNames, twoPathConstructors, recursorTwoPathMethodNames, path.name),
                sourceComputation: sourceData.computation,
                targetComputation: targetData.computation,
                proof
            };
        ...
        const leftStrongComputation = makeStrongTwoPathComputation(path.leftExpression, recursorHead, false).proof;
        const rightStrongComputation = makeStrongTwoPathComputation(path.rightExpression, recursorHead, false).proof;
        const leftStrongComputationFull = makeStrongTwoPathComputation(path.leftExpression, fullRecursorHead, true).proof;
        const rightStrongComputationFull = makeStrongTwoPathComputation(path.rightExpression, fullRecursorHead, true).proof;
        ...
        fullApd3Type = sandboxWrapPis(allFullDependentPathBinders, fullApd3Type);
        ...
        publicAp3Type = sandboxPi(motiveName, extractSandboxPiBinder(base.recursor[1], signature.parameters.length).type, publicAp3Type);
        publicAp3Type = sandboxWrapPis(signature.parameters, publicAp3Type);
        let fullAp3Type = sandboxWrapPis(path.arguments, sandboxEquality(sandboxApply(sandboxVar("ap3"), fullRecursorHead, Core.clone(pathTerm)), correctedMethodFull));
        fullAp3Type = sandboxWrapPis(allRecursorPathBinders, fullAp3Type);
        fullAp3Type = sandboxWrapPis(extractSandboxPiBinders(fullRecursorEntry[1], 1 + signature.parameters.length + 1, signature.pointConstructors.length), fullAp3Type);
    ...
    const metadataPathLevels = createHitPathLevels(metadataPathConstructors, metadataTwoPathConstructors, metadataThreePathConstructors);
...
export function inspectSandboxHitSource(source) {
    const text = String(source ?? "");
    const pattern = /(?:^|\|)\s*path([0-9]+)\b/gi;
    let maxPathLevel = 0;
    let sectionIndex = 0;
    let firstUnsupportedPath;
    for (let match = pattern.exec(text); match; match = pattern.exec(text)) {
        ...
        if (level <= 3 || firstUnsupportedPath)
            continue;
...
export function sandboxSourceLimitError(source, maxSourceChars) {
    const normalizedMaxSourceChars = normalizeSandboxLimit(maxSourceChars);
    if (normalizedMaxSourceChars === undefined)
        return undefined;
...
export class SandboxEnvironment {
    ...
    withRemainingCoreBudget(engine, budget, started, callback) {
        if (budget.timeoutMs === undefined)
            return callback();
        const remaining = Math.max(0, budget.timeoutMs - (performance.now() - started));
        return engine.core.withTimeoutBudget(remaining, Date.now() + remaining, callback);
    }
    add(source) {
    ...
    addInFolder(source, folderId) {
        ...
        const declaration = createSandboxDeclaration(source, id, this.validationBudget.maxSourceChars);
    ...
    replace(source, id) {
        ...
        const replacement = createSandboxDeclaration(source, id, this.validationBudget.maxSourceChars);
    ...
    validate(options = {}) {
        ...
        const sourceChars = orderedDeclarations.reduce((total, declaration) => total + declaration.source.length, 0);
        if (budget.maxSourceChars !== undefined && sourceChars > budget.maxSourceChars) {
            return this.validationInterrupted("budget-exhausted", started, 0, 0, `沙盒验证资源上限：源码字符 ${sourceChars} 超过 ${budget.maxSourceChars}`);
        }
        ...
        if (this.dirtyFrom < this.validatedThrough || this.validatedThrough > orderedDeclarations.length) {
            ...
            this.replayValidatedPrefix(orderedDeclarations, prefixLength, layout, budget, started);
        ...
        const incomplete = this.declarations.some(declaration => declaration.enabled && declaration.status === "unchecked");
    ...
    load(value, validationOptions = {}) {
        ...
        const rawDeclarations = parsed?.declarations;
        ...
        if (Array.isArray(rawDeclarations) && loadBudget.maxSourceChars !== undefined) {
            ...
            if (rawSourceChars > loadBudget.maxSourceChars) {
                return this.validationInterrupted("budget-exhausted", performance.now(), 0, 0, `沙盒验证资源上限：源码字符 ${rawSourceChars} 超过 ${loadBudget.maxSourceChars}`);
            }
        }
        ...
        this.declarations = save.declarations.map((raw, index) => {
            ...
            const declaration = createSandboxDeclaration(source, String(raw.id || `sandbox-${index + 1}`), this.validationBudget.maxSourceChars);
    ...
    replayValidatedPrefix(orderedDeclarations, prefixLength, layout, budget, started) {
    ...
    validationBudgetFor(options) {
        return {
            ...
            maxSourceChars: normalizeSandboxLimit(options.maxSourceChars ?? this.validationBudget.maxSourceChars),
