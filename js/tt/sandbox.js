...
export function lowerSandboxHit(signature) {
    ...
    for (let index = 0; index < twoPathConstructors.length; index++) {
        ...
        const pathArguments = path.arguments.map(argument => sandboxVar(argument.name));
        const leftMethod = sandboxHitPathMethodValue(path.left, signature.parameters, pathConstructors, pathMethodNames, path.name);
        const rightMethod = sandboxHitPathMethodValue(path.right, signature.parameters, pathConstructors, pathMethodNames, path.name);
        ...
        const leftRecursorMethod = sandboxHitPathMethodValue(path.left, signature.parameters, pathConstructors, recursorPathMethodNames, path.name);
        const rightRecursorMethod = sandboxHitPathMethodValue(path.right, signature.parameters, pathConstructors, recursorPathMethodNames, path.name);
    ...
    const dependentTwoPathExpressionMethod = (expression, owner, sourceValue, targetValue) => {
        ...
        const sourcePath = substituteSandboxFreeVars(path.left, replacements);
        ...
        return {
            ...
            sourceMethod: sandboxHitPathMethodValue(sourcePath, signature.parameters, pathConstructors, pathMethodNames, owner),
            targetMethod: sandboxHitPathMethodValue(targetPath, signature.parameters, pathConstructors, pathMethodNames, owner),
    ...
    for (let index = 0; index < threePathConstructors.length; index++) {
        ...
        const sourceMethod = sandboxHitPathMethodValue(path.sourcePath, signature.parameters, pathConstructors, pathMethodNames, path.name);
        const targetMethod = sandboxHitPathMethodValue(path.targetPath, signature.parameters, pathConstructors, pathMethodNames, path.name);
    ...
    for (let index = 0; index < twoPathConstructors.length; index++) {
        ...
        const leftPathData = sandboxHitPathData(path.left, signature.parameters, signature.pointConstructors, pathConstructors, motiveName, branchNames, path.name);
        const rightPathData = sandboxHitPathData(path.right, signature.parameters, signature.pointConstructors, pathConstructors, motiveName, branchNames, path.name);
        const endpointValue = sandboxHitBranchValue(path.leftPoint, signature.parameters, signature.pointConstructors, branchNames);
    ...
    for (let index = 0; index < threePathConstructors.length; index++) {
        ...
        const sourcePath = sandboxHitPathData(path.sourcePath, signature.parameters, signature.pointConstructors, pathConstructors, motiveName, branchNames, path.name);
        const targetPath = sandboxHitPathData(path.targetPath, signature.parameters, signature.pointConstructors, pathConstructors, motiveName, branchNames, path.name);
        const sourceDependentMethod = sandboxHitPathMethodValue(path.sourcePath, signature.parameters, pathConstructors, pathMethodNames, path.name);
        const targetDependentMethod = sandboxHitPathMethodValue(path.targetPath, signature.parameters, pathConstructors, pathMethodNames, path.name);
        const endpointValue = sandboxHitBranchValue(path.sourcePoint, signature.parameters, signature.pointConstructors, branchNames);
        const targetEndpointValue = sandboxHitBranchValue(path.targetPoint, signature.parameters, signature.pointConstructors, branchNames);
        ...
        const makeDependentTwoPathComputation = (expression, head, full) => {
            ...
            if (expression.kind === "refl") {
                ...
                const sourceMethod = sandboxApply(sandboxVar(pathMethodNames[data.pathIndex]), ...data.arguments_);
                const sourcePathData = sandboxHitPathData(data.pathTerm, signature.parameters, signature.pointConstructors, pathConstructors, motiveName, branchNames, path.name);
            ...
            const atomSourcePath = substituteSandboxFreeVars(atom.left, replacements);
            const atomTargetPath = substituteSandboxFreeVars(atom.right, replacements);
            const sourceMethod = sandboxHitPathMethodValue(atomSourcePath, signature.parameters, pathConstructors, pathMethodNames, path.name);
            const targetMethod = sandboxHitPathMethodValue(atomTargetPath, signature.parameters, pathConstructors, pathMethodNames, path.name);
            ...
            const sourcePathData = sandboxHitPathData(atomSourcePath, signature.parameters, signature.pointConstructors, pathConstructors, motiveName, branchNames, path.name);
            const targetPathData = sandboxHitPathData(atomTargetPath, signature.parameters, signature.pointConstructors, pathConstructors, motiveName, branchNames, path.name);
            return {
                ...
                targetMethod,
                method,
                sourceComputation: makeDependentPathComputation(sourcePathData, full),
                targetComputation: makeDependentPathComputation(targetPathData, full),
        ...
        const makeStrongTwoPathComputation = (expression, head, full) => {
            ...
            const boundary = sandboxHitTwoPathExpressionBoundary(expression, signature.parameters, pathConstructors, twoPathConstructors, path.name);
            const sourceMethod = sandboxHitPathMethodValue(boundary.sourcePath, signature.parameters, pathConstructors, recursorPathMethodNames, path.name);
            const targetMethod = sandboxHitPathMethodValue(boundary.targetPath, signature.parameters, pathConstructors, recursorPathMethodNames, path.name);
            const sourcePathData = sandboxHitPathData(boundary.sourcePath, signature.parameters, signature.pointConstructors, pathConstructors, motiveName, recursorBranchNames, path.name);
            const targetPathData = sandboxHitPathData(boundary.targetPath, signature.parameters, signature.pointConstructors, pathConstructors, motiveName, recursorBranchNames, path.name);
            return {
                term: sandboxHitTwoPathExpressionTerm(expression, signature.parameters, pathConstructors, twoPathConstructors, path.name),
        ...
        const rightPathComputation = makeRecursorPathComputation(targetPath, false);
        const leftPathComputationFull = makeRecursorPathComputation(sourcePath, true);
        const rightPathComputationFull = makeRecursorPathComputation(targetPath, true);
        ...
        const sourceMethod = sandboxHitPathMethodValue(path.sourcePath, signature.parameters, pathConstructors, recursorPathMethodNames, path.name);
        const targetMethod = sandboxHitPathMethodValue(path.targetPath, signature.parameters, pathConstructors, recursorPathMethodNames, path.name);
