...
export function validBondVarId(id) {
    return typeof id === "number" && Number.isFinite(id) && id > 0;
}
export function isBinderType(type) {
    return type === "L" || type === "P" || type === "S" || type === "W";
}
export function isBinderNode(ast) {
...
export function referencesBoundBinder(ast, targetId) {
    if (!validBondVarId(targetId))
        return false;
    const visit = (node, shadowed) => {
        ...
        if (isBinderNode(node)) {
            return visit(node.nodes?.[0], shadowed)
                || visit(node.nodes?.[1], shadowed || node.bondVarId === targetId);
        }
        return (node.nodes ?? []).some(child => visit(child, shadowed));
    };
    return visit(ast, false);
}
function contextIsCached(context, cached) {
 ...
 * checker call.
 */
export function getContextIndex(context) {
    if (!context.length)
        return EMPTY_CONTEXT_INDEX;
    const cached = contextIndexes.get(context);
    if (cached && contextIsCached(context, cached))
        return cached;
    const index = {
        byName: new Map(),
        byBondVarId: new Map(),
    ...
    for (let position = 0; position < context.length; position++) {
        ...
        if (validBondVarId(id))
            index.nextId = Math.max(index.nextId, id + 1);
        index.bindings.push({
            name,
...
export function cloneSyntax(ast) {
    if (!ast)
        return ast;
    return {
        type: ast.type,
...
export function contextWithScope(scope, context) {
    if (!scope.length)
        return context;
 ...
 */
export function lookupScope(ast, scope, sourceIdsOnly = false) {
    if (scope instanceof ScopeCursor)
        return scope.lookup(ast, sourceIdsOnly);
...
export function lookupScopePosition(ast, scope, sourceIdsOnly = false) {
    if (scope instanceof ScopeCursor) {
    ...
    if (validBondVarId(ast?.bondVarId)) {
        ...
        if (sourcePosition >= 0)
            return { binding: scope[sourcePosition], position: sourcePosition };
        if (sourceIdsOnly)
            return undefined;
...
export function findKernelScopeIndex(ast, scope) {
    if (scope instanceof ScopeCursor)
        return scope.findIndex(ast);
    if (validBondVarId(ast?.bondVarId)) {
...
export function scopePosition(ast, scope) {
    if (scope instanceof ScopeCursor)
        return scope.position(ast);
    if (validBondVarId(ast?.bondVarId)) {
        return scope.findIndex(binding => binding.id === ast.bondVarId
...
export class ScopeCursor {
    entries = [];
    idPositions = new Map();
    sourceIdPositions = new Map();
    unmarkedSourceNamePositions = new Map();
    unmarkedNamePositions = new Map();
    ...
    lookup(ast, sourceIdsOnly = false) {
        if (validBondVarId(ast?.bondVarId)) {
    ...
    pop() {
        ...
        if (!namePositions?.length)
            this.namePositions.delete(binding.name);
        if (validBondVarId(binding.id)) {
            const positions = this.idPositions.get(binding.id);
            positions?.pop();
            if (!positions?.length)
                this.idPositions.delete(binding.id);
        }
        else {
            const positions = this.unmarkedNamePositions.get(binding.name);
            positions?.pop();
            if (!positions?.length)
                this.unmarkedNamePositions.delete(binding.name);
        }
        if (validBondVarId(binding.sourceId)) {
            const positions = this.sourceIdPositions.get(binding.sourceId);
            positions?.pop();
            if (!positions?.length)
                this.sourceIdPositions.delete(binding.sourceId);
        }
        else {
            const positions = this.unmarkedSourceNamePositions.get(binding.name);
            positions?.pop();
            if (!positions?.length)
                this.unmarkedSourceNamePositions.delete(binding.name);
        }
        return binding;
    }
    withBinding(binding, callback) {
        this.push(binding);
        try {
            return callback();
        }
...
export function markScopedBondVars(ast, context, allocateId) {
    const scope = new Map();
    ...
    const visit = (node) => {
        ...
        if (node.type === "var") {
            if (!node.bondVarId)
                node.bondVarId = scope.get(node.name)?.[2];
            return;
        }
        if (isBinderNode(node)) {
            if (node.bondVarId)
...
export function collectFreeBondVarIds(ast, result = new Set(), scope = new Set()) {
    if (!ast || typeof ast !== "object")
        return result;
    if (ast.type === "var" && validBondVarId(ast.bondVarId)) {
        if (!scope.has(ast.bondVarId))
...
//# sourceMappingURL=scoped-syntax.js.map