 ...
 */
const BINDER_SYMBOLS = Object.freeze({
    L: "λ",
    P: "Π",
    S: "Σ"
});
const LEGACY_OPERATORS = Object.freeze([
    ["===", "≡"],
    ["~=", "≃"],
    ["->", "→"],
    ["*", "▪"]
...
function isProtectedWordAt(source, index) {
    if (index > 0 && isNameChar(source[index - 1]))
        return false;
    let end = index;
...
function binderAt(source, index, options) {
    ...
    if (isProtectedWordAt(source, index))
        return null;
    ...
    if (!options.allowLongBinders && cursor - variableStart > 1)
        return null;
...
function compactIdentifierAround(source, index, direction) {
...
function isKnownLegacyProductAtom(name) {
 ...
 */
function productAt(source, index) {
    if (source[index] !== "X")
        return false;
    ...
    if (isNameChar(immediateLeft) && immediateRight === "(") {
        ...
        if (leftName.length > 1 && !isKnownLegacyProductAtom(leftName))
            return false;
    }
    ...
    if (isNameChar(immediateLeft)
        && (!immediateRight || isWhitespace(immediateRight)))
        return false;
    if ((!immediateLeft || isWhitespace(immediateLeft))
        && isNameChar(immediateRight))
        return false;
    let left = index - 1;
    while (left >= 0 && isWhitespace(source[left]))
        left--;
    let right = index + 1;
    while (right < source.length && isWhitespace(source[right]))
        right++;
    if (!isTermEnd(source[left]) || !isTermStart(source[right]))
        return false;
 ...
 */
function isCompositionAliasAt(source, index) {
...
export function migrateLegacySurfaceExpression(source, options = {}) {
...
function topLevelDelimiter(source, delimiter) {
    let paren = 0;
    let bracket = 0;
    let brace = 0;
    let cursor = 0;
    while (cursor < source.length) {
        ...
        if (paren === 0 && bracket === 0 && brace === 0) {
            if (delimiter === ":=" && char === ":" && source[cursor + 1] === "=")
                return cursor;
            if (delimiter === ":" && char === ":" && source[cursor + 1] !== "=")
                return cursor;
        }
...
export function migrateLegacyDeclarationSource(source) {
    if (!source || typeof source !== "string")
        return source;
    const assignment = topLevelDelimiter(source, ":=");
    if (assignment >= 0) {
        const rhsStart = assignment + 2;
        return source.slice(0, rhsStart) + migrateLegacySurfaceExpression(source.slice(rhsStart), { allowLongBinders: true });
    }
...
export function migrateLegacyProofCommand(command) {
    if (!command || typeof command !== "string")
        return command;
    let cursor = 0;
    while (cursor < command.length && isWhitespace(command[cursor]))
        cursor++;
    ...
    if (cursor === commandStart)
        return command;
    return command.slice(0, cursor) + migrateLegacySurfaceExpression(command.slice(cursor), { allowLongBinders: true });
}
export function migrateLegacyProofHistory(history) {
...
export function migrateLegacyProofScript(script) {
...
export function hasLegacySurfaceSyntax(source) {
    return typeof source === "string" && migrateLegacySurfaceExpression(source) !== source;
}
export function hasModernSurfaceSyntax(source) {
    return typeof source === "string" && /[λΠΣ×→≃▪≡]/u.test(source);
}
//# sourceMappingURL=surface-syntax-migration.js.map