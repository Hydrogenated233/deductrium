...
const certifiedPathComputationResourceLimits = Object.freeze({
    inputMaxNodes: 4_096,
 ...
 */
const certifiedHitStructureResourceLimits = Object.freeze({
...
function legacyCoreHitTwoPathExpression(endpoint, expectedName, parameterCount, label) {
    ...
    const name = generatedFreeConstantName(application.head);
    if (!name || name !== expectedName) {
        throw new Error(`${label} legacy 三阶路径端点头与 metadata 不一致：`
            + `${name ?? "<非常量>"} != ${expectedName}`);
    }
    if (application.arguments.length < parameterCount) {
...
function migrateLegacyCoreHitThreePathMetadata(path, twoPathByName, parameterCount) {
    const label = `三维 HIT 三阶路径构造子 ${path.name}`;
    const leftExpression = legacyCoreHitTwoPathExpression(path.left, path.leftTwoPath, parameterCount, `${label} 左端点`);
    const rightExpression = legacyCoreHitTwoPathExpression(path.right, path.rightTwoPath, parameterCount, `${label} 右端点`);
    if (!path.sourcePath || !path.targetPath) {
        throw new Error(`${label} legacy 边界 metadata 不完整`);
    }
    const boundaryFor = (expression, side) => {
        if (expression.kind !== "atom")
            throw new Error(`${label} ${side}端点不是 direct atom`);
...
export class Core {
    ...
    withTimeoutBudget(timeout, deadline, callback) {
    ...
    certifiedLargeSystemTypes = new Set();
    silentErrors = 0;
    ...
    validateCanonicalInductiveRuleTypes(bundle, normalizedEntries, normalizedRules, parameters, indexCount, semanticResourceLimits) {
        ...
        const checkAgainst = (term, expected, context, label) => {
            try {
                ...
                }, Core.cloneContext(context), false, undefined, false, true, false, semanticResourceLimits));
            }
            catch {
    ...
    registerSystemInductive(bundle) {
        ...
        const canonicallyCertifiedTypeFormations = new Set();
        if (bundle.metadata?.kind === "hit1"
            || bundle.metadata?.kind === "hit2"
            || bundle.metadata?.kind === "hit3") {
            const metadata = bundle.metadata;
            ...
            const requireHitIndexEquality = (left, right, context, label, mismatch = "不在同一索引纤维") => {
            ...
            const requireHitBoundaryEquality = (left, right, context, label) => {
                if (sameGeneratedAstAlpha(left, right))
                    return;
                requireHitIndexEquality(left, right, context, label, "不一致");
            };
            const hitPathIndexContext = (argumentNames, argumentTypes) => {
                const context = [];
                let nextContextId = 1;
            ...
            const evaluateOnePathExpression = (expression, owner, side, indexContext = [], state = { nodes: 0, ancestors: new WeakSet() }, depth = 0) => {
                ...
                try {
                    ...
                    if (expression.kind === "compose") {
                        ...
                        if (indexCount > 0) {
                            ...
                            for (let index = 0; index < indexCount; index++) {
                                requireHitIndexEquality(leftIndices[index], rightIndices[index], indexContext, `${label}组合项第 ${index + 1} 个索引`);
                            }
                        }
                        requireHitBoundaryEquality(left.targetPoint, right.sourcePoint, indexContext, `${label}组合项的中间点边界`);
                        return {
                            kind: "compose",
                            left,
                            right,
                    ...
                    if (expression.kind === "inverse") {
                        const value = evaluateOnePathExpression(expression.value, owner, side, indexContext, state, depth + 1);
                        return {
                            kind: "inverse",
                            value,
            ...
            for (const path of metadataTwoPathEntries) {
                ...
                const leftEndpoint = evaluateOnePathExpression(path.leftExpression, path.name, "左", pathIndexContext);
                const rightEndpoint = evaluateOnePathExpression(path.rightExpression, path.name, "右", pathIndexContext);
                const pathResultIndices = path.resultIndices ?? [];
                if (pathResultIndices.length !== indexCount) {
                ...
                requireHitBoundaryEquality(leftEndpoint.sourcePoint, rightEndpoint.sourcePoint, pathIndexContext, `二维 HIT 二阶路径构造子 ${path.name} 的起点边界`);
                requireHitBoundaryEquality(leftEndpoint.targetPoint, rightEndpoint.targetPoint, pathIndexContext, `二维 HIT 二阶路径构造子 ${path.name} 的终点边界`);
                const endpoints = generatedEqualityEndpoints(conclusion);
                if (!endpoints
            ...
            const evaluateTwoPathExpression = (expression, owner, side, indexContext = [], state = { nodes: 0, ancestors: new WeakSet() }, depth = 0) => {
                ...
                try {
                    if (expression.kind === "atom") {
                        ...
                        if (argumentNames.length !== referencedPath.argumentTypes.length
                            || new Set(argumentNames).size !== argumentNames.length) {
                            throw new Error(`三维 HIT 二阶路径 ${expression.name} argumentNames 与 telescope 不一致`);
                        }
                        if (expression.arguments.length !== argumentNames.length) {
                            throw new Error(`${label}参数数量与二阶路径 ${expression.name} telescope 不一致：`
                                + `需要 ${argumentNames.length} 个，实际 ${expression.arguments.length} 个`);
                        }
                        const arguments_ = expression.arguments.map(normalizedMetadataAst);
                        const replacements = new Map();
                    ...
                    if (expression.kind === "compose") {
                        ...
                        requireHitBoundaryEquality(left.targetPath, right.sourcePath, indexContext, `${label}组合项的中间一阶路径边界`);
                        return {
                            kind: "compose",
                            left,
                            right,
            ...
            const twoPathContainsRefl = (expression) => expression.kind === "atom"
            ...
            const evaluatedThreePathEndpoints = new Map();
            for (const path of metadataThreePathEntries) {
                ...
                if (path.computationName !== `apd3_${path.name}`) {
                    throw new Error(`三维 HIT dependent 计算定理不存在：${path.computationName ?? ""}`);
                }
                if (path.actionComputationName !== `ap3_${path.name}`) {
                    throw new Error(`三维 HIT action 计算定理不存在：${path.actionComputationName ?? ""}`);
                }
                const pathType = auxiliaryTypes.get(path.name);
                if (!pathType) {
                ...
                requireHitBoundaryEquality(leftEndpoint.sourcePath, rightEndpoint.sourcePath, pathIndexContext, `三维 HIT 三阶路径构造子 ${path.name} 的起始二阶边界`);
                requireHitBoundaryEquality(leftEndpoint.targetPath, rightEndpoint.targetPath, pathIndexContext, `三维 HIT 三阶路径构造子 ${path.name} 的终止二阶边界`);
                requireHitBoundaryEquality(leftEndpoint.sourceExpression.sourcePoint, rightEndpoint.sourceExpression.sourcePoint, pathIndexContext, `三维 HIT 三阶路径构造子 ${path.name} 的起点边界`);
                requireHitBoundaryEquality(leftEndpoint.targetExpression.targetPoint, rightEndpoint.targetExpression.targetPoint, pathIndexContext, `三维 HIT 三阶路径构造子 ${path.name} 的终点边界`);
                const endpoints = generatedEqualityEndpoints(conclusion);
                if (!endpoints
                    || !sameGeneratedAst(endpoints[0], leftEndpoint.term)
                    || !sameGeneratedAst(endpoints[1], rightEndpoint.term)) {
            ...
            if (metadataTwoPathEntries.length) {
                ...
                const strongLambda = (name, type, body) => ({
                    type: "L",
                    name,
                    nodes: [type, body]
                });
                ...
                const validateDependentTwoPathComputation = (path, full) => {
                    ...
                    const expressionData = (expression) => {
                        ...
                        if (expression.kind === "atom") {
                            ...
                            computation = wrapApply(wrapVar("@hit_apd1_corrected_inv"), Core.clone(hitUniverseLevel), Core.clone(motiveUniverseLevel), Core.clone(hitType), Core.clone(expression.value.sourcePoint), Core.clone(expression.value.targetPoint), Core.clone(expression.value.term), Core.clone(motiveAtFiber), Core.clone(dependentHead), Core.clone(value.method), Core.clone(value.computation));
                        }
                        return {
                ...
                const validateThreePathActionComputation = (path, full) => {
                    const recursorName = full ? metadata.fullRecursorName : metadata.recursorName;
                    const recursorType = full
                        ? (metadata.fullRecursorName
                            ? auxiliaryTypes.get(metadata.fullRecursorName)
                    ...
                    const methodValue = wrapApply(wrapVar(threePathMethodNames[pathIndex]), ...argumentValues.map(argument => Core.clone(argument)));
                    const occupied = new Set([
                        ...prefixBinders.map(binder => binder.name),
                        ...argumentNames
                    ]);
                    ...
                    const recursorExpressionMethod = (expression) => {
                        ...
                        return wrapApply(wrapVar("inveq"), recursorExpressionMethod(expression.value));
                    };
                    const strongExpressionComputation = (expression) => {
                        ...
                        if (expression.kind === "compose") {
                            ...
                            return {
                                ...
                                proof: wrapApply(wrapVar("@hit_ap2_corrected_comp"), Core.clone(hitUniverseLevel), Core.clone(motiveUniverseLevel), Core.clone(hitType), wrapVar(motiveName), Core.clone(pointBoundary.sourcePoint), Core.clone(pointBoundary.targetPoint), Core.clone(expression.left.sourcePath), Core.clone(expression.left.targetPath), Core.clone(expression.right.targetPath), Core.clone(recursorHead), Core.clone(left.sourceMethod), Core.clone(left.targetMethod), Core.clone(right.targetMethod), Core.clone(expression.left.term), Core.clone(expression.right.term), Core.clone(left.sourceComputation), Core.clone(left.targetComputation), Core.clone(right.targetComputation), Core.clone(left.method), Core.clone(right.method), Core.clone(left.proof), Core.clone(right.proof))
                            };
                        }
                        ...
                        return {
                            ...
                            proof: wrapApply(wrapVar("@hit_ap2_corrected_inv"), Core.clone(hitUniverseLevel), Core.clone(motiveUniverseLevel), Core.clone(hitType), wrapVar(motiveName), Core.clone(pointBoundary.sourcePoint), Core.clone(pointBoundary.targetPoint), Core.clone(expression.value.sourcePath), Core.clone(expression.value.targetPath), Core.clone(recursorHead), Core.clone(value.sourceMethod), Core.clone(value.targetMethod), Core.clone(expression.value.term), Core.clone(value.sourceComputation), Core.clone(value.targetComputation), Core.clone(value.method), Core.clone(value.proof))
                        };
                    };
                ...
                const validateThreePathDependentComputation = (path, full) => {
                    const eliminatorName = full
                        ? metadata.fullEliminatorName
                        : metadata.eliminatorName;
                    const eliminatorType = full
                    ...
                    const dependentExpressionMethod = (expression) => {
                        ...
                        if (expression.kind === "compose") {
                            ...
                            return {
                                sourceMethod: left.sourceMethod,
                                targetMethod: right.targetMethod,
                                proof: wrapApply(wrapVar(twoPathContainsRefl(expression)
                                    ? "@hit_dep2_comp"
                                    : "hit_dep2_comp"), ...(twoPathContainsRefl(expression)
                                    ? [
                                    ...
                                    : []), Core.clone(motiveAtFiber), Core.clone(sourceValue), Core.clone(targetValue), Core.clone(left.sourceMethod), Core.clone(left.targetMethod), Core.clone(right.targetMethod), Core.clone(expression.left.term), Core.clone(expression.right.term), Core.clone(left.proof), Core.clone(right.proof))
                            };
                        }
                        ...
                        return {
                            ...
                            proof: wrapApply(wrapVar(twoPathContainsRefl(expression)
                                ? "@hit_dep2_inv"
                                : "hit_dep2_inv"), ...(twoPathContainsRefl(expression)
                                ? [
                                    Core.clone(hitUniverseLevel),
                                    Core.clone(motiveUniverseLevel),
                                    Core.clone(hitType),
                                ...
                                : []), Core.clone(motiveAtFiber), Core.clone(sourceValue), Core.clone(targetValue), Core.clone(value.sourceMethod), Core.clone(value.targetMethod), Core.clone(expression.value.term), Core.clone(value.proof))
                        };
                    };
                    const dependentExpressionComputation = (expression) => {
                        ...
                        if (expression.kind === "compose") {
                            ...
                            const method = wrapApply(wrapVar(explicit ? "@hit_dep2_comp" : "hit_dep2_comp"), ...(explicit
                                ? [
                                    Core.clone(hitUniverseLevel),
                                    Core.clone(motiveUniverseLevel),
                                    Core.clone(hitType),
                                ...
                                : []), Core.clone(motiveAtFiber), Core.clone(sourceValue), Core.clone(targetValue), Core.clone(left.sourceMethod), Core.clone(left.targetMethod), Core.clone(right.targetMethod), Core.clone(expression.left.term), Core.clone(expression.right.term), Core.clone(left.method), Core.clone(right.method));
                            return {
                        ...
                        const explicit = twoPathContainsRefl(expression);
                        const method = wrapApply(wrapVar(explicit ? "@hit_dep2_inv" : "hit_dep2_inv"), ...(explicit
                            ? [
                                Core.clone(hitUniverseLevel),
                                Core.clone(motiveUniverseLevel),
                                Core.clone(hitType),
                            ...
                            : []), Core.clone(motiveAtFiber), Core.clone(sourceValue), Core.clone(targetValue), Core.clone(value.sourceMethod), Core.clone(value.targetMethod), Core.clone(expression.value.term), Core.clone(value.method));
                        return {
                    ...
                    const twoPathDomain = strongEquality(Core.clone(sourcePath), Core.clone(targetPath));
                    const occupied = new Set([
                        ...prefixBinders.map(binder => binder.name),
                        ...argumentNames
                    ]);
                    ...
                    const correctedMethod = strongCompose(strongCompose(transportedComputation, mappedMethod), wrapApply(wrapVar("inveq"), Core.clone(rightComputation)));
                    const action = wrapApply(wrapVar("apd3"), Core.clone(dependentHead), Core.clone(pathTerm));
                    const expected = normalizedMetadataAst(strongWrapPis([...prefixBinders, ...localBinders], strongEquality(action, correctedMethod)));
                    if (!sameGeneratedAstAlpha(actualComputationType, expected)) {
                        throw new Error(`三维 HIT dependent 计算定理 ${computationName} 与 metadata 不一致`);
                    }
                    canonicallyCertifiedTypeFormations.add(computationName);
                };
                for (const path of metadataThreePathEntries) {
            ...
            if (metadata.kind === "hit3") {
                const readTelescope = (source, label) => {
                    const binders = [];
                    let cursor = source;
                    while ((cursor.type === "P" || cursor.type === "->")
                        && cursor.nodes?.[0] && cursor.nodes?.[1]) {
                ...
                const surfaceEquality = (left, right) => ({
                    type: "=",
                    name: "",
                    nodes: [left, right]
                });
                const surfaceCompose = (left, right) => ({
                    type: "*",
                    name: "",
                    nodes: [left, right]
                });
                const surfaceLambda = (name, type, body) => ({
                    type: "L",
                    name,
                    nodes: [type, body]
                });
                    ...
                    for (let index = 0; index < threePathEntries.length; index++) {
                        ...
                        const recursorExpressionMethod = (expression) => {
                            if (expression.kind === "atom") {
                                const endpointIndex = twoPathEntries.findIndex(entry => entry.name === expression.path.name);
                                if (endpointIndex < 0) {
                                    throw new Error(`${label} ${path.name} 引用了未知二阶路径：${expression.path.name}`);
                                }
                                return wrapApply(wrapVar(twoPathMethodNames[endpointIndex]), ...expression.arguments.map(argument => Core.clone(argument)));
                            }
                            if (expression.kind === "refl") {
                        ...
                        if (dependent) {
                            ...
                            const dependentExpressionMethod = (expression) => {
                                if (expression.kind === "atom") {
                                    const endpointIndex = twoPathEntries.findIndex(entry => entry.name === expression.path.name);
                                    if (endpointIndex < 0) {
                                        throw new Error(`${label} ${path.name} 引用了未知二阶路径：`
                                ...
                                if (expression.kind === "compose") {
                                    ...
                                    return {
                                        ...
                                        proof: wrapApply(wrapVar(twoPathContainsRefl(expression)
                                            ...
                                            : []), Core.clone(motiveAtFiber), Core.clone(pointValue), Core.clone(targetPointValue), Core.clone(left.sourceMethod), Core.clone(left.targetMethod), Core.clone(right.targetMethod), Core.clone(expression.left.term), Core.clone(expression.right.term), Core.clone(left.proof), Core.clone(right.proof))
                                    };
                                }
                                const value = dependentExpressionMethod(expression.value);
                                return {
                                    ...
                                    proof: wrapApply(wrapVar(twoPathContainsRefl(expression)
                                        ...
                                        : []), Core.clone(motiveAtFiber), Core.clone(pointValue), Core.clone(targetPointValue), Core.clone(value.sourceMethod), Core.clone(value.targetMethod), Core.clone(expression.value.term), Core.clone(value.proof))
                                };
                            };
                        ...
                        const expected = normalizedMetadataAst(surfaceWrapPis(argumentNames, argumentTypes, expectedBody));
                        const actual = binders[offset + index]?.type;
                        if (!actual || !sameGeneratedAstAlpha(actual, expected)) {
                ...
                validateThreeCoherenceTelescope(normalizedMetadataAst(publicEliminatorType), false, true, "公开消去器");
                validateThreeCoherenceTelescope(fullEliminatorType, true, true, "完整消去器");
                validateThreeCoherenceTelescope(normalizedMetadataAst(publicRecursorType), false, false, "公开递归器");
                validateThreeCoherenceTelescope(fullRecursorType, true, false, "完整递归器");
            }
        ...
        const isCanonicalHitStructureType = (name) => isCanonicalHitBundle
        ...
        const isCanonicalHitBundle = strictRuleSchema
            && (bundle.metadata?.kind === "hit1"
                || bundle.metadata?.kind === "hit2"
                || bundle.metadata?.kind === "hit3");
        const usesCertifiedHitStructureBudget = (name) => isCanonicalHitStructureType(name)
        ...
        try {
            ...
            if (!strictRuleSchema)
                publishComputeRules();
            for (const [name, type] of normalizedEntries) {
                if (deferredComputationTypes.has(name) || isCanonicalHitStructureType(name))
                    continue;
                this.checkTypeFormation(type, []);
            }
            if (strictRuleSchema) {
                this.validateCanonicalInductiveRuleTypes(bundle, normalizedEntries, normalizedRules, parameters, indexCount, isCanonicalHitBundle ? certifiedHitStructureResourceLimits : undefined);
            ...
            for (const [name, type] of normalizedEntries) {
                if (!deferredComputationTypes.has(name) && !isCanonicalHitStructureType(name))
                    continue;
                this.checkTypeFormation(type, [], canonicallyCertifiedTypeFormations.has(name)
     ...
     */
    checkTypeFormation(ast, context = [], semanticResourceLimits) {
        const candidate = Core.clone(ast);
        const inferred = this.checkType(candidate, context, false, undefined, false, true, false, semanticResourceLimits);
    ...
    checkType(ast, context, allowModify, beforeInferResolution, allowUnsolvedTermMetas = false, requireSemantic = false, preservePresentation = true, semanticResourceLimits) {
        ...
        if (semanticAssertionTarget) {
            const semanticAssertion = this.trySemanticTypeAssertion(semanticAssertionTarget, context, !beforeInferResolution, allowUnsolvedTermMetas
                || hasSemanticElaborationHole(semanticAssertionTarget.nodes[0]), semanticAssertionSource, semanticResourceLimits);
    ...
    trySemanticTypeAssertion(ast, context, annotateTerm, allowUnsolvedTermMetas, sourceAssertion, semanticResourceLimits) {
        if (ast.type !== ":")
            return;
        ...
        const checkedOutputMaxNodes = semanticResourceLimits?.outputMaxNodes
            ?? expectedMaxNodes;
        if (!fitsSemanticNbeBudget(checked.type, checkedOutputMaxNodes, false, context, false, returnedSchematicMetaNames)) {
            return exceedsSemanticNbeNodeBudget(checked.type, checkedOutputMaxNodes) ? { status: "unsupported", code: "budget-exhausted" } : undefined;
        }
    ...
    trySemanticTypeSynthesis(ast, context, options = {}) {
        ...
        const certifiedOutputMaxNodes = (() => {
            if (!this.certifiedLargeSystemTypes.size)
                return undefined;
        ...
        const canTryWithoutElaboration = requestedElaboration
            && fitsSemanticNbeBudget(semanticAst, inputMaxNodes, false, context, false);
        ...
        if (!fitsSemanticNbeBudget(semanticType.type, Math.max(inputMaxNodes, outputMaxNodes), false, context, false, generalizedMetaNames)) {
            if (exceedsSemanticNbeNodeBudget(semanticType.type, Math.max(inputMaxNodes, outputMaxNodes))) {
                options.captureFailure?.({
                    status: "unsupported",
                    code: "budget-exhausted"
                });
        ...
        if (!fitsSemanticNbeBudget(compactType, outputMaxNodes, false, context, false, generalizedMetaNames)) {
            if (exceedsSemanticNbeNodeBudget(compactType, outputMaxNodes)) {
                options.captureFailure?.({
                    status: "unsupported",
                    code: "budget-exhausted"
                });
