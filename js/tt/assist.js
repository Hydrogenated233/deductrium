...
function parseAssistInput(source) {
    return parser.parseSurfaceOrLegacy(source);
}
/** Parse only the pattern prefix; obtain owns any following type annotation. */
function parsePairPattern(source) {
    let cursor = 0;
    let nodes = 0;
    const names = [];
    const skipSpace = () => { while (/\s/.test(source[cursor] ?? ""))
        cursor++; };
    const read = (depth) => {
        if (depth > 64 || ++nodes > 255)
            throw TR("有序对模式过大或嵌套过深");
        skipSpace();
...
export class Assist {
    ...
    constructor(h, target) {
        ...
        if (typeof target === "string") {
            target = markExplicitAtSyntax(parseAssistInput(target));
        }
    ...
    dependVarId = 0;
    multipleApplyArity(valueName, goal) {
        if (Assist.disableMultipleApply)
            return 0;
        let fn;
        try {
            fn = core.checkType(wrapVar(valueName), goal.context, false, undefined, false, true);
        }
        catch (e) {
            return 0;
        }
    ...
    collectAutofillTactics() {
        ...
        if (dynamicInductive) {
            ...
            for (const constructor of dynamicInductive.constructors) {
                const source = dynamicInductive.indexArguments.length
                    ? this.dynamicConstructorApplication(constructor, dynamicInductive.uniformArguments, g)
                    : this.dynamicConstructorSource(constructor.name, dynamicInductive.uniformArguments);
                if (!source)
                    continue;
            ...
            if (matchEq) {
                if (matchEq["$1"].name === "0I" && matchEq["$2"].name === "1I")
                    tactics.push("apply segI");
                else if (matchEq["$1"].name === "1I" && matchEq["$2"].name === "0I" && core.hasConst("inveq"))
                    tactics.push("apply inveq segI");
                if (matchEq["$1"].name === "base" && matchEq["$2"].name === "base")
                    tactics.push("exact loop");
                try {
                    if (core.checkType({
                        type: "===",
                        name: "",
        ...
        for (const [val, typ] of g.context) {
            ...
            if (this.isIndType(typ)) {
                ...
                if (this.getDynamicInductiveMetadata(typ))
                    tactics.push("induction " + val);
            }
            try {
        ...
        for (const v of defs) {
            if (vars.has(v) && !g.context.find(e => e[0] === v)) {
                if (ignore.has(v) || v.startsWith("ind_") || types.has("@" + v))
                    continue;
    ...
    getDynamicInductiveMetadata(typ) {
        ...
        if (uniformArguments.length !== metadata.parameters.length)
            return undefined;
        const indexArguments = application.slice(metadata.parameters.length + 1, metadata.parameters.length + metadata.indices.length + 1);
    ...
    dynamicConstructorSource(name, uniformArguments) {
        return [name, ...uniformArguments.map(argument => parser.stringify(argument))].join(" ");
    }
    dynamicConstructorApplication(constructor, uniformArguments, goal) {
        const source = this.dynamicConstructorSource(constructor.name, uniformArguments);
        const candidate = parseAssistInput(source + constructor.argumentTypes.map(() => " _").join(""));
        const assertion = {
            type: ":",
            name: "",
        ...
        try {
            core.withSilentErrors(() => core.checkType(assertion, goal.context, true, undefined, true, true));
            ...
            for (const argument of inferredArguments) {
                if (this.containsInferenceHole(argument))
                    break;
    ...
    reorderContextBindings(goal, names) {
    ...
    rcases(spec) {
        ...
        if (!match) {
            if (/[<>⟨⟩]/.test(value))
                throw TR("rcases目前支持 rcases h with ⟨x, hx⟩");
    ...
    destructurePairPattern(name, parsed) {
        const goal = this.goal[0];
        if (!goal)
            throw TR("无证明目标，请使用qed命令结束证明");
        ...
        const leaves = [];
        const split = (bindingName, pattern) => {
            ...
            if (Assist.disableDestructConds && this.goal[0].context.some(([name, type]) => name !== bindingName && Core.getFreeVars(type).has(bindingName))) {
                throw TR("当前未解锁依赖假设解构");
            }
            const names = pattern.children.map(child => child.kind === "name" && child.name !== "_" ? child.name : fresh());
    ...
    construct() {
        ...
        const dynamicConstructor = dynamicInductive?.constructors
            .map(constructor => ({
            constructor,
            source: this.dynamicConstructorApplication(constructor, dynamicInductive.uniformArguments, goal)
        }))
        ...
        if (goal.type.type === "S")
            return this.ex("");
        if (goal.type.name === "True")
            return this.exact("true");
        if (goal.type.type === "=" || goal.type.nodes?.[0]?.nodes?.[0]?.name === "eq")
            return this.rfl();
    ...
    ].map(e => [parser.parse(e[0]), parser.parse(e[1]), e[2] || []]);
    eq(test) {
        const goal = this.goal.shift();
        if (!goal)
            throw TR("无证明目标，请使用qed命令结束证明");
        this.goal.unshift(goal);
        const recurse = (ast, pattern, resArr = []) => {
            let res;
        ...
        for (let [pattern, eq, evidences] of Assist.eq_matches) {
            ...
            for (const res of resarr) {
                const npattern = Core.clone(pattern);
                Core.replaceByMatch(npattern, res, /^\$/);
                ...
                if (eq.name === "_")
                    continue;
                if (!test) {
                    const neq = Core.clone(eq);
                    Core.replaceByMatch(neq, res, /^\$/);
    ...
    exact(ast) {
        ...
        if (typeof ast === "string") {
            ast = markExplicitAtSyntax(parseAssistInput(ast));
        }
        const goal = this.goal.shift();
        if (!goal)
            throw TR("无证明目标，请使用qed命令结束证明");
    ...
    clearBondIds(ast, seen = new WeakSet()) {
        ...
        for (const node of ast.nodes ?? [])
            this.clearBondIds(node, seen);
        if (ast.checked)
            this.clearBondIds(ast.checked, seen);
    }
    findInputHole(ast) {
        if (!ast)
            return null;
    ...
    elaborateAndAutofill(ast, expected, context) {
    ...
    apply2(ast) {
        ...
        while (true) {
            ...
            try {
                ...
                core.checkType({ nodes: [fnWith_, Core.clone(goal.type)], type: ":", name: "" }, goal.context, false, undefined, true);
                break;
            }
            catch (e) { }
            holeNumbers++;
    ...
    rw(eq, back = false, forcingMatchAST) {
        if (typeof eq === "string") {
        ...
        if (typeof eq === "string")
            eq = markExplicitAtSyntax(parseAssistInput(eq));
        if (!eq)
        ...
        if (!goal)
            throw TR("无证明目标，请使用qed命令结束证明");
        let matched;
        const matchEquality = (type) => Core.match(type, parser.parse("$2 = $3"), /^\$/)
            || Core.match(type, parser.parse("eq $2 $3"), /^\$/)
            || Core.match(type, parser.parse("@eq $0 $1 $2 $3"), /^\$/);
        const containsInputHole = (ast) => ast?.type === "var" && ast.name === "_"
        ...
        if (hadInputHoles) {
            ...
            for (const candidate of candidates) {
                ...
                let completed;
                try {
                    completed = this.elaborateAndAutofill(eq, expectedEquality, goal.context);
                }
                catch {
                    continue;
                }
                if (containsInputHole(completed))
                    continue;
        ...
        let isRfl = false;
        try {
            core.checkType(wrapLambda("===", "", eq, wrapVar("rfl")), goal.context, false, undefined, false, hadInputHoles);
            isRfl = true;
        }
        catch (e) {
        }
        ...
        if (!isRfl) {
            ...
            let newAst;
            const wrapInvOrNot = (ast, wrap) => wrap ? wrapApply(wrapVar("inveq"), ast) : ast;
            if (compeq["$2"] && !Core.getFreeVars(compeq['$2']).has(fnparam) && (!back || core.hasConst("inveq")) && core.hasConst("compeq")) {
                ...
                if (this.hasRewriteParameterInBinderType(newAst, fnparam)) {
                    try {
                        core.withSilentErrors(() => core.checkType(Core.clone(newAst), goal.context, false));
                    }
                    catch (error) {
                        this.goal.unshift(goal);
                        throw error;
    ...
    hasRewriteParameterInBinderType(ast, parameter) {
        if (!ast || typeof ast !== "object")
            return false;
        if (ast.type === "L" || ast.type === "P" || ast.type === "W" || ast.type === "S") {
            if (ast.nodes?.[0] && Core.getFreeVars(ast.nodes[0]).has(parameter))
                return true;
        }
        return (ast.nodes ?? []).some(node => this.hasRewriteParameterInBinderType(node, parameter));
    }
    genReplaceFn(ast, search, varname, excludedNames, freevarsinSearch = Core.getFreeVars(search), scope = []) {
    ...
    search(ast, search) {
        ...
        if (ast.nodes?.length) {
            ...
            for (let i = ast.nodes.length - 1; i >= 0; i--) {
                if (this.search(ast.nodes[i], search))
                    return true;
            }
        }
        return false;
    ...
    hasHeadReduction(ast) {
        ...
        if (ast.type === "apply") {
            ...
            if (head?.type === "L" || core.semanticKernel.canSemanticReduce(ast))
                return true;
        }
    ...
    have(spec) {
        ...
        if (!declaration || proofSource === "")
            throw TR("have语法应为 have h : T 或 have h [: T] := t");
        ...
        if (!goal)
            throw TR("无证明目标，请使用qed命令结束证明");
        if (goal.context.some(entry => entry[0] === name.name))
            throw TR("无法引入重复名称的假设变量");
    ...
    renameInductionBindings(goal, branchRoot, introducedNames, names) {
        if (introducedNames.length !== names.length)
            return;
        if (names.some((name, index) => name !== introducedNames[index] && introducedNames.includes(name))) {
    ...
    destruct(n, inductionNames = []) {
        ...
        if (dynamicInductive?.indexArguments.length) {
            for (let index = dynamicInductive.indexArguments.length - 1; index >= 0; index--) {
                ...
                const indexName = Core.getNewName(dynamicInductive.indices[index]?.name || `i${index}`, excludedSet);
                excludedSet.add(indexName);
                C = wrapLambda("L", indexName, indexType, Core.clone(C, true));
                C.nodes[1] = this.genReplaceFn(C.nodes[1], indexArgument, indexName, excludedSet);
            }
        }
        if (groupParam) {
        ...
        try {
            if (selfLoopEq) {
                try {
                    core.withSilentErrors(() => core.checkType(Core.clone(indFnHead), goal.context, false));
                }
                catch {
    ...
    ex(n) {
        ...
        try {
            ...
            Core.assign(goal.ast, wrapApply(wrapVar("@pair"), wrapVar("_"), wrapVar("_"), Core.clone(dfn.nodes[0]), dfn, val, wrapVar("(?#0)")), true);
            goal.ast.checked = goal.type;
            if (n) {
                core.checkType(goal.ast.nodes[0], goal.context, false);
            }
    ...
    case() {
        ...
        const family = wrapLambda("L", "_", Core.clone(goal.type.nodes[0]), Core.clone(goal.type.nodes[1]));
        Core.assign(goal.ast, wrapApply(wrapVar("@pair"), wrapVar("_"), wrapVar("_"), Core.clone(goal.type.nodes[0]), family, wrapVar("(?#0)"), wrapVar("(?#0)")), true);
    ...
    expand(n) {
        ...
        try {
            ...
            if (expandedDefinition && core.hasDefinitionCache(n) && !expansionHasHoles) {
                ...
                core.checkType(Core.clone(goal.type), goal.context, false, undefined, false, true, false);
    ...
    semanticSimplification(ast, context) {
        const source = Core.clone(ast);
        try {
            core.checkType(source, context, false, undefined, true, true, false);
            const normalized = Core.clone(source);
            core.checkType({ type: "whnf", nodes: [normalized, wrapVar("_")], name: "" }, context, true, undefined, false, true, false);
    ...
    semanticFunctionType(ast, context) {
        const candidate = Core.clone(ast);
        let type;
        try {
            type = core.checkType(candidate, context, false, undefined, true, true);
            if (type.type !== "P" && type.type !== "->") {
                type = Core.clone(type);
                core.checkType({ type: "whnf", nodes: [type, wrapVar("_")], name: "" }, context, true, undefined, false, true);
            }
        }
        catch {
            return null;
