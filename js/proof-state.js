...
function readPreviousState(host, state) {
    const root = host.querySelector(".proof-state-root");
    if (root) {
        state.rootOpen = root.open;
        state.goalsOpen = new Map(Array.from(root.querySelectorAll(".proof-goal"))
            .map(goal => [goal.getAttribute("data-proof-goal-id"), goal.open])
            .filter((entry) => !!entry[0]));
    }
    const settings = host.querySelector(".proof-state-settings");
    if (settings)
        state.settingsOpen = settings.open;
    const expected = host.querySelector(".proof-expected-type");
    if (expected)
        state.expectedOpen = expected.open;
    const messages = host.querySelector(".proof-messages");
    if (messages)
        state.messagesOpen = messages.open;
}
function disconnectErrorObserver(state) {
...
export function clearProofState(host) {
    ...
    if (error)
        hostErrors.set(host, error);
    host.replaceChildren(...(error ? [error] : []));
}
function node(className, text, tag = "div") {
...
function appendNode(parent, content) {
    parent.appendChild(content());
}
function goalCount(goals) {
    return `${goals.length} ${goals.length === 1 ? "goal" : "goals"}`;
}
function settingLabel(popover, setting, labelText, checked, onChange) {
...
function renderExpected(state, expected) {
    if (!expected)
        return null;
...
function renderGoal(parent, goal, index, state) {
    ...
    for (const hypothesis of goal.hypotheses) {
        if (state.settings.hideTypeAssumptions && hypothesis.isType)
            continue;
    ...
    if (state.settings.targetFirst)
        body.append(target, hypotheses);
    else
        body.append(hypotheses, target);
    details.appendChild(body);
    if (index === 0 && state.settings.emphasizeFirst)
        details.classList.add("proof-goal-emphasized");
    parent.appendChild(details);
}
/** Render the shared Lean-style proof state used by both proof assistants. */
export function renderProofState(host, options) {
    ...
    if (error)
        hostErrors.set(host, error);
    state.goalsOpen = new Map(options.goals.map(goal => [goal.id, state.goalsOpen.get(goal.id) ?? true]));
    ...
    const updateSetting = (key, value) => {
        if (hostStates.get(host) !== state || !host.contains(settings))
            return;
        settingsByScope.set(scope, { ...settingsByScope.get(scope), [key]: value });
        renderProofState(host, { ...state.lastOptions, settings: undefined });
    };
    settingLabel(settingsPopover, "targetFirst", "Target first", state.settings.targetFirst, value => updateSetting("targetFirst", value));
    settingLabel(settingsPopover, "hideTypeAssumptions", "Hide type assumptions", state.settings.hideTypeAssumptions, value => updateSetting("hideTypeAssumptions", value));
    settingLabel(settingsPopover, "hideGoalNames", "Hide goal names", state.settings.hideGoalNames, value => updateSetting("hideGoalNames", value));
    settingLabel(settingsPopover, "emphasizeFirst", "Emphasize current goal", state.settings.emphasizeFirst, value => updateSetting("emphasizeFirst", value));
    ...
    if (state.error)
        messagesContent.appendChild(state.error);
...
//# sourceMappingURL=proof-state.js.map