...
class LangMgr {
    ...
    dataEnInCanvas = {
        ...
        "解锁整数前继运算predZ": "Unlock integer predecessor operation predZ",
        "解锁整数相反数运算negZ": "Unlock integer negation operation negZ",
    ...
    dataEn = {
        ...
        "使用exact策略作用于类型": " using the strategy 'exact' to the type",
        ...
        "无法对非 #fn 节点调用 removeFn": "Cannot remove a function from a non-#fn node",
        ...
        "新定理": "New theorem",
        "新文件夹": "New folder",
        "在文件夹底部添加定理": "Add theorem at the bottom of this folder",
        "拖动排序": "Drag to reorder",
        "局部常量": "Local constant",
        "局部常量仅在所在文件夹及子文件夹中可见": "Local constants are visible only in their folder and nested folders",
        "局部常量需放在文件夹中": "Local constants must be placed inside a folder",
        "引用文件夹：": "Reference folder: ",
        "不使用局部常量": "No local constants",
        "文件夹名称：": "Folder name:",
        "停用子定理": "Disable nested theorems",
        "重命名文件夹": "Rename folder",
        "删除文件夹": "Delete folder",
        "删除文件夹后，里面的定理会移动到上一级。确定继续吗？": "Deleting this folder moves its theorems to the parent folder. Continue?",
        "qed命名参数必须是单个常量名": "The qed name must be a single constant name",
        "类型检查未返回定义结果": "Type checking did not return a registered definition",
        ...
        "只能注册具名定义": "Only named definitions can be registered",
        ...
        "类型推断暂不支持该表达式": "Type inference does not yet support this expression",
        "类型推断中仍有未确定的占位符": "Type inference still contains unresolved placeholders",
        "类型推断无法判定类型是否相等": "Type inference could not decide whether the types are equal",
        ...
        "表达式不能有自由变量": "Free variable cannot appear in the expression",
        // saveload.ts
        "[创造模式]": "[Creative Mode]",
        "[生存模式]": "[Survival Mode]",
