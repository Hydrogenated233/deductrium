...
export class Game {
    ...
    constructor() {
        ...
        if (this.creative) {
            ...
            this.sandboxGui = new TTSandboxGui(document.getElementById("panel-4"), (bridge, options) => this.ttGui.setSandboxAxioms(bridge, options), ast => this.ttGui.renderSandboxAst(ast), { systemRuleIds: [...this.ttGui.unlockedTypes] }, true);
        }
        document.getElementById("panel").classList.remove("hide");
        this.hyperGui = new HyperGui();
        document.querySelectorAll("#panel>button").forEach((btn, idx) => {
            btn.onclick = () => {
                const panelId = btn.dataset.panel || `panel-${idx}`;
                if (panelId === "panel-4" && !sandboxEnabledInMode(gamemode))
                    return;
        ...
        this.hyperGui.world.onGetReward = (hash, tile, isLoading) => {
            ...
            switch (tile.name) {
                ...
                case "ttacticeq":
                    ...
                    return;
                case "ttelimeq":
                    this.ttGui.enableAssistDestructEq();
                    return;
                case "ttelimcond":
                    this.ttGui.enableAssistDestructConds();
                    return;
                case "ttapply2":
                    this.ttGui.enableAssistMultipleApply();
                    return;
                case "ttrw":
        ...
        if (this.sandboxGui)
            this.sandboxGui.onStateChange = scheduleAutosave;
        ...
        if (saves)
            ...
            void this.sandboxGui?.initializeFromStandaloneSave();
