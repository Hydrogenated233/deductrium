...
export class InferenceProofAssistant {
     ...
     */
    preserveFormalAssertions = false;
    nextNodeId = 1;
    committed = false;
    ...
    validateDeferredMaterialization() {
        const systemSnapshot = this.captureFormalSystemState();
        const previousActive = this.fs.inferencePages.activeId;
        try {
     ...
     */
    collectExternalPremiseRefs(includePageScope = false) {
        ...
        const visit = (node) => {
                ...
                : [
                    node.kind === "haveApply"
                        ...
                        : node.kind === "obtainExists"
                            ...
                            : node.source,
                    ...(node.kind === "haveApply"
                        ? (node.haveArgumentSources ?? []).filter((source) => !!source)
                        : [])
                ];
            for (const source of sources) {
                ...
                if (!pageOrder.has(source.pageId))
                    pageOrder.set(source.pageId, pageOrder.size);
                result.set(key, {
    ...
    collectExternalPremises(includePageScope = false) {
    ...
    rintro(argument) {
        ...
        if (!patterns.length)
            throw new Error(TR("rintro需要至少一个模式"));
        for (const pattern of patterns) {
            ...
            if (!pair && !branch)
                throw new Error(TR("rintro暂不支持该模式：") + pattern);
            const node = this.requireCurrentNode();
            const temporary = this.nextHypothesisName(node);
            this.intro(temporary);
            if (pair)
                this.cases(`${temporary} with ${pair[1]} ${pair[2]}`);
            else
                this.cases(`${temporary} with ${branch[1]} | ${branch[2]}`);
        }
    }
    ...
    change(argument) {
        const value = argument.trim();
        if (!value)
            throw new Error(TR("change需要一个目标命题"));
        const node = this.requireCurrentNode();
    ...
    introNode(node, name) {
        ...
        if (formalBinder.type !== "replvar")
            throw new Error(TR("全称量词形式化约束变量无效"));
    ...
    exact(argument) {
        ...
        if (source.kind !== "rule") {
            ...
            const formalMatchTable = this.withFormalRuleMatchValues(deduction, match.context, match.matchTable, node.formalReplaceValues);
            node.formalAppliedProposition = this.preserveFormalAssertions
                ? this.stripInertFormalRuleAssertion(this.instantiateRuleAst(deduction.conclusion, match.context, formalMatchTable))
    ...
    cases(argument) {
        ...
        if (!match)
            throw new Error(TR("cases语法应为 cases h [with h1 h2]"));
        const sourceName = match[1];
        const node = this.requireCurrentNode();
        const source = this.resolveSource(sourceName, node);
        if (source.kind === "rule")
            throw new Error(TR("cases来源必须是假设或页面命题"));
        ...
        if (names.length !== 2)
            throw new Error(TR("cases需要两个分支名称"));
        this.obtain(`⟨${names[0]},${names[1]}⟩ := ${sourceName}`);
    }
    ...
    parseCasesNames(value, node) {
        if (!value.trim())
            return this.nextHypothesisNames(node, 2);
    ...
    planRewrite(target, source, destination, nth) {
        ...
        const allowUnknownOutsideMatch = Object.keys(this.fs.assert.getVarNamesAndIsNots(source, {}, null)).length === 0;
        const probe = astmgr.clone(target);
        const allMatches = this.fs.assert.getSubAstMatchTimesAndReplace(probe, astmgr.clone(source), astmgr.clone(destination), -1, [], [], false, allowUnknownOutsideMatch);
        if (allMatches === false)
        ...
        for (const index of indexes) {
            ...
            const matches = this.fs.assert.getSubAstMatchTimesAndReplace(after, astmgr.clone(source), astmgr.clone(destination), index, [], [], false, allowUnknownOutsideMatch);
    ...
    findInverseRewriteOccurrence(before, after, source, destination) {
        ...
        const matches = this.fs.assert.getSubAstMatchTimesAndReplace(probe, astmgr.clone(source), astmgr.clone(destination), -1, [], [], false, Object.keys(this.fs.assert.getVarNamesAndIsNots(source, {}, null)).length === 0);
        if (matches === false)
            throw new Error(TR("rw无法构造反向替换证明"));
        for (let index = 0; index < matches.length; index++) {
            const candidate = astmgr.clone(after);
            const result = this.fs.assert.getSubAstMatchTimesAndReplace(candidate, astmgr.clone(source), astmgr.clone(destination), index, [], [], false, Object.keys(this.fs.assert.getVarNamesAndIsNots(source, {}, null)).length === 0);
    ...
    applyRule(argument) {
        ...
        if (source.kind === "rule") {
            ...
            const formalMatchTable = this.withFormalRuleMatchValues(deduction, application.context, matchTable, formalReplaceValues);
            const instantiate = (value, preserveSurfaceAssertions = false) => {
                ...
                if (this.astContainsPrivateRuleVariable(result)) {
                    ...
                    astmgr.assign(result, this.normalizeAssertionSyntax(result, !(preserveSchematicAssertions || preserveSurfaceAssertions)));
                }
                this.fs.assert.checkGrammer(result, "p");
                return result;
            };
            const instantiateFormal = (value) => {
            ...
            node.appliedProposition = instantiate(deduction.conclusion, this.containsSchematicAssertion(node.target));
            node.formalAppliedProposition = this.preserveFormalAssertions
            ...
            node.children = conditions.map((condition, index) => this.makeNode(condition, this.cloneHypotheses(node.hypotheses), formalConditions[index]));
            return;
        }
        if (parts.length)
        ...
        node.children = instantiatedPremises.map((condition, index) => this.makeNode(condition, this.cloneHypotheses(node.hypotheses), formalPremises[index]));
    }
    ...
    applyAt(sourceText, hypothesisName) {
        ...
        if (!hypothesis?.proposition)
            throw new Error(TR("未找到要应用的假设：") + hypothesisName);
        const parts = this.splitApplicationTerms(sourceText);
        if (!parts.length)
            throw new Error(TR("apply at需要一个证明来源"));
        const sourceName = parts.shift();
        if (sourceName === "_")
            throw new Error(TR("推理层证明助手暂不支持_模糊匹配"));
        const source = this.resolveSource(sourceName, node);
        if (source.kind !== "rule") {
            ...
            const application = this.instantiateHaveApplication(sourceProposition, [...parts, hypothesisName], node);
        ...
        const match = this.matchConclusion(deduction, hypothesis.proposition, explicit, deduction.conditions[0], node, context, [deduction.conditions[0]]);
        this.assertRuleMatchComplete(match, source.name);
    ...
    specialize(argument) {
        ...
        if (parts.length < 2)
            throw new Error(TR("specialize语法应为 specialize h 参数..."));
        const sourceName = parts.shift();
        const hypothesis = node.hypotheses.find(item => item.name === sourceName && !!item.proposition);
        if (!hypothesis?.proposition)
            throw new Error(TR("specialize只能作用于局部假设：") + sourceName);
        if (hypothesis.kind === "variable")
            throw new Error(TR("specialize只能作用于命题假设：") + sourceName);
    ...
    createHaveGoal(node, name, proposition, consumedHypothesis, formalProposition = proposition) {
    ...
    obtain(argument) {
        ...
        for (const [index, [fact, formalFact, canonicalRule]] of facts.entries()) {
            ...
            const current = this.requireCurrentNode();
            this.createHaveGoal(current, index === 0 ? firstName : secondName, astmgr.clone(fact), source.kind === "hypothesis" && index === facts.length - 1 ? source.name : undefined, formalFact);
            this.applyRule(rule.name);
            this.exact(sourceText);
        }
    }
    /** Introduce a witness variable and its proposition from an existential source. */
    obtainExistential(node, variableName, hypothesisName, source, proposition, formalProposition = proposition) {
        if (source.kind === "rule")
            throw new Error(TR("obtain只支持假设或页面命题来源"));
        ...
        if (!formalBinderName)
            throw new Error(TR("obtain来源的形式化存在变量无效"));
     ...
     */
    instantiateHaveApplication(proposition, terms, node) {
        let result = astmgr.clone(proposition);
     ...
     */
    expandGeneratedAssertionsCaptureAvoiding(ast) {
        if (ast.type === "fn" && ast.name === "#rp") {
            ...
            if (!source)
                throw new Error(TR("生成规则替换源不是变量"));
     ...
     */
    selectExplicitGeneratedRule(candidateName, deduction, target, expectedConditions, explicitValues) {
        if (deduction.replaceNames.length !== explicitValues.length)
            return undefined;
        ...
        if (/^[vuc<>]*\.Vcn$/.test(candidateName) && deduction.conditions.length === 1
            ...
            const match = this.matchConclusion(deduction, expectedConditions[0], { positional: explicitValues, named: new Map() }, deduction.conditions[0], undefined, undefined, undefined, expectedConditions);
        ...
        try {
            ...
            for (const candidateName of this.generatedRuleCandidates(baseName, prefixes)) {
                ...
                try {
                    ...
                    const match = this.matchConclusion(deduction, matchingTarget, { positional: explicitValues.map(value => astmgr.clone(value)), named: new Map() }, deduction.conclusion, undefined, undefined, undefined, matchingConditions);
                    this.assertRuleMatchComplete(match, candidateName);
                    const instantiate = (value) => {
                ...
                for (const candidateName of this.generatedRuleCandidates(baseName, prefixes)) {
                    ...
                    try {
                        ...
                        if (!deduction)
                            continue;
                        this.assertGeneratedDeductionMetaRules(candidateName, existingNames);
                        const selection = this.selectExplicitGeneratedRule(candidateName, deduction, target, expectedConditions, explicitValues);
                        if (selection)
     ...
     */
    getSourceFormalProposition(source, node) {
        if (source.kind !== "hypothesis")
            return this.getSourceProposition(source, node);
        const hypothesis = node.hypotheses.find(item => item.name === source.name)
            ?? this.findPendingHaveForNode(node, source.name);
        if (!hypothesis)
            throw new Error(TR("未找到证明来源") + source.name);
    ...
    instantiateFormalHaveApplication(proposition, terms) {
    ...
    matchConclusion(deduction, target, explicit, conclusion = deduction.conclusion, node, context, inferencePatterns, inferenceCandidates) {
        ...
        try {
            ...
            if (!astmgr.equal(target, pattern)) {
                this.fs.assert.match(astmgr.clone(target), pattern, PRIVATE_RULE_METAVARIABLE_PATTERN, false, matchTable, replacedTypes, null, []);
            }
        }
        catch (error) {
     ...
     */
    explicitArgumentsContainSchematicAssertions(explicit, deduction) {
        const values = [...explicit.positional, ...explicit.named.values()];
        if (values.some(value => this.containsSchematicAssertion(value)))
            return true;
        if (!deduction)
            return false;
    ...
    containsSurfacePattern(ast) {
        if (ast.type === "replvar" && ast.name.startsWith("$")
            && !ast.name.startsWith("$$assistant_rule_"))
            return true;
        return !!ast.nodes?.some(child => this.containsSurfacePattern(child));
    }
    containsSchematicAssertion(ast) {
        if (ast.type === "fn" && (ast.name === "#rp" || /^#v*nf$/.test(ast.name))
            && this.containsSurfacePattern(ast))
            return true;
     ...
     */
    deriveFormalReplaceValues(values, normalized, formal) {
        const result = values.map(value => astmgr.clone(value));
        const visit = (normalNode, formalNode) => {
            for (const [index, value] of values.entries()) {
                if (astmgr.equal(normalNode, value))
                    result[index] = astmgr.clone(formalNode);
            }
            if (normalNode.type !== formalNode.type || normalNode.name !== formalNode.name
                || normalNode.nodes?.length !== formalNode.nodes?.length)
                return;
    ...
    withFormalRuleMatchValues(deduction, context, matchTable, formalValues) {
        const result = {};
        for (const [name, value] of Object.entries(matchTable))
            result[name] = astmgr.clone(value);
        deduction.replaceNames.forEach((name, index) => {
            const internal = context.internalByOriginal.get(name);
            const value = formalValues[index];
            if (internal && value)
                result[internal] = astmgr.clone(value);
        });
        return result;
    }
        ...
        for (const original of context.names) {
            ...
            for (const pattern of patterns) {
                ...
                for (const candidate of candidates) {
                    ...
                    try {
                        this.fs.assert.match(astmgr.clone(candidate), astmgr.clone(pattern), PRIVATE_RULE_METAVARIABLE_PATTERN, false, trial, {}, null, []);
     ...
     */
    stripInertFormalReplacement(ast) {
        let result = astmgr.clone(ast);
        for (;;) {
            let params = false;
            try {
                params = this.fs.assert.getRpParams(result);
            }
            catch {
                break;
            }
            ...
            if (source.type !== "replvar" || this.containsRigidFreeName(body, source.name))
                break;
     ...
     */
    stripInertFormalRuleAssertion(ast) {
        const result = astmgr.clone(ast);
        ...
        if (result.type === "fn" && result.name === "#rp"
            && this.containsSchematicAssertion(result))
            return result;
        if (result.type === "sym" && result.name === ">" && result.nodes?.length === 2) {
    ...
    assertSameProposition(a, b) {
        ...
        if (astmgr.equal(strippedLeft, strippedRight))
            return;
    ...
    makeNode(target, hypotheses, formalTarget = target) {
        return {
            id: this.nextNodeId++,
    ...
    materialize(includeExternalPremises = false, allowIntroRules = false) {
        ...
        try {
            ...
            const dischargeHypothesis = (result, hypothesis, hypothesisIndex) => {
                ...
                const transform = (absoluteIndex) => {
                    ...
                    try {
                        ...
                        const selection = this.selectGeneratedRule(row.from.deductionIdx, desired, conditions.map(condition => condition.proposition), ["c", "<", ">"], /^[vuc<>]*\.Vcn$/.test(row.from.deductionIdx) ? row.from.replaceValues : []);
                        if (!selection)
            ...
            const quantifyResult = (result, binding) => {
                ...
                if (renameAfter)
                    this.renameBinder(binder, originalName, binding.name);
                const binderName = this.fs.assert.getVarName(binder);
                if (!binderName)
                    throw new Error(TR("全称量词约束变量无效"));
                const transformed = new Map();
                ...
                const transform = (absoluteIndex) => {
                    ...
                    const desired = this.normalizeAssertionSyntax(quantifiedValue, !this.containsSchematicAssertion(quantifiedValue));
                    if (!row.from) {
                        ...
                        if (this.containsSchematicAssertion(row.value)
                            && this.fs.assert.nf(binderName, this.substituteBound(row.value, binding.name, binderName)) !== 1) {
                ...
                const target = {
                    type: "sym", name: "V",
                    nodes: [astmgr.clone(binding.binder), this.substituteBound(result.proposition, binding.name, originalName)]
                };
             ...
             */
            const emitBinderRenaming = (left, right, binders = []) => {
                const iff = { type: "sym", name: "<>", nodes: [left, right] };
                const desired = binders.reduceRight((body, binder) => ({
                    type: "sym", name: "V", nodes: [binder, body]
                }), iff);
                const derive = (name, conditions, values) => {
                    ...
                    if (!rule)
                        throw new Error(TR("have避免变量捕获需要推理规则：") + ruleName);
                ...
                return derive(".<>r" + (left.name === "~" ? "n" : left.name), left.nodes.map((child, index) => emitBinderRenaming(child, right.nodes[index], binders)), []);
            };
                ...
                for (let index = 0; index < args.length; index++) {
                    ...
                    if (currentProposition.type === "sym" && currentProposition.name === "V"
                        ...
                        if (!astmgr.equal(body, freshBody)) {
                            ...
                            const projection = appendDerived(implication(bridge.proposition, implication(currentProposition, freshSource)), {
                    ...
                    if (currentProposition.type === "sym" && currentProposition.name === ">"
                        ...
                        this.assertSameProposition(propositionAt(argumentRow).value, currentProposition.nodes[0]);
            ...
            const emitBody = (node, incomingRows) => {
                ...
                const finish = (result) => {
                    for (let index = introEntries.length - 1; index >= 0; index--) {
                        ...
                        if (intro.binding.proposition) {
                            ...
                            result = dischargeHypothesis(result, intro.binding.formalProposition ?? intro.binding.proposition, intro.index);
                        }
                ...
                if (node.kind === "exact" && node.source) {
                    ...
                    const idStep = { deductionIdx: ".i", conditionIdxs: [], replaceValues: [proposition] };
...
registerDeferredAssistantMaterializer((fs, deduction) => {
    ...
    const remapPageReferences = (command) => {
        return command.replace(/\bp([0-9]+)\b/g, (whole, rawIndex) => {
            const originalIndex = Number(rawIndex);
            if (localNames.has(whole))
                return whole;
    ...
    try {
        replayHistory = payload.history.map(command => {
            ...
            if (!/^(?:apply|exact|have|obtain|rw|rwb|nth_rw)\b/.test(trimmed))
                return command;
