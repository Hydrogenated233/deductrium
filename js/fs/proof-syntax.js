...
export function migrateInferenceProofCommand(input) {
    ...
    command = command.replace(/^(obtain\s+)<([^<>]*)>\s*(:=\s*[\s\S]*)$/, (_match, prefix, pattern, suffix) => `${prefix}⟨${pattern}⟩ ${suffix}`);
    ...
    if (legacyHave && !/^have\s+[^\s,:=]+\s*(?::|:=)/.test(command)) {
        ...
        if (proposition && name)
            return `have ${name} : ${proposition}`;
    }
    return command;
}
export function migrateInferenceProofHistory(history) {
    return Array.isArray(history)
        ? history.filter((command) => typeof command === "string")
            .map(migrateInferenceProofCommand)
        : [];
}
//# sourceMappingURL=proof-syntax.js.map