...
export class RuleParser {
    ...
    stringify(tree) {
        if (tree.length === 1)
            return tree[0];
        if (tree[0][0] === "#") {
            return tree[0] + "," + tree.slice(1).map(e => this.stringify(e)).join(",");
        }
        if (tree[0] === ":") {
            return tree.slice(1).map((e, i) => (tree.length - 2 === i ? "," : ":") + this.stringify(e)).join("");
        }
...
export class ConstrainSolver {
    dbg(matchTable, constrains = [], assertions = []) {
        console.log("====================");
    ...
    matchEq(a, b, isItem, matchTable, constrains, assertions) {
        ...
        let nfParams;
        if (nfParams = assert.getNfParams(a)) {
            assertions.push([a, isItem]);
            a = a.nodes[0];
        }
        if (nfParams = assert.getNfParams(b)) {
    ...
    solveConstrain(constrains, assertions) {
        ...
        while (constrains.length > 0) {
            ...
            if (astmgr.equal(left, right)) {
                ...
                if (res === false)
                    throw TR("无法解算变量约束，可能存在不一致替换");
            }
        }
        return matchTable;
    }
    getNfAssertionsOfVarsInAST(ast, isItem, scope, res = {}) {
        if (ast.type === "replvar") {
            if (!scope)
                return; // just ignore it, sometimes it can be ok (e.g. #rp($0,$1,$2) match #nf($x,$2))
