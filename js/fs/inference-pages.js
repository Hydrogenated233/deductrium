...
function clonePropositions(propositions) {
    return [...(propositions ?? [])];
}
/**
 ...
 */
export class InferencePageStore {
    pagesById = new Map();
    orderedIds = [];
    nextId = 1;
    activePageId;
    ...
    create(name, init = {}) {
        ...
        if (this.pages.some(page => page.name === name || page.id === name))
            throw new Error(`推理表 ${name} 已存在`);
        const page = {
            id: this.allocateId(),
            name,
    ...
    reorder(idOrName, beforeIdOrName = null) {
        const page = this.requirePage(idOrName);
    ...
    setPropositions(propositions, idOrName = this.activePageId) {
        this.requirePage(idOrName).propositions = clonePropositions(propositions);
    }
    appendProposition(proposition, idOrName = this.activePageId) {
        const page = this.requirePage(idOrName);
        page.propositions.push(proposition);
        return page.propositions.length - 1;
    }
    clearPropositions(idOrName = this.activePageId) {
        this.requirePage(idOrName).propositions = [];
    }
    setCommandSnapshot(snapshot, idOrName = this.activePageId) {
        const page = this.requirePage(idOrName);
    ...
    static deserialize(serialized) {
        if (!serialized || !Array.isArray(serialized.pages) || !serialized.pages.length) {
            return new InferencePageStore();
        }
        return new InferencePageStore(serialized.pages, serialized.activeId);
    }
    addInitialPage(init) {
        this.assertPageName(init.name);
        if (this.pages.some(page => page.name === init.name || page.id === init.name))
            throw new Error(`推理表 ${init.name} 已存在`);
        const id = init.id ?? this.allocateId();
        if (this.pagesById.has(id))
            throw new Error(`推理表编号 ${id} 已存在`);
        if (this.pages.some(page => page.name === id))
            throw new Error(`推理表编号 ${id} 已存在`);
        const page = {
            id,
            name: init.name,
    ...
    requirePage(idOrName) {
        const page = this.page(idOrName);
        if (!page)
            throw new Error(`推理表 ${idOrName} 不存在`);
        return page;
    }
}
//# sourceMappingURL=inference-pages.js.map