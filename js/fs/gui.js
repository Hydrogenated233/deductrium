...
export class FSGui {
    ...
    loadInferencePages(serialized, propositions) {
        if (this.inferenceProofAssistant)
            this.closeInferenceProofAssistant(true);
        if (serialized)
    ...
    getInferenceCompletionContext() {
        ...
        if (this.metarules.includes("cpt"))
            commands.add("tauto");
        return {
    ...
    scheduleInferenceProofTextReplay() {
        if (!this.inferenceProofTextMode || !this.inferenceProofAssistant || this.inferenceProofBusy)
            return;
    ...
    async replayInferenceProofText(explicitRun, toCursor = false) {
        if (!this.inferenceProofTextMode || !this.inferenceProofAssistant || this.inferenceProofBusy)
            return;
        if (this.inferenceProofTextReplayTimer !== null) {
            window.clearTimeout(this.inferenceProofTextReplayTimer);
        ...
        try {
            ...
            for (let index = 0; index < entries.length; index++) {
                if (generation !== this.inferenceProofGeneration || this.formalSystem !== system
                    || this.pageStore.activeId !== pageId)
                    return;
            ...
            if (generation !== this.inferenceProofGeneration)
                return;
            if (snapshot) {
                this.inferenceProofAssistant = assistant;
                this.inferenceProofSnapshot = snapshot;
    ...
    async finishInferenceProof(name) {
        if (!this.inferenceProofAssistant || this.inferenceProofBusy)
            return null;
    ...
    setInferenceProofBusy(busy) {
        ...
        const close = document.getElementById("fs-proof-close");
        if (close)
            close.textContent = busy ? TR("取消") : TR("关闭");
    }
    cancelInferenceProofWork() {
        if (!this.inferenceProofBusy)
            return;
