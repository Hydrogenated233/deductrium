...
export class Polygon {
    ...
    n1: Hvec; n2: Hvec; n3: Hvec; // 3 reflect planes normals
    vertex: Hvec; // vertex in foundamental domain
    edge: Hvec; // edge center in foundamental domain
    R: Rotor; // R^p = e
    I: Rotor; // I^2 = e, 
    invR: Rotor;// (RI)^q = e
    cosetTable: CosetTable;
    rotors: Map<string, Rotor>;
    neighborMatrix: Rotor[] = [];
    // rotors: Rotor[];
    ...
    isInDomain(p: Hvec) {
        for (let i = 0; i < this.p; i++) {
            if (Rotor.rotate(-Math.PI * 2 * i / this.p).apply(p).dot(this.n3) < 0) return i;
        }
        return -1;
    }
    generateRotorsByToddCoxter() {
        const cosetTable = this.cosetTable;
        const rs = new Array<Rotor>(cosetTable.length);
        rs[0] = new Rotor;
        for (const [c, coset] of cosetTable.cosets.entries()) {
            ...
            for (const [x, cx] of coset.entries()) {
                ...
                rs[cx] = g.mul(r); continue;
            }
        }
        // this.rotors = rs;
    }
    generateRotors(t: TileHash = []) {
        const maxNums = 512;
        const frontiers: TileHash[] = [t];
        this.rotors = new Map<string, Rotor>([[t.join(","), new Rotor]]);
        let f: TileHash;
        while ((f = frontiers.pop()) && this.rotors.size < maxNums) {
            for (let i = 0; i < this.p; i++) {
                const [nf, dir] = this.getNeighborAndDir(f, i, true);
                const str = nf.join(",");
                if (this.rotors.has(str)) continue;
                this.rotors.set(str, this.rotors.get(f.join(",")).mul(this.getNeighborMatrix(dir, i)));
                frontiers.unshift(nf);
            }
        }
    }
    getState(t: TileHash, idx: number): 0 | 1 | 2 {
        if (!t.length) return 0;
        if (idx === 0) return 1;
        const tidx = t[idx];
        if (this.q === 3) {
            if (tidx > 3) return 1; //short circuit to avoid recursion
            if (this.getState(t, idx - 1) === 1) return tidx === 2 ? 2 : 1;
            else return tidx === 3 ? 2 : 1;
        }
        if (this.q === 4) {
            if (tidx > 2) return 1; //short circuit to avoid recursion
            if (this.getState(t, idx - 1) === 1) return tidx === 1 ? 2 : 1;
            else return tidx === 2 ? 2 : 1;
        }
    }
    getNeighborAndDir(t: TileHash, n: TileNeighbor, copy: boolean): [TileHash, TileNeighbor] {
        if (n < 0) { n += this.p; }
        if (n >= this.p) { n -= this.p; }
        if (this.q !== 4 && this.q !== 3) throw "not implemented yet";
        if (copy) t = t.slice(0);

        // state 0
        if (!t.length) { t.push(n); return [t, 0]; }
        ...
        if (n === this.p - 1) {
            ...
            if (this.q === 4) { const nd = this.getNeighborAndDir(t, dir + 1, false)[1]; return [t, nd + 1]; }
            if (this.q === 3) { return [t, dir + 1]; }
        }
        if (this.q === 3 && n === this.p - 2) {
            this.getNeighborAndDir(t, this.p - 1, false);
            const state1 = this.getState(t, t.length - 1) === 1;
            t.push(state1 ? 2 : 3); return [t, 1];
        }
        const state1 = this.getState(t, t.length - 1) === 1;
        if (state1) {
            // state 1
            if (n === 1) {
                if (this.q === 4) { t.push(n); return [t, 0]; }
                if (this.q === 3) {
                    const pn = t.pop() - 1;
                    this.getNeighborAndDir(t, pn, false); return [t, this.p - 1];
                }
            }
        } else {
            // state 2
            if (n === 1) {
                ...
                if (this.q === 3) {
                    const pn = t.pop() - 1;
                    const dir = this.getNeighborAndDir(t, pn, false)[1]; return [t, dir - 1];
                }
            }
            if (this.q === 3 && n === 2) {
                this.getNeighborAndDir(t, 1, false); t.push(this.p - 3); return [t, this.p - 1];
            }
        }
        t.push(n); return [t, 0];
    }
    getNeighborMatrix(dir: TileNeighbor, n: TileNeighbor) {
        return this.neighborMatrix[n].mul(Rotor.rotate(-dir * 2 * Math.PI / this.p));
    }
}
export type TileNeighbor = number;
const TileParent: TileNeighbor = 0;
const TileLeft: TileNeighbor = -1;
const TileRight: TileNeighbor = -2;
export type TileHash = TileNeighbor[];
...
export class HoroRect {
    ...
    upperBound: number;
    lowerBound: number;
    leftBoundNormal: Hvec;
    rightBoundNormal: Hvec;
    branchBoundNormals: Hvec[] = [];

    rotors = new Map<string, Rotor>;
    ...
    getIndex(t: TileHash, slice: number = t.length - 1) {
        if (t[slice] > 0) return t[slice];
        return ((slice + 1) % this.p) + 1;
    }
    getNeighbor(t: TileHash, n: TileNeighbor, copy: boolean): TileHash {
        if (copy) t = t.slice(0);
        if (n > 0) {
            if (t[t.length - 1] === 0) {
                if (n === ((t.length - 1) % this.p) + 1) { t.pop(); return t; }
            }
            t.push(n); return t;
        }
        if (n === TileParent) {
            if (t[t.length - 1] > 0) t.pop(); else t.push(n); return t;
        }
        if (n === TileLeft) {
            const idx = this.getIndex(t);
            if (idx > 1) return this.getNeighbor(this.getNeighbor(t, TileParent, copy), idx - 1, false);
            return this.getNeighbor(this.getNeighbor(this.getNeighbor(t, TileParent, copy), TileLeft, false), this.p, false);
        }
        if (n === TileRight) {
            const idx = this.getIndex(t);
            if (idx < this.p) return this.getNeighbor(this.getNeighbor(t, TileParent, copy), idx + 1, false);
            return this.getNeighbor(this.getNeighbor(this.getNeighbor(t, TileParent, copy), TileRight, false), 1, false);
        }
        throw "cannot reach";
    }
    getNeighborMatrix(idx: TileNeighbor, n: TileNeighbor): Rotor {
        if (n === TileLeft) {
    ...
    getAtlasMatrix(src: TileHash, dst: TileHash) {
        // [a2b,b2c,c2d]
        const words: TileNeighbor[] = [];
        // [aidx,bidx,cidx,didx]
        const wordsIndex = [this.getIndex(src)];
        // pushAndReduce: idx is pushed, but words not
        const pushAndReduce = (n: TileNeighbor) => {
            const l = words.length; if (!l) { words.push(n); return; }
            const prev = words[l - 1]; const prevIdx = wordsIndex[l - 1];
            // canceled
            const cancel = (prev > 0 && n === 0) || (prev === 0 && n === prevIdx) || (prev === TileLeft && n === TileRight) || (prev === TileRight && n === TileLeft);
            if (cancel) {
                words.pop(); wordsIndex.pop(); wordsIndex.pop(); return;
            }
            // atomic left/right
            if (prev === 0) {
                if (prevIdx > 0 && prevIdx + 1 === n) {
                    words.pop(); words.push(TileRight);
                    const lastId = wordsIndex.pop();
                    wordsIndex.pop(); wordsIndex.push(lastId); return;
                }
                if (n > 0 && n + 1 === prevIdx) {
                    words.pop(); words.push(TileLeft);
                    const lastId = wordsIndex.pop();
                    wordsIndex.pop(); wordsIndex.push(lastId); return;
                }
            }
            ...
            if (n === 1 && prev2 === 0 && prev === TileRight && wordsIndex[l - 2] === this.p) {
                words.pop(); words.pop(); words.push(TileRight);
                const lastId = wordsIndex.pop();
        ...
        for (let i = src.length - 1; i >= 0; i--) {
            ...
            if (src[i] > 0) { pushAndReduce(0); continue; }// if go to child, inv: go parent
            if (src[i] === 0) {// if go to parent, inv: go to child
                pushAndReduce(this.getIndex(src, i - 1)); continue;
            }
