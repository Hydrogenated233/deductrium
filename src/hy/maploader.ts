...
/** Duplicate names are retained for diagnostics instead of silently overwriting the first tile. */
export const duplicateNames: { name: string, first: string, duplicate: string }[] = [];
let prevTilehash: TileHash = null;
export function initMap(p: Polygon) {
    // Map state includes dynamically generated ordinal tiles. Rebuild from
    // the static map on every reload so importing another save cannot retain
    // blocks or name aliases generated by the previous save.
    blockMap.clear();
    nameMap.clear();
    ...
    mapData.split("\n").map(e => e.trimStart().replaceAll("\n", "")).forEach(e => {
        ...
        if (matched[2]) {
            const tile = t.join(",");
            const first = nameMap.get(matched[2]);
            if (first !== undefined && first !== tile) {
                duplicateNames.push({ name: matched[2], first, duplicate: tile });
            } else if (first === undefined) {
                nameMap.set(matched[2], tile);
            }
        }
        if (!(parent && parent[1] === "%*")) prevTilehash = t;
    });
