...
export function genOrd(nums: number[]) {
    ...
    while (max > 1) {
        ...
        while (badRoot >= 1) {
            ...
            if (foundBrach || r2 === -1) { badRoot--; continue; }
...
function convert2Ord(a: number[]) {
    if (!a.length) return "0";
    let out = ""; a = a.map(e => e - 1); a.unshift(-1);
    let i: number;
    for (i = 1; i < a.length; i++) if (a[i] > a[i - 1]) out += "ω^("; else out += "0)" + ")".repeat(a[i - 1] - a[i]) + "+ω^(";
    return (out + "0)" + ")".repeat(a[i - 1])).replace(/ω\^\(0\)/g, "1").replace(/\^\(1\)/g, "");
}
export function printOrd(a: number[]) {
        ...
        if (ast[0] === "+") {
            ...
            for (let a of ast) {
                ...
                if (item && isFinite(item)) {
                    if (isFinite(a)) { item = Number(item) + Number(a); } else {
                        newTerm.push(item); item = null;
                        continue;
                    }
                }
                if (item && a[0] === "*") {
            ...
            if (item) newTerm.push(item[2] === 1 ? item[1] : item);
            if (newTerm.length === 1) {
                return newTerm[0];
            } else {
                newTerm.unshift("+");
...
const branchTables: (number | number[])[][] = [null, [5], [5, 4], [5, 4, 3], [5, 4, 3, 2], [5, 4, 3, 2, 1]];
export function genOrdTiles(blockMap: Map<string, TileBlock>, nameMap: Map<string, string>, p: Polygon, tile: TileHash, ord: number[], depth: number = 5) {
    depth += ord.length;
    let step = function (t: TileHash, ord: number[]) {
        const hashT = t.join(",");
...
export function calcMaxReachOrd(o: number[], base: number, stepInf: boolean) {
    const len = o.length;
    for (let i = 1; i < len; i++) {
        ...
        for (const n of genOrd(ord2).reverse()) {
            if (n === 1) continue;
            ord2.push(n);
            if (cmp(ord2, o) > 0 && cmp(expandOrd(ord2, base), o) <= 0) return ord2;
            ord2.pop();
        }
    }
    let ord2 = o.slice(0);
    if (stepInf) {
        while (ord2[ord2.length - 1] === 1) ord2.pop();
        if (genOrd(ord2).includes(2)) ord2.push(2); else {
            ord2.push(1, 2)
        };
    } else {
        ord2.push(1);
...
export function lowerOrdinalBase(currentBase: number, rewardBase: number) {
    return Math.min(currentBase, rewardBase);
}

export function cmp(o1: number[], o2: number[]) {
...
export function expandOrd(s: number[], base: number) {
    let len = s.length;
    let c = s[len - 1];
    ...
    if (c !== 1) {
        ...
        //bad part r～len-1
        for (let i = 0; i < base; i++)for (let x = r; x < len - 1; x++) {
            e.push(s[x]);
        }
    }
    return e;
