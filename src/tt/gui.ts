...
export class TTGui {
    ...
    constructor(creative: boolean, skipRendering: boolean) {
        if (typeof Worker !== "undefined") {
            ...
            try { this.assistWorker = new TTAssistWorkerClient(); } catch (error) { console.warn("Proof-assistant worker unavailable", error); }
        }
    ...
    setLastGateTarget(target: string) {
        if (target === this.lastGateTarget) return;
    ...
    private async startAssistFallback(target: string, history: string[], config = this.getWorkerConfig(this.getInhabitatArray().length)) {
    ...
    ast2HTML(idx: string, ast: AST, scopes: AST[] = [], context: Context = [], userLineNumber = 0) {
        ...
        if (ast.type === "var") {
            ...
            else if (this.isKnownTheoremName(astname)) el.classList.add("macro");
            else if (!ast.name.startsWith("U@")) {
    ...
    private isKnownTheoremName(name: string) {
        if (isKnownTheoremIdentifier(name, consts, macro, sysmacro, this.userConstNames)) return true;
    ...
    updateTypeList(terms: Set<string>) {
        ...
        for (const rule of allrules) {
            ...
            if (rule.ast.type === ":" && rule.ast.nodes[0].type === "var") {
                if (this.unlockedTypes.has("// " + vname)) delete this.core.state.sysTypes[vname];
            ...
            if (rule.ast.type === ":=" && rule.ast.nodes[0].type === "var") {
                ...
                if (this.unlockedTypes.has("// " + vname)) delete this.core.state.sysDefs[vname];
                else this.core.state.sysDefs[vname] = this.core.desugar(Core.clone(val), true);
            }
            if (this.unlockedTypes.has("// " + vname)) { delete this.core.state.defTypes[vname]; continue; }
            ...
            if ((rule.inferMode === "@" && this.inferDisplayMode === "_") || (rule.inferMode === "_" && this.inferDisplayMode === "@")) {
                if (rule.ast.type === ":=") {
                    if (rule.ast.nodes[1].type === ":") {
                        ...
                        try { this.core.registConstType(vname, rule.ast.nodes[1]); } catch (e) { console.log("regist const " + vname + ":", e); }
                }
                continue;
            }
            ...
            if (ast.type === ":=") {
                ...
                if (rule.ast.nodes[1].type === ":") {
                    this.core.state.sysTypes[vname] = this.core.desugar(Core.clone(rule.ast.nodes[1].nodes[1]), true);
                } else if (!error) this.core.registConstType(vname, val);
            }
            const infoArr = [];
            for (let i = 0; i < 6; i++) {
        ...
        try {
            for (const item of items) {
                if (item.kind === "folder") {
                    ...
                    if (theorem?.kind === "theorem") theorem.input.value = String(item.value ?? "");
                }
            }
        } finally {
    ...
    getHottDefCtxt(input: HTMLInputElement | number) {
        ...
        if (typeof input === "number") {
            for (let i = 0; i <= input; i++) {
                if (this.isTheoremInputDisabled(inputs[i])) continue;
    ...
    private getWorkerDefinitionSlots(definitionEnd: number): TTDefinitionSlot[] {
    ...
    private getWorkerConfig(definitionEnd: number): TTCoreConfig {
    ...
    private prepareCoreWorker(definitionEnd: number) {
    ...
    private prepareAssistWorker(
        ...
        const promise = this.assistWorkerMutations.enqueue(() => this.assistWorker.configure(
            config,
            definitions ?? this.getWorkerDefinitionSlots(definitionEnd)
        ));
    ...
    private invalidateWorkerDefinitions(startIndex: number) {
        ...
        if (this.coreWorker) {
            ...
            this.coreWorkerMutations.enqueue(() => this.coreWorker.truncate(startIndex)).catch(() => {
                if (this.coreWorker.generation !== generation) return;
                this.coreWorkerGeneration = -1;
                this.coreWorkerConfigKey = "";
                this.coreWorkerConfigurePromise = null;
            });
        ...
        if (this.assistWorker && this.assistWorkerGeneration === this.assistWorker.generation && this.assistWorkerConfigKey) {
            ...
            this.assistWorkerMutations.enqueue(() => this.assistWorker.truncate(startIndex)).catch(() => {
                if (this.assistWorker.generation !== generation) return;
                this.assistWorkerGeneration = -1;
                this.assistWorkerConfigKey = "";
                this.assistWorkerConfigurePromise = null;
            });
    ...
    private syncAssistWorkerDefinition(index: number, definition: TTDefinitionSlot) {
        if (!this.assistWorker || this.assistWorkerGeneration !== this.assistWorker.generation || !this.assistWorkerConfigKey) return;
        this.assistWorkerSessionReady = false;
        const generation = this.assistWorker.generation;
    ...
    updateInhabitList(insertPos?: HTMLDivElement) {
        ...
        input.onblur = ev => {
                ...
                if (ast && !error) {
                    try {
                        if (ast.type === ":=") {
                            ...
                            if (definitionCache) this.core.restoreDefinitionCache(defname, definitionCache);
            ...
            this.prepareCoreWorker(currentIdx).then(() => this.coreWorker.validate(
                currentIdx,
                Core.clone(ast, true),
                [],
                Core.timeout
                ...
                if (result.ok) finish(result.ast, "", result.filledDefinition, result.definitionCache, true);
    ...
    private settlePendingTheorems() {
        const inputs = this.getInhabitatArray();
        if (!inputs.some(input => input.parentElement?.classList.contains("checking"))) return;
        const first = inputs.find(input => !this.isTheoremInputDisabled(input));
        if (first) first.onblur({ forceSync: true, updateDefs: true } as any);
    }
    ...
    async executeTactic(value: string | AST) {
        if (this.tacticBusy) return;
    ...
    getInhabitatArray() {
        return Array.from(document.querySelectorAll<HTMLInputElement>(".inhabitat .tt-theorem-input"));
    }
