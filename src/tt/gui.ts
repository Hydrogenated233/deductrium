...
function sandboxInductiveBundlePrefix(bundle: CoreSystemInductiveBundle): SandboxInductiveEntryPresentation["prefix"] {
...
export function sandboxInductiveEntryPresentation(
    ...
    const paths = flattenHitPathLevels(
    ...
    if (paths.some(path =>
        path.computationName === publicName
...
export class TTGui {
    ...
    private initTacticTargetInput() {
        const target = document.getElementById("tactic-target") as HTMLInputElement | null;
        ...
        target.addEventListener("keydown", event => {
            if (composing || event.isComposing || event.keyCode === 229 || event.key !== "Enter") return;
            event.preventDefault();
            void this.startTacticFromInput();
        });
        begin.addEventListener("click", () => void this.startTacticFromInput());
    }
    private async startTacticFromInput() {
        if (this.tacticBusy) return;
        const input = document.getElementById("tactic-target") as HTMLInputElement | null;
        ...
        try {
            ...
            if (!target) throw new Error(TR("请输入待证命题"));
    ...
    private setTacticBusy(busy: boolean) {
        ...
        const target = document.getElementById("tactic-target") as HTMLInputElement | null;
        const begin = document.getElementById("tactic-target-begin") as HTMLButtonElement | null;
        if (target) target.disabled = busy;
        if (begin) begin.disabled = busy;
    ...
    private beginTacticTargetInput(createPage = false) {
        if (this.tacticBusy) return;
        this.captureActiveTacticSession();
    ...
    private async activateTacticSession(id: string, captureCurrent = true) {
        ...
        this.renderTacticSessionTabs();
        this.onStateChange();
        const targetInput = document.getElementById("tactic-target") as HTMLInputElement | null;
        if (targetInput) targetInput.value = session.target;
    ...
    private async replayTacticText(explicitRun: boolean, toCursor = false) {
        ...
        let entries: ReturnType<TTGui["parseTacticScript"]>;
        try {
        ...
        try {
            ...
            if (terminal && explicitRun) {
                ...
                if (!qed) throw new Error(TR("qed命名参数必须是单个常量名"));
                const qedName = qed?.[1];
                if (qedName) {
            ...
            if (blockError) errorLine = blockError.lineNumber;
            if (this.assistSnapshot) {
                this.renderTacticTextSnapshot(this.assistSnapshot,
                    blockError ? blockError.message : String(error), errorLine);
            } else {
    ...
    private async ensureAssistSessionCurrent() {
        if (!(this.mode instanceof Array)) throw new Error(TR("请先输入待证命题并启动证明"));
    ...
    private renderAssistSnapshot(snapshot: TTAssistSnapshot) {
        ...
        hint.innerHTML = "";
        if (history !== statediv) history.replaceChildren();
        if (this.mode instanceof Array) {
