...
export type TTTacticNode = {
    kind: "tactic" | "sequence" | "focus" | "have" | "exact";
    command: string;
    source: string;
    lineNumber: number;
    body: TTTacticNode[];
...
type Line = { content: string, indent: number, lineNumber: number };

/** Worker/process transports wrap Error strings but preserve the block line. */
export function locateTTTacticError(error: unknown, blockStartLine: number) {
...
export function parseTTTacticScript(source: string): TTTacticNode[] {
    ...
    let cursor = 0;
    let count = 0;
    const sequence = (indent: number, depth: number): TTTacticNode[] => {
        const nodes: TTTacticNode[] = [];
        while (cursor < lines.length && lines[cursor].indent >= indent) {
            ...
            let kind: TTTacticNode["kind"] = "tactic";
