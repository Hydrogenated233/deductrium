...
function inferredPairMotive(ast: AST, state: CheckerState): AST | null {
    if (ast.type !== "apply") return null;
    const { head, args } = flattenApplication(ast);
    if (head?.type !== "var" || head.name !== "pair" || head.bondVarId
        || args.length !== 3 || args[0]?.type !== "L") return null;
    const motive = args[0];
    return motive.nodes?.length === 2 && motive.nodes.every(node =>
        isLocalMeta(node, state)
        && state.inputMetas.has(node.name)
        && !state.inputMetaSurfaceNames.has(node.name)
        && !state.metaSolutions.has(node.name)
    ) ? motive : null;
}
...
function lookupContextType(ast: AST, context: Context): AST | null {
    ...
    // unmarked contexts.
    const binding = context.find(([name, , id]) =>
        !validId(id) && name === ast.name);
    return binding?.[1] ? cloneSyntax(binding[1]) : null;
}
...
function reduceBetaSpine(ast: AST, budget = 32): AST {
    let current = ast;
...
export class SemanticNbeTypeChecker {
    ...
    private checkTermAgainstExpected(
        ...
        if (pairMotive) {
            ...
            const motiveBody = instantiateBinder(
                expectedWhnf.nodes[1],
                expectedWhnf.name,
                expectedWhnf.bondVarId,
                makeVariable(pairMotive.name, pairMotive.bondVarId)
            );
            ...
            const bodyContext = prependContext(
                [pairMotive.name, motiveDomain, pairMotive.bondVarId],
                context
            );
            for (const [meta, value, scope] of [
                ...
                if (!ensureLocalMetaIsType(meta, scope, state)) return unsupported("metavariable");
                const bound = this.bindMeta(meta.name, value, scope, state);
                if (bound === "budget-exhausted") return unsupported("budget-exhausted");
                if (bound === "unsupported") return unsupported("conversion-unsupported");
                if (bound === "unequal") return invalid("argument-type-mismatch");
            }
        ...
        if (isLocalMeta(ast, state)) {
            ...
            if (state.annotateTerm) ast.checked = cloneSyntax(expectedType);
    ...
    private hasElaborationDefinition(ast: AST, targetHeadName?: string): boolean {
        ...
        while (!visited.has(name)) {
            ...
            if (!definitionHeadName) return flattenApplication(definition).head?.type === "L";
