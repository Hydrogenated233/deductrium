...
function reserveBondVarIds(ast: AST, state: CheckerState) {
    ...
    reserveBondVarId(ast.bondVarId, state);
    for (const child of ast.nodes ?? []) reserveBondVarIds(child, state);
}
...
function containsForeignMetavariable(ast: AST, state: CheckerState): boolean {
    ...
    if (ast.type === "var" && (ast.name === "_"
        || (ast.name?.startsWith("?") && !state.metas.has(ast.name)))) return true;
...
function containsLocalMeta(ast: AST, state: CheckerState): boolean {
    if (!ast || typeof ast !== "object") return false;
    if (isLocalMeta(ast, state)) return true;
...
export class SemanticNbeTypeChecker {
    ...
    trySynthesize(
        ...
        return {
            ...
            ...(unresolvedResultMetas.size ? {
                schematicMetaNames: [...unresolvedResultMetas].map(name =>
                    prepared.value.state.inputMetaSurfaceNames.get(name) ?? name)
            } : {}),
    ...
    tryCheck(
        ...
        const publicType = resolvedTerm && !options.preserveKernelType
            ? compactImplicitAliasSyntax(type, this.kernel)
            : type;
        const schematicMetaNames = [...unresolvedResultMetas].map(name =>
            prepared.value.state.inputMetaSurfaceNames.get(name) ?? name);
        return {
            status: "success",
            type: publicType,
    ...
    tryDefinitionalEquality(
        ...
        const publicType = needsTerms && !options.preserveKernelType
            ...
            : type;
        const schematicMetaNames = [...unresolvedResultMetas].map(name =>
            state.inputMetaSurfaceNames.get(name) ?? name);
        return {
            status: "success",
            type: publicType,
            ...(needsTerms ? {
