...
export type CoreHitOnePathExpression =
    ...
    | {
        /** Reflexivity at one declared point-constructor application. */
        kind: "refl";
        pointName: string;
        arguments: AST[];
    }
...
function normalizeCoreSystemInductiveMetadata(
    ...
    const twoPathConstructors = sourceTwoPathConstructors.map(path => {
        ...
        if (indexCount > 0 && resultIndices.length !== indexCount) {
            throw new Error(
                `indexed HIT 二阶路径构造子 ${path.name} 的 resultIndices 数量与索引不一致`
            );
        }
        if (resultIndices.length && resultIndices.length !== indexCount) {
            throw new Error(
                `HIT 二阶路径构造子 ${path.name} 的 resultIndices 数量与索引不一致`
            );
        }
        return {
            name: path.name,
    ...
    const threePathConstructors = sourceThreePathConstructors.map(path => {
        ...
        if (indexCount > 0 && resultIndices.length !== indexCount) {
            throw new Error(
                `indexed HIT 三阶路径构造子 ${path.name} 的 resultIndices 数量与索引不一致`
            );
        }
        if (resultIndices.length && resultIndices.length !== indexCount) {
            throw new Error(
                `HIT 三阶路径构造子 ${path.name} 的 resultIndices 数量与索引不一致`
            );
        }
        return {
            ...path,
...
export class Core {
    ...
    registerSystemInductive(bundle: CoreSystemInductiveBundle) {
        ...
        return kind === "hit3"
            ? this.semanticKernel.withSemanticTermSharing(
                () => this.registerSystemInductiveImpl(bundle)
            )
            : this.registerSystemInductiveImpl(bundle);
    }

    private registerSystemInductiveImpl(bundle: CoreSystemInductiveBundle) {
        ...
        if (bundle.metadata?.kind === "hit1"
            ...
            const evaluateOnePathExpression = (
                ...
                try {
                    if (expression.kind === "atom") {
                        ...
                        const resultIndices = (referencedPath.resultIndices ?? []).map(index =>
                            substituteGeneratedFreeNames(
                                normalizedMetadataAst(index),
                                replacements
                            )
                        );
                        return {
                    ...
                    if (expression.kind === "refl") {
                        const point = metadata.constructors.find(candidate =>
                            candidate.name === expression.pointName
                        );
                        if (!point) {
                        ...
                        if (argumentNames.length !== point.argumentTypes.length
                            || new Set(argumentNames).size !== argumentNames.length) {
                            throw new Error(
                                `二维 HIT 点构造子 ${expression.pointName} argumentNames 与 telescope 不一致`
                            );
                        }
                        if (expression.arguments.length !== argumentNames.length) {
                            throw new Error(
                                `${label}参数数量与点构造子 ${expression.pointName} telescope 不一致：`
                                + `需要 ${argumentNames.length} 个，实际 ${expression.arguments.length} 个`
                            );
                        }
                        const arguments_ = expression.arguments.map(normalizedMetadataAst);
                        const pointTerm = wrapApply(
                            wrapVar(expression.pointName),
                            ...parameters.map(parameter => wrapVar(parameter.name)),
                            ...arguments_.map(argument => Core.clone(argument))
                        );
                        return {
            ...
            const onePathRecursorMethodValue = (
                ...
                if (expression.kind === "refl") {
                    return wrapApply(
                        wrapVar("refl"),
                        hitBranchValue(expression.sourcePoint, branchNames, label)
                    );
                }
                if (expression.kind === "compose") {
                    return {
                        ...
                        nodes: [
                            onePathRecursorMethodValue(
                                expression.left, branchNames, methodNames, label
                            ),
                            onePathRecursorMethodValue(
                                expression.right, branchNames, methodNames, label
                            )
                        ]
                    };
                }
                return wrapApply(
                    wrapVar("inveq"),
                    onePathRecursorMethodValue(
                        expression.value, branchNames, methodNames, label
                    )
                );
            };
            type DependentOnePathMethodContext = {
                hitUniverseLevel: AST;
                motiveUniverseLevel: AST;
                hitType: AST;
            };
            const onePathContainsRefl = (
                expression: EvaluatedOnePathExpression
            ): boolean => expression.kind === "refl"
            ...
            const dependentOnePathMethodContext = (
                motiveUniverseLevel: AST,
                resultIndices: readonly AST[],
                label: string
            ): DependentOnePathMethodContext => {
            ...
            const onePathDependentMethodValue = (
                ...
                methodNames: readonly string[],
                label: string,
                explicitContext?: DependentOnePathMethodContext
            ): AST => {
                if (expression.kind === "atom") {
                    const index = metadataPathEntries.findIndex(entry =>
                        entry.name === expression.path.name
                ...
                if (expression.kind === "compose") {
                    const leftMethod = onePathDependentMethodValue(
                        expression.left, motive, branchNames, methodNames, label, explicitContext
                    );
                    const rightMethod = onePathDependentMethodValue(
                        expression.right, motive, branchNames, methodNames, label, explicitContext
                    );
                    if (explicitContext && onePathContainsRefl(expression)) {
                        return wrapApply(
                            wrapVar("@hit_dep1_comp"),
                            Core.clone(explicitContext.hitUniverseLevel),
                            Core.clone(explicitContext.motiveUniverseLevel),
                            Core.clone(explicitContext.hitType),
                ...
                const valueMethod = onePathDependentMethodValue(
                    expression.value, motive, branchNames, methodNames, label, explicitContext
                );
                if (explicitContext && onePathContainsRefl(expression)) {
                    return wrapApply(
            ...
            for (const path of metadataTwoPathEntries) {
                ...
                if (pathResultIndices.length !== indexCount) {
                    throw new Error(
                        `二维 HIT 二阶路径构造子 ${path.name} resultIndices 与索引数量不一致`
                    );
                }
                if (indexCount > 0) {
                    ...
                    for (let index = 0; index < indexCount; index++) {
                        requireHitIndexEquality(
                            ...
                            rightIndices[index],
                            pathIndexContext,
                            `二维 HIT 二阶路径构造子 ${path.name} 第 ${index + 1} 个端点索引`
                        );
                        requireHitIndexEquality(
                            leftIndices[index],
                            normalizedMetadataAst(pathResultIndices[index]),
                            pathIndexContext,
                ...
                try {
                    ...
                    if (expression.kind === "refl") {
                        ...
                        return {
                            ...
                            resultIndices: (referencedPath.resultIndices ?? []).map(index =>
                                substituteGeneratedFreeNames(
                                    normalizedMetadataAst(index),
                                    replacements
                                )
                    ...
                    if (expression.kind === "compose") {
                        ...
                        if (indexCount > 0) {
                            ...
                            for (let index = 0; index < indexCount; index++) {
                                requireHitIndexEquality(
                                    left.resultIndices[index],
                                    right.resultIndices[index],
                                    indexContext,
                                    `${label}组合项第 ${index + 1} 个索引`
                                );
                            }
            ...
            for (const path of metadataThreePathEntries) {
                ...
                if (pathResultIndices.length !== indexCount) {
                    throw new Error(
                        `三维 HIT 三阶路径构造子 ${path.name} resultIndices 与索引数量不一致`
                    );
                }
                if (indexCount > 0) {
                    if (leftEndpoint.resultIndices.length !== indexCount
                        ...
                        throw new Error(
                            `索引 HIT 三阶路径构造子 ${path.name} 的端点索引纤维不完整`
                        );
                    }
                    for (let index = 0; index < indexCount; index++) {
                        requireHitIndexEquality(
                            leftEndpoint.resultIndices[index],
                            rightEndpoint.resultIndices[index],
                            pathIndexContext,
                            `三维 HIT 三阶路径构造子 ${path.name} 第 ${index + 1} 个端点索引`
                        );
                        requireHitIndexEquality(
                            leftEndpoint.resultIndices[index],
                            normalizedMetadataAst(pathResultIndices[index]),
            ...
            if (metadataTwoPathEntries.length) {
                ...
                const validateTwoPathCoherenceTelescope = (
                    ...
                    for (let index = 0; index < metadataTwoPathEntries.length; index++) {
                        ...
                        const methodContext = dependent
                            ? dependentOnePathMethodContext(
                                full ? wrapVar(binders[0].name) : wrapVar("@0"),
                                path.resultIndices ?? [],
                                `${label} ${path.name}`
                            )
                            : undefined;
                ...
                const validateDependentTwoPathComputation = (
                    ...
                    const motiveAtFiber = twoPathFiberMotive(
                        motiveName,
                        path.resultIndices ?? []
                    );
                    const methodContext: DependentOnePathMethodContext = {
                        hitUniverseLevel,
                        motiveUniverseLevel,
                        hitType
                    };
                    ...
                    const endpointComputation = (endpoint: EvaluatedOnePathExpression): AST => {
                        ...
                        if (endpoint.kind === "refl") {
                            return wrapApply(
                                wrapVar("refl"),
                                onePathRecursorMethodValue(
                                    endpoint, branchNames, pathMethodNames, computationName
                                )
                            );
                        }
                        if (endpoint.kind === "compose") {
                            ...
                            return wrapApply(
                                ...
                                onePathRecursorMethodValue(
                                    endpoint.left, branchNames, pathMethodNames, computationName
                                ),
                                onePathRecursorMethodValue(
                                    endpoint.right, branchNames, pathMethodNames, computationName
                                ),
                                left,
                                right
                            );
                        ...
                        return wrapApply(
                            ...
                            Core.clone(recursorHead),
                            onePathRecursorMethodValue(
                                endpoint.value, branchNames, pathMethodNames, computationName
                            ),
                            value
                        );
                    };
                ...
                const validateThreePathActionComputation = (
                    ...
                    const sourceMethod = onePathRecursorMethodValue(
                        sourcePathExpression, branchNames, pathMethodNames, computationName
                    );
                    const targetMethod = onePathRecursorMethodValue(
                        targetPathExpression, branchNames, pathMethodNames, computationName
                    );
                    const argumentNames = path.argumentNames ?? [];
                    const argumentValues = argumentNames.map(wrapVar);
                    ...
                    const strongExpressionComputation = (
                        ...
                        if (expression.kind === "atom") {
                            const sourceMethod = onePathRecursorMethodValue(
                                expression.sourceExpression, branchNames, pathMethodNames, computationName
                            );
                            const targetMethod = onePathRecursorMethodValue(
                                expression.targetExpression, branchNames, pathMethodNames, computationName
                            );
                            return {
                                sourceMethod,
                                targetMethod,
            ...
            if (metadata.kind === "hit3") {
                ...
                const validateThreeCoherenceTelescope = (
                    ...
                    for (let index = 0; index < threePathEntries.length; index++) {
                        ...
                        const motiveAtFiber = threePathFiberMotive(
                            motiveName,
                            path.resultIndices ?? []
                        );
                        const methodContext = dependent
    ...
    expandDef(ast: AST, context: Context, n: string | Set<string>, count = [0, 1]): boolean {
        ...
        if (ast.type === "~=" && (n === "eqv" || (typeof n === "object" && n.has("eqv")))) {
            ...
            count[1]++;
        } else if (ast.type === "var" && !ast.bondVarId && (
