...
type SemanticPatternBinding = {
    name: string;
    sourceId?: number;
    id: number;
};

function isPositiveBondVarId(id: number) {
...
function prepareSemanticTypePattern(
    ...
    let nextId = firstId;
    const contextScope: SemanticPatternBinding[] = [];
    ...
    const visit = (node: AST, scope: readonly SemanticPatternBinding[]) => {
...
type ContextIndex = {
    /** Every position for a name, ordered from innermost to outermost. */
    byName: Map<string, number[]>;
    /** Every position for a binder id, ordered from innermost to outermost. */
    byBondVarId: Map<number, number[]>;
    /** Entry references let us detect structural array mutations cheaply. */
...
function getContextIndex(context: Context): ContextIndex {
...
export function findContextIndexByBondVarId(
    ...
    for (const [candidate, positions] of index.byBondVarId) {
        if (candidate !== id && equivalent(candidate, id)) return positions[0];
    }
    return -1;
}

/** Return duplicate-name bindings before a given position, nearest first. */
export function findContextEntriesBeforeByName(context: Context, name: string, end: number): Context[number][] {
...
export class Core {
    ...
    private prepareSemanticProofAssistantNormalizeAst(ast: AST, context: Context) {
        ...
        const contextScope: SemanticPatternBinding[] = context
        ...
        const visit = (node: AST, scope: readonly SemanticPatternBinding[]) => {
            ...
            if (node.type === "var") {
                if (!isPositiveBondVarId(node.bondVarId)
                    || scope.some(binding => binding.id === node.bondVarId)) return;
