...
function isSurfaceDelimiter(char: string | undefined): boolean {
    return !char || /\s/u.test(char) || SURFACE_DELIMITERS.has(char);
}

function previousSurfaceChar(source: string, index: number): string | undefined {
    let cursor = index - 1;
    while (cursor >= 0 && /\s/u.test(source[cursor])) cursor--;
    return cursor >= 0 ? source[cursor] : undefined;
}

function skipSurfaceWhitespace(source: string, index: number): number {
    let cursor = index;
 ...
 */
function isSurfaceUniverseToken(token: string): boolean {
 ...
 */
function isSurfaceWBinder(source: string, start: number, end: number): boolean {
    if (source.slice(start, end) !== "W") return false;
    ...
    if (previous === "λ" || previous === "Π" || previous === "Σ") return false;

    let cursor = skipSurfaceWhitespace(source, end);
    const nameStart = cursor;
    while (cursor < source.length && !isSurfaceDelimiter(source[cursor])) cursor++;
    if (cursor === nameStart) return false;
    cursor = skipSurfaceWhitespace(source, cursor);
    return source[cursor] === ":";
}

function shieldSurfaceIdentifiers(source: string): {
    source: string;
    ...
    for (let i = 0; i < source.length;) {
        ...
        const start = i;
        while (i < source.length && !isSurfaceDelimiter(source[i])) i++;
...
function restoreSurfaceIdentifiers(ast: AST | undefined, placeholders: Map<string, string>, seen = new Set<AST>()): void {
    if (!ast || seen.has(ast)) return;
    seen.add(ast);
    const replacement = placeholders.get(ast.name);
    if (replacement !== undefined) ast.name = replacement;
...
export class ASTParser {
     ...
     */
    parseSurface(source: string): AST {
     ...
     */
    parseSurfaceOrLegacy(source: string): AST {
    ...
    private typeTerm3(): AST {
        ...
        if (this.acceptSym("[[")) {
            ...
            if (this.tokens[this.cursor - 1] !== "]]") {
                if (!this.acceptSym("]")) throw TR("语法错误：未找到符号“]]”");
                if (this.tokens[this.cursor - 1] === "]]") this.token = this.tokens[this.cursor - 1] = "]";
                else if (!this.acceptSym("]")) throw TR("语法错误：未找到符号“]]”");
            } else this.nextSym();
        } else if (this.acceptSym("[")) {
            val = { type: "[]", nodes: [this.type()], name: "" };
            if (this.tokens[this.cursor - 1] === "]") this.nextSym();
            else if (this.tokens[this.cursor - 1] === "]]") this.token = this.tokens[this.cursor - 1] = "]";
        } else if (this.acceptSym("(")) {
            val = this.type();
            ...
            this.expectSym(":");
            const paramType = this.type();
            if (!(this.acceptSym(".") || this.acceptSym(","))) throw TR("W未匹配“,”号");
            const fnbody = this.type();
