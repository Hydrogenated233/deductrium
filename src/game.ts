...
import { renderSymbolAliases } from "./symbol-alias-help.js";
...
export class Game {
    ...
    constructor() {
        ...
        langMgr.init();
        renderSymbolAliases(document.getElementById("type-symbol-aliases"));
        ...
        this.hyperGui.world.onPassGate = (name: string, tile: TileBlock, hash: string) => {
            const gateTest = () => {
                ...
                if (tile.text.endsWith("#p")) {
                    ...
                    // A #p gate may use any completed, hypothesis-free inference
                    // page.  The active page (or an `entr` temporary view) must
                    // not hide proofs that live on another page.
                    return this.fsGui.hasPropositionForGate(ast);
                }
                ...
                if ((reg = tile.text.match(/^通过此门需消耗推理素(.+)$/)) && reg[1]) {
                    ...
                    if (this.deductriums < needed * 2) {
                        this.hyperGui.blur();
                        if (!confirm(
                            TR("进入此门将消耗") + reg[1] + TR("推理素，您只有") + stringifyDeductriumAmout(this.deductriums) + TR("推理素，确认要继续吗？") + (!this.rewards.includes("delgate") ? TR("注意，退出后重新进入门会重复扣钱！") : "")
                        )) return false;
                        setTimeout(() => {
        ...
        this.hyperGui.world.onGetReward = (hash: string, tile: TileBlock, isLoading?: boolean) => {
            ...
            switch (tile.name) {
                ...
                case "kit+*":
                    ...
                    ].forEach(s => this.fsGui.addToDeductions(s)); return;
                case ".1st":
                    [
                ...
                case "dSd0": this.fsGui.addToDeductions("d0"); this.fsGui.addToDeductions("dS"); this.fsGui.addToDeductions("dN"); return;
                ...
                case "mcpt": this.unlockMetarule("cdt"); this.unlockMetarule("idt"); return this.unlockMetarule("cpt");
                ...
                case "ttsimplEq": this.ttGui.disableSimpleEq = false; this.ttGui.unlock("eq.="); return this.ttGui.updateAfterUnlock();
                case "ttnotFn": return this.ttGui.unlock("False.not", true);
                case "ttEq": this.ttGui.unlock("eq"); this.ttGui.updateAfterUnlock(); return;
                case "ttactic1":
                ...
                case "ttindTrue": this.ttGui.unlock("True.ind"); this.ttGui.updateAfterUnlock(); return;
                case "ttindFalse": this.ttGui.unlock("False.ind"); this.ttGui.updateAfterUnlock(); return;
                case "ttindBool": this.ttGui.unlock("Bool.ind"); this.ttGui.updateAfterUnlock(); return;
                case "ttindnat": this.ttGui.unlock("nat.ind"); this.ttGui.updateAfterUnlock(); return;
                case "ttProd": this.ttGui.unlock("Prod"); this.ttGui.updateAfterUnlock(); return;
                case "ttSum": this.ttGui.unlock("Sum"); this.ttGui.updateAfterUnlock(); return;
                case "ttindProd": this.ttGui.unlock("Prod.ind"); this.ttGui.updateAfterUnlock(); return;
                case "ttindSum": this.ttGui.unlock("Sum.ind"); this.ttGui.updateAfterUnlock(); return;
                case "ttindeq": this.ttGui.unlock("eq.ind"); this.ttGui.updateAfterUnlock(); return;
                case "ttpr": this.ttGui.unlock("Prod.pr0"); this.ttGui.unlock("Prod.pr1"); this.ttGui.unlock("Prod.prd1"); this.ttGui.updateAfterUnlock(); return;
                case "ttinveq": this.ttGui.unlock("eq.inv"); this.ttGui.updateAfterUnlock(); return;
                case "ttcompeq": this.ttGui.unlock("eq.*"); this.ttGui.updateAfterUnlock(); return;
                case "ttransC": this.ttGui.unlock("eq.transconst"); this.ttGui.updateAfterUnlock(); return;
                case "ttpred": this.ttGui.unlock("nat.pred", true); return;
                case "ttdbl": this.ttGui.unlock("nat.double", true); return;
                case "ttadd": this.ttGui.unlock("nat.add", true); return;
                ...
                case "tteqprop1": this.ttGui.unlock("eq.rightrfl"); this.ttGui.unlock("eq.invinv"); this.ttGui.unlock("eq.rightinv"); this.ttGui.unlock("eq.leftinv"); this.ttGui.updateAfterUnlock(); return;
                case "ttransleft": this.ttGui.unlock("eq.transleft"); this.ttGui.updateAfterUnlock(); return;
                case "ttransleftright": this.ttGui.unlock("eq.rightrfl"); this.ttGui.unlock("eq.transleftright"); this.ttGui.updateAfterUnlock(); return;
                case "ttransright": this.ttGui.unlock("eq.rightrfl"); this.ttGui.unlock("eq.transright"); this.ttGui.updateAfterUnlock(); return;
                case "ttS1": this.ttGui.unlock("S1"); this.ttGui.updateAfterUnlock(); return;
                case "ttindS1": this.ttGui.unlock("S1.ind"); this.ttGui.updateAfterUnlock(); return;
                case "ttrecS1": this.ttGui.unlock("S1.rec"); this.ttGui.updateAfterUnlock(); return;
                case "ttaploop": this.ttGui.unlock("S1.ap_loop"); this.ttGui.updateAfterUnlock(); return;
                case "tteqv": this.ttGui.unlock("eqv"); this.ttGui.updateAfterUnlock(); return;
                case "ttua": this.ttGui.unlock("eqv.id2eqv"); this.ttGui.unlock("eqv.refl"); this.ttGui.unlock("ua"); this.ttGui.updateAfterUnlock(); return;
                case "ttfnext": this.ttGui.unlock("fnext"); this.ttGui.unlock("eq.happly"); this.ttGui.updateAfterUnlock(); return;
                case "ttLiftU": this.ttGui.unlock("LiftU"); this.ttGui.updateAfterUnlock(); return;
                case "ttpseudoeqv": this.ttGui.unlock("naiveqv"); this.ttGui.updateAfterUnlock(); return;
                case "ttI": this.ttGui.unlock("I"); this.ttGui.updateAfterUnlock(); return;
                case "ttZ": this.ttGui.unlock("Z"); this.ttGui.updateAfterUnlock(); return;
                case "ttW": this.ttGui.unlock("W"); this.ttGui.updateAfterUnlock(); return;
                case "ttisProp": this.ttGui.unlock("Prop"); this.ttGui.updateAfterUnlock(); return;
                case "ttisSet": this.ttGui.unlock("Set"); this.ttGui.updateAfterUnlock(); return;
                case "ttisContr": this.ttGui.unlock("Contr"); this.ttGui.updateAfterUnlock(); return;
                case "ttTrunc": this.ttGui.unlock("Trunc"); this.ttGui.updateAfterUnlock(); return;
                case "ttnat2Z": this.ttGui.unlock("Z.nat2"); this.ttGui.updateAfterUnlock(); return;
                case "ttnegZ": this.ttGui.unlock("Z.neg"); this.ttGui.updateAfterUnlock(); return;
                case "ttsuccZ": this.ttGui.unlock("Z.succ"); this.ttGui.updateAfterUnlock(); return;
                case "ttpredZ": this.ttGui.unlock("Z.pred"); this.ttGui.updateAfterUnlock(); return;
                case "pred_succZ": this.ttGui.unlock("Z.predsucc"); this.ttGui.updateAfterUnlock(); return;
                case "succ_predZ": this.ttGui.unlock("Z.succpred"); this.ttGui.updateAfterUnlock(); return;
                case "ttaddZ": this.ttGui.unlock("Z.add"); this.ttGui.updateAfterUnlock(); return;
                case "ttmulZ": this.ttGui.unlock("Z.mul"); this.ttGui.updateAfterUnlock(); return;
                case "ttloop^": this.ttGui.unlock("S1.loop_pow"); this.ttGui.updateAfterUnlock(); return;
                case "ttSus": this.ttGui.unlock("Sus"); this.ttGui.updateAfterUnlock(); return;
                case "ttS2": this.ttGui.unlock("S2"); this.ttGui.updateAfterUnlock(); return;
                case "ttS3": this.ttGui.unlock("S3"); this.ttGui.updateAfterUnlock(); return;
                case "ttindS2": this.ttGui.unlock("S2.ind"); this.ttGui.updateAfterUnlock(); return;
                case "ttEven": this.ttGui.unlock("Even"); this.ttGui.updateAfterUnlock(); return;
                case "ttList": this.ttGui.unlock("List"); this.ttGui.updateAfterUnlock(); return;
                case "ttAleph": this.ttGui.unlock("Aleph"); this.ttGui.updateAfterUnlock(); return;
                case "ttOption": this.ttGui.unlock("Option"); this.ttGui.updateAfterUnlock(); return;
                case "ttrans2": this.ttGui.unlock("eq.trans2"); this.ttGui.updateAfterUnlock(); return;
                case "ttapd2": this.ttGui.unlock("eq.apd2"); this.ttGui.updateAfterUnlock(); return;
                case "ttpo": this.ttGui.unlock("Pushout"); this.ttGui.updateAfterUnlock(); return;
                case "ttJoin": this.ttGui.unlock("Pushout.Join"); this.ttGui.updateAfterUnlock(); return;
                case "ttWedge": this.ttGui.unlock("Pushout.Wedge"); this.ttGui.updateAfterUnlock(); return;
                case "ttNW": this.ttGui.unlock("W.nat"); this.ttGui.updateAfterUnlock(); return;
