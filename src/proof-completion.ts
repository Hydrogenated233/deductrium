export type ProofCompletionContext = {
    commands: readonly string[];
    locals: readonly string[];
    constants: readonly string[];
    aliases: readonly { alias: string; symbol: string }[];
};
export type ProofCompletion = { label: string; insert: string; kind: string };
export type ProofCompletionResult = { start: number; end: number; items: ProofCompletion[] };

/** Syntax-only lookup: callers supply names from their current proof scope. */
export function completeProofInput(source: string, caret: number,
    context: ProofCompletionContext, explicit = false): ProofCompletionResult {
    const empty = { start: caret, end: caret, items: [] };
    const before = source.slice(0, caret);
    const line = before.slice(before.lastIndexOf("\n") + 1);
    if (line.includes("--") || /["`]/.test(line)) return empty;
    const alias = /\\([A-Za-z][A-Za-z0-9]*|\*)?$/.exec(line);
    if (alias) {
        const start = caret - alias[0].length;
        if (start > 0 && /[\p{L}\p{N}_\\]/u.test(source[start - 1])) return empty;
    ...
    if (!token && !explicit && (!line.trim() || !/[\s([]$/.test(line))) return empty;
    ...
    if (!commandPosition && ["intro", "intros", "rintro", "qed"].includes(command)) return empty;
    if (!commandPosition && /^(have|obtain)\b/.test(preceding) && !/[:=]/.test(preceding)) return empty;
    if (preceding.lastIndexOf(" with") > preceding.lastIndexOf(" generalizing")) return empty;
    const localOnly = /\b(at|generalizing)\b/.test(preceding) || ["clear", "revert"].includes(command);
    const values: [readonly string[], string][] = commandPosition
        ? [[context.commands, "策略"]]
        : [[context.locals, "局部"], [localOnly ? [] : context.constants, "定理"]];
    const seen = new Set<string>();
    const items: ProofCompletion[] = [];
    if (["cases", "destruct", "rcases", "induction"].includes(command)
        ...
        for (const keyword of ["generalizing", "with"]) {
            if (keyword.startsWith(prefix)) items.push({ label: keyword, insert: keyword, kind: "选项" });
        }
    }
...
export function installProofCompletion(
    input: HTMLInputElement | HTMLTextAreaElement | null,
    getContext: () => ProofCompletionContext,
    multiline = false
): void {
    if (!input) return;
    ...
    let result: ProofCompletionResult = { start: 0, end: 0, items: [] };
    let selected = 0;
    let composing = false;
    const close = () => {
        popup.hidden = true;
    ...
    const accept = () => {
        ...
        if (composing || input.disabled || input.selectionStart !== input.selectionEnd) return close();
    ...
    const refresh = (explicit = false) => {
        if (composing || input.disabled || document.activeElement !== input
            || input.selectionStart !== input.selectionEnd) return close();
    ...
    input.addEventListener("keyup", event => {
        if (["ArrowLeft", "ArrowRight", "Home", "End"].includes((event as KeyboardEvent).key)) refresh();
    });
    ...
    input.addEventListener("keydown", event => {
        ...
        if (composing || key.isComposing || key.keyCode === 229) return;
        const consume = () => { key.preventDefault(); key.stopImmediatePropagation(); };
        if (key.key === "Escape" && !popup.hidden) { consume(); close(); return; }
        if (key.ctrlKey || key.metaKey || key.altKey) {
            if (key.ctrlKey && key.key === " ") { consume(); refresh(true); }
            return;
        }
        ...
        if ((key.key === "Enter" || key.key === "Tab") && !key.shiftKey) {
            if (popup.hidden && key.key === "Tab") refresh();
            if (!popup.hidden && result.items.length) {
                if (accept()) { consume(); return; }
            }
        }
        if (key.key === "Tab" && multiline) {
            ...
            if (start !== end) {
                ...
                if (spaces) replace(lineStart, lineStart + spaces, "");
            } else {
                replace(start, end, "  ");
            }
