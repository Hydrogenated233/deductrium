...
export class AssertionSystem {
    ...
    getNfParams(ast: AST): [AST, AST[], Set<string>] | false {
        ...
        if (ast.name.match(/^#v*nf$/)) {
            ...
            for (; i < ast.nodes.length; i++) {
                const n = this.getVarName(ast.nodes[i]) as string;
                if (!n) { throw TR('系统函数') + ast.name + TR(`第${i + 1}个参数中：`) + TR(`表达式出现在了变量的位置中`); }
                vars.add(n);
            }
    ...
    removeFn(ast: AST) {
        if (ast.type !== "fn" && !ast.name.startsWith("#")) throw TR("无法对非 #fn 节点调用 removeFn");
    ...
    getReplVarsType(ast: AST, res: ReplvarTypeTable = {}, isItem: boolean, resfn: ReplvarTypeTable = {}): ReplvarTypeTable {
        if (ast.type === "replvar") {
            res[ast.name] ??= isItem;
        ...
        if (ast.type === "fn" && !ast.name.startsWith("#")) {
            ...
            if (resfn[ast.name] !== isItem) throw TR("符号 ") + ast.name + TR('不能同时为函数和谓词');
            // return res;
        }
    ...
    getAstType(ast: AST, varLists: ReplvarTypeTable): boolean {
        ...
        if (ast.type === "sym") {
            ...
            if (ast.name === "{|" || ast.name === "|}") return true;
            return true;
        }
        if (ast.type === "fn") {
            if (ast.name === "{" || ast.name === "(") return true;
    ...
    getSubAstMatchTimesAndReplace(
        ...
        allowUnknownOutsideMatch = false
    ): AST[][] | false {
        ...
        if (eq === U) {
            ...
            if (allowUnknownOutsideMatch
                && Object.keys(this.getVarNamesAndIsNots(subAst, {}, null)).length === 0) return res;
    ...
    getSubAstType(ast: AST, idx: number, parentType: boolean) {
        ...
        if (ast.type === "fn") {
            if (ast.name === "#array") return false;
    ...
    expand(ast: AST, isItem: boolean, varLists?: ReplvarTypeTable) {
        ...
        if (rpParams) {
            ...
            const scopes = this.getSubAstMatchTimesAndReplace(
                sub, src, dst, nth, undefined, undefined, right, allowUnknownOutsideMatch
            );
            if (scopes) {
                for (const [idx, scope] of scopes.entries()) {
                    // if not match all, just verify nth
    ...
    checkGrammer(
        ...
        if (ast.type === "sym") {
            if (ast.name === "{|") {
                ...
                if (type !== "p") throw TR("意外出现了量词") + ast.name;
            ...
            if (logicSyms.includes(ast.name)) {
                ...
                if (ast.nodes[1]) this.checkGrammer(ast.nodes[1], "p");
                return;
            }
        }
        if (ast.type === "fn") {
            if (ast.name === "#rp" || ast.name === "#crp") {
                ...
                this.checkGrammer(ast.nodes[1], "i");
                this.checkGrammer(ast.nodes[2], "i");
                if (ast.nodes[3]) this.checkGrammer(ast.nodes[3], "v");
                return;
            }
            if (ast.name.match(/^#v*nf$/)) {
            ...
            if (ast.name === "Prime") {
                ...
                for (const n of ast.nodes) this.checkGrammer(n, "i");
                return;
            }
            if (ast.name === "Equiv") {
