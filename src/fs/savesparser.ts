...
function savedDeductionSteps(value: unknown): SerilizedDeductionStep[] {
    if (!Array.isArray(value) || !Array.isArray(value[2])) return [];
    return value[2].filter(step => Array.isArray(step)) as SerilizedDeductionStep[];
}

function assistantHistoryUsesRule(
    payload: Pick<SerializedAssistantPayload, "history"> | undefined,
    name: string
): boolean {
...
function savedAssistantUsesRule(value: unknown, name: string): boolean {
    if (!Array.isArray(value)) return false;
    return assistantHistoryUsesRule(
        value[5] as SerializedAssistantPayload | undefined,
        name
    );
}

function deductionNameUsesAny(
    fs: FormalSystem,
    deductionIdx: unknown,
    names: ReadonlySet<string>
): boolean {
    if (typeof deductionIdx !== "string" || !deductionIdx) return false;
    const atoms: string[] = [];
    try {
...
function creativeOnlySavedDeductions(
    fs: FormalSystem,
    ...
    while (changed) {
        ...
        for (const [name, value] of Object.entries(dictionary ?? {})) {
            ...
            const dependsOnBlocked = savedDeductionSteps(value).some(step =>
                deductionNameUsesAny(fs, step[0], blocked)
            );
            const replaysBlockedAssistant = Array.from(blocked).some(rule =>
                savedAssistantUsesRule(value, rule)
            );
            if (!dependsOnBlocked && !replaysBlockedAssistant) continue;
            blocked.add(name);
            changed = true;
        }
    }
...
function neutralizeCreativeOnlyPageProofs(
    fs: FormalSystem,
    ...
    for (const page of fs.inferencePages.pages) {
        ...
        for (const proposition of page.propositions) {
            if (!tainted && proposition.from) {
                tainted = deductionNameUsesAny(
                    fs,
                    proposition.from.deductionIdx,
                    blocked
                ) || Array.from(blocked).some(rule =>
                    assistantHistoryUsesRule(proposition.from?.assistant, rule)
                );
            }
            if (!tainted) continue;
...
export class SavesParser {
    ...
    deserializeArr(fs: FormalSystem, arr: any[]) {
        ...
        if (!this.creative) neutralizeCreativeOnlyPageProofs(fs, blocked);
