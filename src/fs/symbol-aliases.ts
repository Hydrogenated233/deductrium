...
export type InferenceSymbolAlias = {
    alias: string;
    symbol: string;
};

export const INFERENCE_SYMBOL_ALIASES: readonly InferenceSymbolAlias[] = Object.freeze([
...
const aliasesByName = new Map(
    INFERENCE_SYMBOL_ALIASES.map(({ alias, symbol }) => [alias.toLowerCase(), symbol])
);

export function inferenceSymbolForAlias(source: string): string | null {
...
function isAliasBoundary(source: string, start: number): boolean {
...
export function expandInferenceAliasesInSurface(source: string): string {
...
export type InferenceAliasExpansion = {
    value: string;
    caret: number;
    symbol: string;
};

export function expandInferenceAliasAtCaret(
    value: string,
    selectionStart: number,
    selectionEnd = selectionStart
): InferenceAliasExpansion | null {
...
export function installInferenceSymbolAliases(input: FormulaInput | null | undefined): void {
    ...
    input.addEventListener("keydown", event => {
        ...
        const expansion = expandInferenceAliasAtCaret(
            input.value,
